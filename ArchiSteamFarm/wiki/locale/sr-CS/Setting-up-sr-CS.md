...
Prvo morate biti sigurni da ASF može biti pokrenut na vašem OS-u. You don't need to know that, but ASF is written in C#, based on .NET platform and may require native libraries that are not available on your platform yet. Think of it like DirectX for the 3D games, or engine for starting your car.
...
Normalno je da neki (ili svi) zahtjevi već postoje na vašem sistemu zbog toga što ih je neki treći softver, koji već koristite, instalirao. Still, that doesn't need to be the case, so you should run appropriate installer for your OS - without those dependencies ASF won't launch at all, and you'll barely get any useful information for telling you what's wrong.
...
Imena paketa zavise od Linux distribucije koju koristite, mi smo naveli najčešće varijacije. Možete ih instalirati sa nativnim paket menađerom na vašoj distribuciji (kao što je `apt` za Debian ili `yum` za CentOS). Those are pretty standard libraries that should be available regardless of what distribution you're using, it's only a matter of finding out how they're called in your environment of choice.
...
Većina, ako ne i sve, bi trebalo da su već instalirane na vašem sistemi. For example, the minimal installation of Debian stable usually requires only `libicu76`.
...
Sada možete pritisnuti "download" dugme i naš web generator konfiguracije će napraviti novi `json` fajl baziran na imenu vašeg bota. Save that file into `config` directory which is located in the folder inside which you've extracted the zip file in the previous step.

Čestitamo! Završili sve veoma jednostavnu konfiguraciju ASF bota. There's a lot more to discover, but for now this is everything that you need.

---

### Pokretanje ASF-a

I know you've been waiting for this moment, and we can't hold you longer, as you're now ready to launch the program for the first time. Jednostavno dvokliknite `ArchiSteamFarm` fajl u ASF direktorijumu. You can also start it from the console.
...
Ovo pokazuje da ASF uspješno radi svoj posao na vašem nalogu, pa ga možete umanjiti i raditi nešto drugo. Go ahead, you deserved it, maybe refill that drink of your choice at least!
...
Drugim riječima, jednostavno ponovo idite na konfiguraciju i uradite istu stvar, za vaš drugi ili treći nalog. Remember to use unique names for all of your bots, to not overwrite existing configs.
...
Sada preuzmite novi konfiguracioni fajl na isti način, pa ga **zamijenite** sa vašim starim. You can also delete your old config file and put the new one in its place of course.
...
As we mentioned before, ASF is a console app and doesn't launch a graphical user interface by default. Ipak, mi aktivno radimo na **[ASF-ui](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC#asf-ui)** interfejsu za naš IPC, koji je veoma lak za korišćenje i pristup raznim ASF mogućnostima.
...
IPC, in a nutshell, is built-in ASF's web server that starts locally on your machine, allowing you to interact with it using your favourite web browser. Assuming that it started correctly, you can access ASF's IPC interface by clicking **[this](http://localhost:1242)** link, as long as ASF is running, from the same machine. You can use ASF-ui for various purposes, e.g. editing the config files in-place or sending **[commands](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**. Slobodno razgledajte okolo da bi pronašli razne ASF-ui funkcije.
...
- Ako želite da naučite sve što postoji u ASF-u i kako vam to može pomoći, posjetite ostatak **[naše wiki-e](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Home)**. Use the sidebar on the right to pick the subject that interests you.
