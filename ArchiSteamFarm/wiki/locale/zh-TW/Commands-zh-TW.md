...
| `2fafinalized [Bots] <2FACodeFromApp>`                                              | `Master`        | 為指定Bot實例使用雙重驗證權杖驗證，匯入已最終化的&#8203;**[雙重驗證](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-zh-TW#建立)**&#8203;憑證。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `fbrm [Bots] <AppIDs>`                                                              | `Master`        | 將給定的&#8203;`AppIDs`&#8203;（或&#8203;`all`&#8203;）從指定Bot實例的應用程式自動掛卡黑名單中移除。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `fq [Bots]`                                                                               | `Master`        | 列出指定Bot實例的優先掛卡佇列。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `fqadd [Bots] <AppIDs>`                                                             | `Master`        | 將給定的&#8203;`AppIDs`&#8203;新增至指定Bot實例的優先掛卡佇列中。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `fqrm [Bots] <AppIDs>`                                                              | `Master`        | 將給定的&#8203;`AppIDs`&#8203;（或&#8203;`all`&#8203;）從指定Bot實例的優先掛卡佇列中移除。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `mabrm [Bots] <AppIDs>`                                                             | `Master`        | 將給定的&#8203;`AppIDs`&#8203;（或&#8203;`all`&#8203;）從&#8203;**[`MatchActively`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/ItemsMatcherPlugin-zh-TW#matchactively)**&#8203;的應用程式自動交易黑名單中移除。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `tbrm [Bots] <SteamIDs64>`                                                          | `Master`        | 將給定的&#8203;`SteamIDs64`&#8203;（或&#8203;`all`&#8203;）從指定Bot的實例交易模組黑名單中移除。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
複數引數，例如&#8203;`[Bots]`&#8203;、&#8203;`<Keys>`&#8203;或&#8203;`<AppIDs>`&#8203;，代表該引數支援多個由逗號分隔的相同類型數值。 舉例來說，&#8203;`status [Bots]`&#8203;可以輸入成&#8203;`status MyBot,MyOtherBot,Primary`&#8203;。 這樣，該指令會在&#8203;**所有的目標Bot**&#8203;上執行，效果等同於向所有的Bot分別傳送&#8203;`status`&#8203;指令。 需要注意的是，逗號「&#8203;`,`&#8203;」後面不能有空格。

ASF使用所有的空白字元作為指令的定界符，例如空格和換行字元。 這代表您不僅可以使用空格來分隔引數，還可以使用任何其他空白字元（例如Tab及換行）。

ASF會將超出指令範圍的多餘引數「連接」至符合語法的最後一個複數引數上。 這代表對於&#8203;`redeem [Bots] <Keys>`&#8203;指令而言，&#8203;`redeem bot key1 key2 key3`&#8203;與&#8203;`redeem bot key1,key2,key3`&#8203;的作用完全相同。 再加上您可以使用換行作為指令定界符，這樣您就可以先輸入&#8203;`redeem bot`&#8203;，然後再貼上產品序號清單，其中的每一項可以由任意空白字元（例如換行字元）或者ASF的標準&#8203;`,`&#8203;作為定界。 請注意，這個技巧僅能用於擁有最多引數的指令變體（所以在這種情形下，指定&#8203;`[Bots]`&#8203;是強制性的；否則您的第一個鍵值會被解譯成執行指令的目標Bot）。
...
此外，需要&#8203;`AppID`&#8203;或&#8203;`SubID`&#8203;的指令足夠智慧，能夠從&#8203;`store.steampowered.com`&#8203;的連結中擷取相關資訊。
...
您可以使用它們的名稱（不區分大小寫）或數值。 未賦值的引數將會被設為預設值&#8203;`Private`&#8203;。 特別注意上述的相依關係，因為指定屬性能夠擁有的最高權限等級取決於相依條目的權限等級。

舉例來說，您&#8203;**無法**&#8203;在將遊戲資料設為&#8203;`Private`&#8203;的前提下，設定遊玩時數為&#8203;`Public`&#8203;；但您可以在個人檔案為&#8203;`Public`&#8203;的情形下，將遊戲資料設為&#8203;`Private`&#8203;。
...
上述指令會將個人檔案設為公開、遊戲資料設為僅限好友、總遊玩時數為私人、好友名單為公開、物品庫為公開、物品庫禮物為私人、個人檔案留言為公開。 若有需要，您也可以使用數值來實現相同效果。
...
若要移除已擁有的授權，兩種方法都可以使用，但我們建議使用Sub來獲得精確且穩定的移除請求，否則Steam有可能會移除您預期外的組合包或授權，特別是您的帳號擁有了同一個App下的多個Sub授權時。 這是因為在系統底層，授權都是透過所屬的組合包來辨識的，因此，若您選擇使用&#8203;`appID`&#8203;來移除，Steam會自行決定要刪除哪一個對應的&#8203;`subID`&#8203;。
