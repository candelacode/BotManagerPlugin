...
若您想要使用不受任何修改的預設ASF紀錄，就什麼也不需要做⸺您也不需要在自訂的&#8203;`NLog.config`&#8203;中定義它。 不過做為參考，硬編碼的ASF預設紀錄等同於：
...
下列範例向您展示了如何依據自身的喜好自訂紀錄。

在一開始，我們將只使用&#8203;**[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)**&#8203;目標。 我們初始的&#8203;`NLog.config`&#8203;看起來像這樣：
...
上述設定的解釋十分簡單：我們定義一個&#8203;**記錄目標**&#8203;，為&#8203;`ColoredConsole`&#8203;，然後我們將&#8203;`Debug`&#8203;及更高層級的&#8203;**所有記錄器**&#8203;（&#8203;`*`&#8203;）重新導向至先前定義的&#8203;`ColoredConsole`&#8203;目標。

若您現在以上述的&#8203;`NLog.config`&#8203;啟動ASF，只有&#8203;`ColoredConsole`&#8203;目標會被啟用，且不論ASF硬編碼的NLog設定為何，ASF都不會寫入&#8203;`File`&#8203;。

因為我們還未定義很多屬性，例如&#8203;`layout`&#8203;，所以它們將會使用預設的內建值來初始化。在本例中，為&#8203;`${longdate}|${level:uppercase=true}|${logger}|${message}`&#8203;。 我們也可以自訂它，例如設定成只記錄訊息：
...
本例使用了&#8203;`SteamTarget`&#8203;，使得名為&#8203;`archi`&#8203;的Bot在收到捐贈交易時，向主Bot的steamID（&#8203;`76561198006963719`&#8203;）傳送一則訊息。 在此過程，需要另一個Bot（因為您無法向自己傳送訊息）。
