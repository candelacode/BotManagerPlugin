...
`--no-restart`&#8203;⸺在預設情形下，ASF會遵照&#8203;**[`AutoRestart`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-zh-TW#autorestart自動重新啟動)**&#8203;設定屬性，您可以用它來指定是否要在需要時重新啟動。 一些我們提供的方案會負責程序管理，它們明顯與ASF的自動重啟功能不相容。例如透過&#8203;`docker`&#8203;或&#8203;`systemd`&#8203;執行ASF，程序只需要結束即可而不必自行重啟，因為這些功能有責任在洽當的時刻重新啟動ASF。 因為直接編輯設定檔對於使用者的體驗來說並不友善，因此這個開關只是簡單地將&#8203;`AutoRestart`&#8203;設定屬性覆寫成&#8203;`false`&#8203;，即使您在設定檔中已指定了不同值。 這樣ASF就可以事先得知在這種環境下執行，而無需提供相容的&#8203;`AutoRestart: false`&#8203;設定檔。

除此之外，與&#8203;`AutoRestart: false`&#8203;不同的是，&#8203;`--no-restart`&#8203;也將禁止您使用&#8203;`restart`&#8203;指令或其他觸發ASF程序重啟的行為，因為這個開關明確宣告了當前部署於不相容重啟行為的環境下；而&#8203;`AutoRestart`&#8203;屬性僅只訂了預設行為。

通常您可以（也應該）在設定檔中控制此類行為，但如果您在自訂腳本或其它類似的環境中執行ASF，您可能也需要使用這個開關來禁止ASF自行重啟。
...
`--system-required`&#8203;⸺使用這個開關，將使ASF嘗試通知作業系統，表明本程序需要在系統執行期間，保持啟動與正常執行狀態。 當您在晚上使用PC或筆電掛卡時，這將會特別有用，因為ASF會在執行時維持系統在甦醒狀態。 Linux與Windows系統目前支援這項功能。

在Linux上，您需要正確運作的&#8203;**[dbus](https://zh.wikipedia.org/zh-tw/D-Bus)**&#8203;，以及支援&#8203;**[`Inhibit()`](https://systemd.io/INHIBITOR_LOCKS/)**&#8203;函數的登入常駐程式。 最常被使用的兩種常駐程式，&#8203;`systemd`&#8203;與&#8203;`elogind`&#8203;，都已被確認有支援。 多數的桌面環境（例如Gnome或KDE）都應直接可用，因為它們自身本就相依於這些功能。

在Windows無特殊需求，應該直接可以使用。