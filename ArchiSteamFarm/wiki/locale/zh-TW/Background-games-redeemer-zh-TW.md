...
不論您選擇使用哪種格式，ASF都會在Bot啟動或執行時匯入您的&#8203;`keys`&#8203;檔案。 在成功剖析並忽略無效的序號後，所有正確偵測到的遊戲都會被加入背景佇列，而&#8203;`BotName.keys`&#8203;檔案也將會自動從&#8203;`config`&#8203;資料夾中移除。 若出現任何無效行，它們將被寫入&#8203;`BotName.keys.invalid`&#8203;檔案中，以供您修正並再次匯入。
