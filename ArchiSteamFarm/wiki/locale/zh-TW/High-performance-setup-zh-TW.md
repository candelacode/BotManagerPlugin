...
./ArchiSteamFarm # 對於適用於特定作業系統的版本
./ArchiSteamFarm.sh # 對於 Generic 版本
```

或在Windows（PowerShell）上：

```powershell
export DOTNET_gcServer=1 ./ArchiSteamFarm # 對於適用於特定作業系統的版本 ./ArchiSteamFarm.sh # 對於 Generic 版本
```
...
套用上述建議，可以使您擁有卓越的ASF效能，在即使啟用了成百上千個Bot後，也能保有快如閃電的效能。 CPU不再成為瓶頸，因為ASF能夠在需要時發揮您CPU的全部能力，將所需時間減少到最低限度。 export DOTNET_gcServer=1 ./ArchiSteamFarm # 對於適用於特定作業系統的版本 ./ArchiSteamFarm.sh # 對於 Generic 版本