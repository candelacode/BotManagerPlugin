...
除非您想要啟用ASF的額外功能，否則您沒有必要獲得／提供授權碼。
...
| 14 | MarketableAscending（可交易升序）          | 嘗試由會掉落無法交易的卡片的遊戲開始掛卡（警告：計算成本高昂）    |
| 15 | MarketableDescending（可交易降序）         | 嘗試由會掉落可以交易的卡片的遊戲開始掛卡（警告：計算成本高昂）    |
...
### `GamingDeviceType（遊戲裝置種類）`

`ushort`&#8203;型別，預設值為&#8203;`1`&#8203;。 本屬性可以在Steam平台啟用一些額外的在線功能，定義如下：

| 值   | 名稱               | 描述               |
| --- | ---------------- | ---------------- |
| 1   | StandardPC（標準PC） | 無特殊模式（預設值）       |
| 544 | SteamDeck        | 將自身當作 Steam Deck |

本屬性依據的基本&#8203;`EGamingDeviceType`&#8203;型別包含了更多可供使用的值，但據我們所知，直到目前它們還仍無任何效果，因此為了可見性將它們刪除了。

若您不確定如何設定本屬性，請保留預設值&#8213;`1`&#8203;。
...
`string`&#8203;型別，預設值為&#8203;`null`&#8203;。 ASF會在登入至Steam網路時使用本屬性，用於自訂Steam如何顯示ASF所在的設備及連線階段。例如&#8203;**[在此處](https://store.steampowered.com/account/authorizeddevices)**&#8203;顯示當前所登入的設備。

ASF提供了數種您能在本文中使用的特殊變數。 `{0}`&#8203;會被取代成您的作業系統的設備名稱；&#8203;`{1}`&#8203;會被取代成ASF的公開識別碼；而&#8203;`{2}`&#8203;會被取代成ASF版本。

除非您知道您在做什麼，否則請保留預設值&#8203;`null`&#8203;。 在這種情形下，ASF會在內部設定正確的值，目前為&#8203;`{0} ({1}/{2})`&#8203;。 請注意，這僅為建議值，Steam網路有可能接受或不接受這個參數。
...
| ---- | ----------------------------------- | ------------------ |
| 0    | 無                                   | 無特殊在線狀態旗標（預設值）     |
| 2    | InJoinableGame（可加入遊玩的遊戲中）           | 用戶端正在遊玩可加入遊玩的遊戲    |
| 8    | RemotePlayTogether（一同遠端遊玩中）         | 用戶端正在遊玩遠端同樂遊戲      |
...
本屬性底層的&#8203;`EUIMode`&#8203;型別包含了更多可供使用的值，但據我們所知，直到目前它們還仍無任何效果，因此為了可見性將它們刪除了。 此外，您可能會有興趣了解&#8203;`GamingDeviceType`&#8203;，因為一些額外的功能需要在那邊啟用。
...
這裡有3種不同含意的可用旗標來開關（&#8203;`A`&#8203;、&#8203;`B`&#8203;、&#8203;`C`&#8203;），因此，一共有&#8203;`8`&#8203;種可能的組合：
...
依據定義，為了實現上述組合，每個獨立的旗標需要是2的冪次。 如果將上述範例新增一個新的旗標&#8203;`D`&#8203;，則它的值必須為&#8203;`8`&#8203;或更大。

一般來說，圖形化工具能協助您計算。 若您手動編輯設定檔，則可以使用計算機，只需將所有要啟用的旗標的基本值加總即可⸺如上述範例所示。

範例：&#8203;`"SteamProtocols": 7`&#8203;（如同上述說明，該數值啟用了值為&#8203;`1`&#8203;、&#8203;`2`&#8203;及&#8203;`4`&#8203;的旗標）
