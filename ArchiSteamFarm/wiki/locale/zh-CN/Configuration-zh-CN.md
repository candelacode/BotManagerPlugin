...
除非您想要启用 ASF 的额外功能，否则无需获取/提供许可证。
...
| 14 | MarketableAscending       | 尝试先挂掉落卡牌无法出售的游戏（警告：计算成本高） |
| 15 | MarketableDescending      | 尝试先挂掉落卡牌可以出售的游戏（警告：计算成本高） |
...
这是一个默认值为 `1` 的 `ushort` 类型属性。 此选项可以启用一些额外的 Steam 平台在线功能，如下所述：

| 值   | 名称         | 描述               |
| --- | ---------- | ---------------- |
| 1   | StandardPC | 无特殊模式，默认值        |
| 544 | SteamDeck  | 显示自己为 Steam Deck |

此属性底层的 `EGamingDeviceType` 类型包括更多可用值，但据我们所知，它们目前并不会有实际效果，因而没有在此包含。

如果您不确定应该如何设置这个属性，请保留默认值 `1`。
...
这是一个默认值为 `null` 的 `string` 类型属性。 当登录 Steam 网络时，ASF 会使用此属性，这可以用来自定义 Steam 如何显示 ASF 所在设备和会话，例如，在[**此处**](https://store.steampowered.com/account/authorizeddevices)显示当前登录的设备。

ASF 提供了一些您可以在文本中使用的特殊变量。 `{0}` 将会被您操作系统提供的计算机名替换。 `{1}` 将会被 ASF 公开标识符替换， `{2}` 将会被 ASF 版本替换。

除非您了解自己在做什么，否则请将其保留为默认值 `null`。 在这种情况下，ASF 会在内部设定正确的值，当前这一值为 `{0} ({1}/{2})`。 请注意，这只是对 Steam 网络的推荐值，它不一定完全采用您的设置。
...
| ---- | ------------------ | -------------- |
| 0    | None               | 无特殊在线状态，默认值    |
| 2    | InJoinableGame     | 客户端正在进行可加入的游戏  |
| 8    | RemotePlayTogether | 客户端正在进行远程同乐会话  |
...
此属性底层的 `EUIMode` 类型包括更多可用值，但据我们所知，它们目前并不会有实际效果，因而没有在此包含。 此外，您可能有兴趣了解 `GamingDeviceType`，因为一些额外功能需要在那里启用。
...
`log.txt` 文件保存您上次运行 ASF 时生成的日志。 此文件不包含任何敏感信息，并且在涉及问题、崩溃或仅提供上次运行 ASF 的信息时非常有用。 如果您遇到了问题或漏洞，我们会经常需要您提供此文件。 ASF 会自动管理这些文件，但如果您是一名高级用户，则可以进一步调整 ASF 的[**日志**](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Logging-zh-CN)模块。
...
这里有 3 个不同含义的可用 Flag 可以任意开关（`A`、`B`、`C`），因此共有 `8` 种可能的组合：
...
根据定义，为了实现以上组合，每个独立 Flag 的值必须是 2 的幂。 因此，如果向上面的示例添加一个新的 Flag `D`，则它的值就必须至少是 `8`。

通常，图形工具将辅助您进行计算。 如果您手动编辑配置，则可以使用计算器，您只需要查看所有要启用的 Flag，将它们的基础值相加——如上例所示。

示例：`"SteamProtocols": 7`（如上所述，启用的 Flag 是 `1`、`2` 和 `4`）
