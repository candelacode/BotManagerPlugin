...
ASF 包含一个独特的 IPC 接口，可以用来与进程进行进一步交互。 IPC 指的是[**进程间通信**](https://en.wikipedia.org/wiki/Inter-process_communication)（Inter-Process Communication），或者简单来说是基于 [**Kestrel HTTP 服务端**](https://learn.microsoft.com/aspnet/core/fundamentals/servers/kestrel)的“ASF Web 接口”，它可以作为最终用户使用的前端（ASF-ui）或者第三方集成工具使用的后端（ASF API），与 ASF 进程进行交互。
...
此时，ASF 的 HTTP 服务器将会监听指定的端点。 如果你没有为 IPC 提供自定义配置文件，则 IPC 将会运行于 `localhost`、IPv4 地址 **[127.0.0.1](http://127.0.0.1:1242)** 和 IPv6 地址 **[[::1]](http://[::1]:1242)** 上的 `1242` 默认端口。 您可以通过上面的两个链接访问我们的 IPC 接口，但前提是必须在同一台机器上运行 ASF。
...
如果您不需要允许所有来源的访问，例如仅允许局域网，那么更好的办法是检查部署 ASF 的设备本地 IP 地址，例如使用 `192.168.0.10` 而非示例中的 `*`。 遗憾的是，只有您的局域网地址始终相同时才能这样配置，否则，使用 `*`，然后额外配置只允许本地子网访问 ASF 端口的防火墙，可能更适合您。
...
![Swagger 文档](https://github.com/user-attachments/assets/ce998e08-f5db-46b0-a9e8-e6b69abd94bb)
...
是的，我们建议您为此设置反向代理。 通过反向代理，您访问的是 Web 服务器，然后借此作为跳板访问服务器上的 ASF IPC。 或者，如果您不打算运行反向代理，您可以将[**自定义配置**](#允许任何-ip-访问)中的 URL 修改为合适的地址。 例如，如果您的设备处于一个 VPN 网络中，地址为 `10.8.0.1`，您就可以在 IPC 配置中设置监听地址为 `http://10.8.0.1:1242`，这将会且仅会在您的这个私有 VPN 网络中启用 IPC。
...
#       proxy_set_header Host 127.0.0.1;
        ...
        # 如果您无法在同一台机器上同时运行 Nginx 与 ASF，您很可能还应该额外设置正确的 KnownNetworks
