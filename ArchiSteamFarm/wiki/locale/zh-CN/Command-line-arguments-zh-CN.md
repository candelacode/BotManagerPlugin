...
`--no-restart`——默认情况下，ASF 会遵守 **[`AutoRestart`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-zh-CN#autorestart)** 配置属性，您可以用它来指定是否允许在需要时重启。 我们提供的一些部署方案会接管进程管理，明确不兼容 ASF 的自动重启功能，例如通过 `docker` 或 `systemd` 运行 ASF 的情况，它们会在进程结束后按需将其重启，进程仅需要退出而非自行重启。 因为直接编辑配置文件对用户体验不友好，此开关会简单地将 `AutoRestart` 配置属性以 `false` 初始化，即使配置文件中已经指定了值。 这样，ASF 就可以事先感知到自身在这类环境下运行，而无需用户在配置文件中提供对应的 `AutoRestart: false`。

除此之外，与 `AutoRestart: false` 不同的是，`--no-restart` 还将禁止您使用 `restart` 命令或其他触发 ASF 重启的操作，因为此选项开关明确说明当前部署方式不兼容重启操作，而 `AutoRestart` 配置仅指定默认行为。

通常您可以（也应该）在配置文件中控制这些行为，但如果您在自定义脚本或其他相似环境下运行 ASF，您可能也需要使用此开关，阻止 ASF 自行重启。
...
`--system-required`——声明这个开关将会导致 ASF 尝试通知操作系统“此进程需要在运行过程中保持系统处于启动状态并正常运行”。 当您在夜间使用 PC 或者笔记本电脑挂卡时，这一功能是相当实用的，因为 ASF 将会在运行时保持计算机处于唤醒状态。 Linux 和 Windows 目前支持此功能。

在 Linux 上，您需要正常工作的 **[dbus](https://en.wikipedia.org/wiki/D-Bus)** 以及支持 **[`Inhibit()`](https://systemd.io/INHIBITOR_LOCKS/)** 函数的登录守护进程。 两个最流行的守护程序，`systemd` 和 </a>elogind</strong> 都确认支持。 大多数桌面环境（例如 Gnome 或 KDE）应该直接可用，因为它们自身就需要依赖这些功能才能运行。

在 Windows 上没有特殊要求，应该直接可用。