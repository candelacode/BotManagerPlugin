...
首先，通过使用 `BotBehaviour` 中的 `DisableIncomingTradesParsing` 选项，您可以禁用**所有**传入的交易报价。 这个选项，顾名思义，会禁用所有与解析收到交易报价相关的功能，包括下面的默认逻辑，也包括所有需要对传入交易报价作出回应的额外功能。 因为默认配置已经是非侵入性的，您应该只在希望 ASF 不对收到的交易报价作出任何动作时，才考虑使用此选项。

传入交易报价解析功能默认启用，下面解释了该功能的逻辑。
