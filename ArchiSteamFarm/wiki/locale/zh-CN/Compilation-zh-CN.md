...
或者，您可以直接构建 ASF-ui，无论是手动还是通过我们仓库的帮助来实现，然后手动复制构建输出到 `${OUT}/www` 目录，其中 `${OUT}` 是您通过 `-o` 参数指定的 ASF 输出目录。 这正是 ASF 在构建过程中做的事，如果 `ASF-ui/dist` 存在，它就将其复制到 `${OUT}/www`，没有任何特殊操作，如果需要，您也可以等 ASF 构建完成再执行。
...
如果您不知道如何选择，我们推荐 **[JetBrains Rider](https://www.jetbrains.com/rider)** 和 **[Visual Studio Code](https://code.visualstudio.com/download)**，前者是 ASF 团队实际偏好使用的 IDE，而后者也是可行的选择。 这两个程序都是跨平台的，可以在 Linux、macOS 和 Windows 上获得。
