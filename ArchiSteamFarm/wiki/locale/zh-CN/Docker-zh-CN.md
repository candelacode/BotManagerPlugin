...
ASF 有几种类型的[**分支**](https://hub.docker.com/r/justarchi/archisteamfarm/tags)。


### `main`

GitHub 中 `main` 分支最新提交构建的 ASF Generic 构建，其工作方式等同于直接从 **[CI](https://github.com/JustArchiNET/ArchiSteamFarm/actions/workflows/publish.yml?query=branch%3Amain)** Pipeline 拉取最新 Artifact。 它用于开发目的，是为开发人员和高级用户准备的，有最高的漏洞风险。 该映像会在每次提交到 GitHub `main` 分支后更新，因此您会发现它的变更十分频繁（并且经常出错）。 该分支记录了 ASF 项目的当前状态，但该状态不一定稳定或者经过测试，就像我们在发布周期中描述的那样。 **这个分支不应该在任何的生产环境中使用**。 该分支可以用于验证特定提交是否修复了您遇到的问题，而不必等待包含该提交的预览版发布。


### `released`

始终指向最新[**发布**](https://github.com/JustArchiNET/ArchiSteamFarm/releases)的 ASF Generic 构建，包括预览版本。 与 `main` 分支不同，该映像会在推送新的 GitHub 版本标签时更新。 一些高级用户喜欢立刻尝试最新的功能，选择处于稳定边缘的版本，这一分支就是为他们准备的。 实际上，此版本等价于拉取时指向最新发布的 `A.B.C.D` 滚动标签的版本。 请注意，使用此分支等同于使用我们的[**预览版**](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-zh-CN)。

### `stable`

始终指向最新发布的[**稳定版**](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest) ASF Generic 构建。 与 `released` 分支不同，该映像会在 ASF 发布新稳定版时更新。 如果您需要上文 `released` 的稳定版本，则推荐使用这个分支。

### `latest`

始终指向最新发布的[**稳定版**](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest) ASF 操作系统构建。 与其他分支相比，这是**唯一**支持 ASF 自动更新的分支。 此分支的目标是提供一个默认的 Docker 容器，支持 ASF 操作系统包的自动更新功能。 因此，此映像不需要频繁更新，因为其中的 ASF 会在需要时自动更新。

当然，`UpdatePeriod` 可以被安全禁用（设置为 `0`），但在这种情况下，您可能更应该选择 `stable` 分支。 同样，如果愿意，您还可以修改默认的 `UpdateChannel` 来跟进预览版。
...
指向固定 ASF 标签的 Generic 构建。 与上面的分支相比，这个分支是完全冻结的，这意味着映像一旦发布就不会再更新。 这类似于我们的 GitHub 发布版本，一经发布就不会更改，这保证了环境的长期稳定。 通常，如果您希望使用特定的 ASF 版本，并且希望构建拥有确定性的行为（例如，自行管理 ASF 版本），就应该使用这种分支。

---

## 哪个分支最适合我？

这取决于您的目标。 对于大多数用户来说，`latest` 分支是最好的，因为它的行为与在桌面上运行 ASF 是相同的，区别仅仅在于它以服务形式运行在 Docker 容器内。 然而，Docker 并非一定要这样用——通常，您会重建容器，而不是让它永远运行，这样的话，您其实可以考虑使用更符合 Docker 规范的 `stable` 或 `released` 分支。 最后，如果您想要运行固定的 ASF 版本，那么使用 `A.B.C.D` 版本就是更合理的选择。
...
有两种方法修改运行 ASF 的用户。 我们推荐使用第一种方式，即声明 `ASF_UID` 环境变量，其值为您要运行 ASF 的用户 UID。 另一种方式，则需要传入 `--user` **[参数](https://docs.docker.com/engine/reference/run/#user)**，由 Docker 直接支持。
...
`ASF_UID` 与 `--user` 参数之间的区别很小，但很重要。 `ASF_UID` 是由 ASF 支持的自定义机制，在这个场景下，Docker 容器仍然以 `root` 用户启动，然后 ASF 的启动脚本会以 `ASF_UID` 指定的用户运行主程序。 在使用 `--user` 参数时，您是以指定的用户启动整个进程，包括 ASF 启动脚本。 第一个选项允许 ASF 启动脚本自动为您处理权限等配置，解决一些您可能会造成的常见问题，例如，它能确保您的 `/app` 和 `/asf` 目录的所有者是 `ASF_UID`。 在第二种情况下，因为我们没有以 `root` 运行，就无法做到这一点，您必须自己提前处理好所有问题。

如果您决定使用 `--user` 参数，您需要将所有 ASF 文件的所有者从默认的 `0000` 更改为您设定的新用户。 您可以执行下面的命令做到这一点：

```shell
# 仅在您未使用 ASF_UID 的情况下执行
docker exec -u root ASF容器名 chown -hR 1001 /app /asf
```
...
除非您需要提供自定义加密密钥或者使用其他高级选项，否则通常您不需要指定任何环境变量，因为我们的 Docker 容器已被配置为使用合适的默认选项 `--no-restart`，您就不必在 `ASF_ARGS` 中指定这些参数。
...
您可以使用如下 `docker compose` 配置来达成与上述 `docker run` 命令相同的效果：
...
除了上述情况以外，我们还在每个示例中添加了 `--restart unless-stopped` 参数，这是为了让容器跟随机器自动重启。 您可以根据需求自行删除或修改。
