...
我们的网站会显示在**过去 15 分钟**内在线的所有符合要求的 ASF 帐户。 用户将会按照他们相关性排序——排在最前面的是启用 `MatchEverything` 的机器人，它们会显示 `Any` 标记表示接受所有 1:1 交易，然后再按游戏数排序，最后按物品数排序。
...
一旦您停止使用（上报）我们的公共列表，最多在 15 分钟内，您的数据就不再公开可见，然后以其他形式在我们的服务器上保留最多两周——所有数据都会这之后自动删除。 全程不需要您介入操作。
...
`MatchActively` 以官方 `ItemsMatcher` 插件的形式复活，使 ASF 仍然有能力进行主动卡牌匹配，但基于完全不同的概念。 复活的 `MatchActively` 功能需要我们在 ASF 后端进行**巨量的工作**，包括全新的托管服务器和上百个用于解析库存的付费代理，一切都专门用来让 ASF 客户端能像以前一样使用 `MatchActively`。 由于涉及的工作量非常大，并且相关资源（域名、服务器、代理）并非免费，而是需要我们每月付费，我们决定仅将此功能提供给我们的赞助者，也就是已经每月支持 ASF 项目的人，没有他们的支持，我们就无法提供这些付费资源。
...
无论如何，我们理解不是每个人都需要 `MatchActively`，并且我们希望您也能够理解为什么我们无法免费提供它。 如果没有人愿意帮助分担这个功能的成本，它从一开始就根本不会存在，因为我们将不得不砍掉这部分没人愿意负担的月度支出。 好在，目前的情况没有这么差，您也可以按自己的意愿决定是否使用 `MatchActively` 功能。
