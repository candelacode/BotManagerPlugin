...
- `linux-arm`，支持 32 位基于 ARM（ARMv7+）并包含 glibc 2.35 / musl 1.2.3 或更高版本的 GNU/Linux 操作系统。 此包涵盖了 Raspberry Pi 2（或者更新）这类平台，**不**支持更早的 ARM 架构，例如 Raspberry Pi 0 & 1 使用的 ARMv6，也不支持未实现所需 GNU/Linux 环境的操作系统（例如 Android）。
- `linux-arm64`，支持 64 位基于 ARM（ARMv8+）并包含 glibc 2.27 / musl 1.2.3 或更高版本的 GNU/Linux 操作系统。 此包涵盖了 Raspberry Pi 3（或者更新）这类平台，**不**支持 32 位操作系统（例如 32 位 Raspberry Pi OS），因为它们缺少所需的 64 位库，也不支持未实现所需 GNU/Linux 环境的操作系统（例如 Android）。
- `linux-x64` 支持 64 位并包含 glibc 2.27 / musl 1.2.4 或更高版本的 GNU/Linux 操作系统。
- `osx-arm64` 支持 64 位 ARM 架构（Apple silicon）的 macOS 13 或更高版本的操作系统。
- `osx-x64` 支持 64 位 macOS 15 或更高版本的操作系统。
- `win-arm64`，支持 64 位 ARM 架构（ARMv8+）的 Windows 10、11 或者更高版本操作系统的**最新版本**。
- `win-x64`，支持 64 位 Windows 10、11、Server 2016+ 或者更高版本操作系统的**最新版本**。
...
您可以访问[**发行说明**](https://github.com/dotnet/core/blob/main/release-notes/10.0/supported-os.md)查看完整的 .NET 10.0 支持的平台与操作系统列表。
...
ASF 目前指向的构建目标是 **.NET 10.0**（`net9.0`），但在未来可能会指向更高版本。 `net10.0` 自 10.0.100 SDK（10.0.0 运行时环境）以来就受到支持，但 ASF 也许更偏向于**编译时最新版本的运行时环境**，所以您应该确保您的机器上有[**最新版 SDK**](https://dotnet.microsoft.com/download)（或至少有运行时环境）。 如果您的运行时环境版本低于编译时的最低支持版本，Generic ASF 包将会拒绝启动。
