...
- `linux-arm64` funciona en sistemas operativos GNU/Linux de 64 bits basados en ARM (ARMv8+) con glibc 2.27/musl 1.2.3 en adelante. Esta variante cubre plataformas tales como Raspberry Pi 3 (y más recientes), **no** funcionará con sistemas operativos de 32 bits que no tengan las bibliotecas de 64 bits requeidas (tal como Raspberry Pi OS de 32 bits), tampoco funcionará con sistemas operativos que no implementen el entorno GNU/Linux requerido (tal como Android).
- `linux-x64` funciona con sistemas operativos GNU/Linux de 64 bits con glibc 2.27/musl 1.2.3 en adelante.
- `osx-arm64` funciona en sistemas operativos macOS de 64 bits basados en ARM (Apple silicon) en versión 13 y más recientes.
- `osx-x64` funciona en sistemas operativos macOS de 64 bits en versión 15 en adelante.
- `win-arm64` funciona  en sistemas operativos Windows **actualizados** de 64 bits basados en ARM (ARMv8+), en versión 10, 11 y más recientes.
- `win-x64` funciona en sistemas operativos Windows **actualizados** de 64 bits, en versión 10, 11, Server 2016+ y más recientes.
...
Para una lista completa de las plataformas y sistemas operativos soportados por .NET 10.0, visita las **[notas de publicación](https://github.com/dotnet/core/blob/main/release-notes/10.0/supported-os.md)**.
...
ASF como programa actualmente se basa en **.NET 10.0** (`net10.0`), pero podría usar una plataforma más nueva en el futuro. `net10.0` es soportado desde 10.0.100 SDK (10.0.0 runtime), aunque ASF podría preferir el **último runtime al momento de la compilación**, así que debes asegurarte de tener el **[último SDK](https://dotnet.microsoft.com/download)** (o al menos runtime) disponible para tu máquina. La variante genérica de ASF podría negarse a iniciar si tu runtime es más antiguo que el mínimo soportado durante la compilación.
