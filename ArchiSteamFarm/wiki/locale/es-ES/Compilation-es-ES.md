...
Alternativamente, puedes compilar solamente ASF-ui, ya sea manualmente o con la ayuda de nuestro repositorio, luego copia manualmente la compilación a la carpeta `${OUT}/www`, donde `${OUT}` es la carpeta de salida de ASF que especificaste con el parámetro `-o`. Esto es exactamente lo que hace ASF como parte del proceso de compilación, copia `ASF-ui/dist` (si existe) a `${OUT}/www`, nada especial, y también puede hacerse después de la compilación como puedes ver, en caso de ser necesario.
...
Si no tienes una mejor opción recomendamos **[JetBrains Rider](https://www.jetbrains.com/rider)** y **[Visual Studio Code](https://code.visualstudio.com/download)**, con el primero siendo el IDE preferido que el equipo de ASF usa personalmente, y el segundo siendo una alternativa viable. Ambos programas son multiplataforma y están disponibles en Linux, macOS y Windows.
