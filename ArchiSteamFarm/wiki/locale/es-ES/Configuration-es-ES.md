...
A menos que quieras habilitar funcionalidades adicionales de ASF, no hay necesidad de obtener/proporcionar la licencia.
...
| 14    | MarketableAscending       | Trata de cultivar juegos con gotas de tarjetas sin marca primero (advertencia: costoso de cálculo)     |
| 15    | MarketableDescending      | Trata de criar juegos con gotas de tarjetas de mercado primero (advertencia: costoso de calcular)      |
...
Tipo `ushort` con valor predeterminado de `1`. Esta propiedad puede habilitar algunas características en línea adicionales en la plataforma de Steam, y se define a continuación:

| Valor | Nombre     | Descripción                       |
| ----- | ---------- | --------------------------------- |
| 1     | StandardPC | Sin modo especial, predeterminado |
| 544   | SteamDeck  | Se presenta como Steam Deck       |

El tipo `EGamingDeviceType` subyacente en el que se basa esta propiedad incluye más banderas disponibles, sin embargo, hasta donde sabemos no tienen ningún efecto actualmente, por lo que fueron omitidas.

Si no estás seguro de cómo establecer esta propiedad, déjala con su valor predeterminado de `1`.
...
Tipo `string` con valor predeterminado de `null`. ASF utilizará esta propiedad al iniciar sesión en la red de Steam, que puede ser usado para personalizar cómo exactamente Steam mostrará la máquina y sesión de ASF, por ejemplo, al mostrar los dispositivos que están conectados actualmente **[aquí](https://store.steampowered.com/account/authorizeddevices)**.

ASF proporciona algunas variables especiales que puedes usar opcionalmente en tu texto. `{0}` será reemplazado por el nombre de la máquina según sea proporcionado por tu sistema operativo, `{1}` será reemplazado por el identificador público de ASF, y `{2}` será reemplazado por la versión de ASF.

A menos que sepas lo que estás haciendo, deberías mantenerlo con el valor predeterminado de `null`. En este caso, ASF decidirá internamente sobre el valor adecuado, que es `{0} ({1}/{2})`. Ten en cuenta que esta es sólo una sugerencia de que la red de Steam pueda, o no, respetar en su totalidad.
...
| 2     | InJoinableGame     | El cliente está en un juego al que es posible unirse      |
| 8     | RemotePlayTogether | El cliente está usando una sesión de Remote Play Together |
...
El tipo `EUIMode` subyacente en el que se basa esta propiedad incluye más banderas disponibles, sin embargo, hasta donde sabemos no tienen ningún efecto actualmente, por lo que fueron omitidas. Además, podría interesarte revisar `GamingDeviceType`, ya que algunas características adicionales están habilitadas allí.
...
El archivo `log.txt` contiene el registro generado por la última ejecución de ASF. Este archivo no contiene ninguna información confidencial, y es extremadamente útil cuando se trata de problemas, fallos o simplemente como información de lo que ocurrió en la última ejecución de ASF. A menudo solicitaremos este archivo si te encuentras con problemas o bugs. ASF gestiona este archivo automáticamente, pero puedes modificar el módulo de **[registro](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Logging-es-ES)** de ASF si eres un usuario avanzado.
...
`BotName.keys` es un archivo especial que puede usarse para importar claves al **[activador de juegos en segundo plano](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Background-games-redeemer-es-ES)**. No es obligatorio y no se genera, pero ASF lo reconoce. Este archivo se elimina automáticamente después de que las claves son importadas exitosamente.

`BotName.maFile` es un archivo especial que puede usarse para importar **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-es-ES)**. No es obligatorio y no se genera, pero ASF lo reconoce si tu `BotName` todavía no usa ASF 2FA. Este archivo se elimina automáticamente después de que ASF 2FA es importado exitosamente.
...
**Recomendamos usar ConfigGenerator para generar configuraciones** - se encarga de la mayoría de cosas básicas (como la validación de los tipos), para que solo tengas que introducir los valores correctos, y tampoco tengas que entender los tipos de variables especificados a continuación. Esta sección es principalmente para las personas que generan/editan las configuraciones manualmente, para que sepan qué valores pueden usar.
...
`ImmutableDictionary<keyType, valueType>` - Diccionario (mapa) inmutable que mapea una clave única especificada en su `keyType`, al valor especificado en su `valueType`. En JSON, está definido como un objeto con pares clave-valor. Ten en cuenta que `keyType` siempre va entre comillas en este caso, incluso si es un tipo de valor, tal como `ulong`. También existe un requisito estricto de que la clave sea única en el mapa, también impuesto por JSON.

Ejemplo para `ImmutableDictionary<ulong, byte>`: `"SteamUserPermissions": { "76561198174813138": 3, "76561198174813137": 1 }`

---

`flags` - Los atributos de bandera combinan varias propiedades diferentes en un valor final aplicando operaciones de bits. Esto permite elegir cualquier combinación posible de diferentes valores permitidos al mismo tiempo. El valor final es el resultado de la suma de los valores de todas las opciones habilitadas.

Por ejemplo, dada la siguiente definición:
...
Hay exactamente 3 banderas importantes, disponibles para activar/desactivar (`A`, `B`, `C`), y por lo tanto `8` posibles combinaciones válidas en total:

| Valor final | Banderas activadas |
| ----------- | ------------------ |
| 0           | `None`             |
| 1           | `A`                |
| 2           | `B`                |
...
Por definición, para hacer posible lo anterior, cada bandera independiente debe ser potencia de dos. Por eso, en el ejemplo anterior una bandera adicional, `D`, necesitaría tener el valor de `8` o mayor.

Generalmente, las herramientas gráficas harán el cálculo por ti. Si estás editando configuraciones manualmente, siempre puedes usar una calculadora y añadir los valores subyacentes de todas las banderas que quieras activar - como en el ejemplo anterior.

Ejemplo: `"SteamProtocols": 7` (lo que activa las banderas con valor de `1`, `2` y `4`, como se explicó anteriormente)
...
Debido a lo anterior, ASF automáticamente migrará/optimizará tus configuraciones dándoles formato y eliminando los campos que tengan el valor predeterminado. Puedes desactivar este comportamiento con el **[argumento de la línea de comandos](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments-es-ES#argumentos)** `--no-config-migrate` si tienes una razón específica, por ejemplo, si estás proporcionando archivos de configuración de solo lectura y no deseas que ASF los modifique.
...
Puedes desactivar este comportamiento con el **[argumento de la línea de comandos](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments-es-ES#argumentos)** `--no-config-watch` si tienes una razón específica, por ejemplo, si no deseas que ASF reaccione a cambios de archivos en la carpeta `config`.