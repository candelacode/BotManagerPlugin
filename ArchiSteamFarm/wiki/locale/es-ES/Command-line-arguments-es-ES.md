...
`--no-restart` - por defecto ASF respeta la propiedad de configuración **[`AutoRestart`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#autorestart)**, que puedes usar para especificar si el reinicio está permitido cuando sea necesario. Algunas soluciones que proporcionamos se encargan de la gestión del proceso y son explícitamente incompatibles con la función de reinicio automático de ASF, tal como ejecutar ASF en `docker` o `systemd`, ya que requieren que el proceso finalice, dado que es su responsabilidad reiniciarlo después, si lo consideran necesario. Debido a que la edición arbitraria de la configuración no es deseable para la experiencia de usuario, esta opción anula la propiedad de configuración `AutoRestart` al inicializarla explícitamente como `false`, incluso si especificaste otra cosa en la configuración. Gracias a eso, ASF puede ser informado de antemano de su ejecución en tal entorno, sin necesidad de proporcionar `AutoRestart: false` en el archivo de configuración.

Además de lo anterior, `--no-restart`, contrario a `AutoRestart: false`, también impide usar el comando `restart` o permitir que el proceso de ASF reinicir de otro modo, ya que la opción explícitamente indica que no es compatible con dicha configuración, mientras que la propiedad `AutoRestart` solo especifica el comportamiento predeterminado.

Normalmente puedes (y deberías) controlar el comportamiento explicado aquí en el archivo de configuración, aunque si ejecutas ASF dentro de un script personalizado u otro entorno similar, también podrías querer usar esta opción, que impedirá que ASF se reinicie automáticamente.
...
`--system-required` - declarar esto causará que ASF intente indicar al sistema operativo que el proceso requiere que el sistema esté activo y ejecutándose durante todo su tiempo de vida. Esto puede ser especialmente útil al recolectar en tu PC o laptop durante la noche, ya que ASF podrá mantener tu sistema despierto mientras se está ejecutando. Esta característica es soportada actualmente en Linux y Windows.

En Linux, necesitarás **[dbus](https://es.wikipedia.org/wiki/D-Bus)** que funcione correctamente con login daemon que soporte la función **[`Inhibit()`](https://systemd.io/INHIBITOR_LOCKS/)**. Los dos daemons más populares, `systemd` así como `elogind`, están confirmados que soportan eso. La mayoría de entornos de escritorio (tales como Gnome o KDE) deberían funcionar al instante, puesto que ya dependen de esta funcionalidad para sus propias necesidades.

Sin requerimientos especiales en Windows, debería funcionar inmediatamente.