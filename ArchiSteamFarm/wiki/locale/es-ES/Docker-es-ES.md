...
ASF está disponible en varios tipos de  **[etiquetas](https://hub.docker.com/r/justarchi/archisteamfarm/tags)**:


### `main`

Compilácion genérica de ASF, compilada a partir del commit más reciente en la rama `main`, que funciona igual que tomar el último artefacto directamente de nuestro flujo de **[CI](https://github.com/JustArchiNET/ArchiSteamFarm/actions/workflows/publish.yml?query=branch%3Amain)**. Es el nivel más alto de software con errores, dedicado a desarrolladores y usuarios avanzados para fines de desarrollo. La imagen se actualiza con cada commit en la rama `main` de GitHub, por lo tanto puedes esperar cambios muy frecuentes (y errores). Esta etiqueta está para marcar el estado actual del proyecto ASF, que no necesariamente se garantiza que sea estable o probado, tal como se indica en nuestro ciclo de lanzamiento. **Esta etiqueta no debe utilizarse en ningún entorno de producción**. Es útil para verificar si un commit en particular corrigió un error detectado, sin esperar a un prelanzamiento con ese commit.


### `released`

Compilación genérica de ASF que siempre apunta a la última versión **[publicada](https://github.com/JustArchiNET/ArchiSteamFarm/releases)** de ASF, incluyendo los prelanzamientos. Comparado con la etiqueta `main`, esta imagen se actualiza cada vez que se hace push a una nueva etiqueta de GitHub. Dedicado a usuarios avanzados que les encanta vivir al límite de lo que se puede considerar estable y fresco al mismo tiempo. En la práctica funciona igual que usar la etiqueta apuntando a la versión `A.B.C.D` más reciente al momento de la solicitud. Ten en cuenta que usar esta etiqueta equivale a usar nuestros **[prelanzamientos](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle)**.

### `stable`

Compilación genérica de ASF que siempre apunta a la última versión **[estable](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** de ASF. Comparada con la etiqueta `released`, esta imagen se actualiza cuando una nueva versión estable de ASF está disponible. Recomendado para personas que buscan una variante estable de la etiqueta `released` mencionada anteriormente.

### `latest`

Compilación de sistema operativo específico de ASF que siempre apunta a la última versión **[estable](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** de ASF. En comparación con otras, esta es la **única etiqueta** que incluye actualizaciones automáticas para ASF. El objetivo de esta etiqueta es proporcionar un contenedor Docker por defecto que sea capaz de ejecutar una compilación autoactualizable de ASF para sistema operativo específico. Debido a eso, la imagen no tiene que actualizarse con tanta frecuencia, ya que la versión incluida de ASF siempre será capaz de actualizarse si es necesario.

Por supuesto, `UpdatePeriod` puede desactivarse de forma segura (establecido a `0`), pero en este caso probablemente debas usar una versión `stable`. Del mismo modo, puedes modificar valor predeterminado de `UpdateChannel` para hacer un seguimiento de los prelanzamientos si lo deseas.
...
Compilación genérica de ASF que apunta a la versión fija de ASF que coincide con la etiqueta. En comparación con las etiquetas anteriores, esta etiqueta está completamente congelada, lo que significa que la imagen no se actualizará una vez publicada. Esto funciona similar a nuestras publicaciones de GitHub que nunca se tocan después de la publicación inicial, lo que te garantiza un entorno estable y congelado. Normalmente deberías usar esta etiqueta cuando quieras usar alguna versión específica de ASF y esperes un resultado determinista de la compilación (por ejemplo, gestionar tú mismo las versiones de ASF).

---

## ¿Qué etiqueta es mejor para mí?

Eso depende de lo que busques. Para la mayoría de los usuarios, la etiqueta `latest` debería ser la mejor ya que ofrece exactamente lo que hace ASF de escritorio, solo que en un contenedor Docker especial como servicio. Sin embargo, docker no debe ser usado necesariamente así - normalmente se espera que reconstruyas tus contenedores y no que los ejecutes por siempre, y en este caso deberías considerar la etiqueta `stable` o `released`, que sigue las pautas de docker. Por último, si quieres ejecutar alguna versión fija de ASF, naturalmente las versiones `A.B.C.D` son lo que buscas.
...
La diferencia entre `ASF_UID` y `--user` es sutil, pero importante. `ASF_UID` es un mecanismo personalizado soportado por ASF, en este escenario el contenedor docker aún inicia como `root`, y después el script de inicio de ASF comienza el ejecutable principal bajo `ASF_UID`. Al usar el parámetro `--user`, estás iniciando todo el proceso, incluyendo el script de inicio de ASF como un usuario determinado. La primera opción permite al script de inicio de ASF manejar permisos y otras cosas automáticamente, resolviendo algunos problemas comunes que podrías haber causado, por ejemplo, asegura que tus directorios `/app` y `/asf` realmente sean propiedad de `ASF_UID`. En el segundo escenario, ya que no estamos ejecutando como `root`, no podemos hacer eso, y se espera que tú mismo lo hagas con antelación.

Si has decidido usar la bandera `--user`, necesitas cambiar la propiedad de todos los archivos de ASF de `1000` por defecto a tu nuevo usuario personalizado. Puedes hacerlo ejecutando el siguiente comando:
...
A menos que desees proporcionar una clave de cifrado personalizada u otras opciones avanzadas, normalmente no necesitas incluir ninguna variable de entorno especial, puesto que nuestros contenedores docker ya están configurados para ejecutarse con la opción predeterminada de `--no-restart`, así que esa bandera no necesita ser especificada explícitamente en `ASF_ARGS`.
...
Puedes lograr el mismo efecto del comando `docker run` usando la siguiente configuración de `docker compose`:
...
De cosas distintas a las ya mencionadas anteriormente, hemos añadido `--restart unless-stopped` a ambos ejemplos para iniciar el contenedor automáticamente después de reiniciar tu máquina. Siéntete libre de eliminarlo/cambiarlo para que se adapte a tus necesidades.
