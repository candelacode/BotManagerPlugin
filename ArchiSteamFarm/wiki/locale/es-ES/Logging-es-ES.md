...
Si quieres usar el registro por defecto de ASF sin modificaciones, no necesitas hacer nada - tampoco necesitas definirlo en `NLog.config`. Para referencia, el equivalente del registro predeterminado de ASF sería:
...
Los ejemplos a continuación muestran cómo puedes personalizar el registro de eventos a tu gusto.

Para empezar solo usaremos el objetivo **[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)**. Nuestro `NLog.config` inicial se verá así:
...
La explicación de la configuración de arriba es bastante sencilla - definimos un **objetivo de registro**, que es `ColoredConsole`, luego redirigimos **todos los loggers** (`*`) de nivel `Debug` y superior al objetivo `ColoredConsole` que definimos previamente.
...
Ya que no definimos muchas propiedades, tal como `layout`, se inicializó con un valor predeterminado, en este caso `${longdate}|${level:uppercase=true}|${logger}|${message}`. Podemos personalizarlo, por ejemplo registrando solamente el mensaje:
...
Este usa `SteamTarget` para enviar un mensaje al steamID del bot principal (`76561198006963719`) cuando el bot nombrado `archi` recibe un intercambio de donación. Se requiere otro bot en el proceso (ya que no puedes enviarte mensajes a ti mismo).
