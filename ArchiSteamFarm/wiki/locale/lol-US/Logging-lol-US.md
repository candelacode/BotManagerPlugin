...
IF U WANTS 2 USE DEFAULT ASF LOGGIN WITHOUT ANY MODIFICASHUNS, U DOAN NED 2 DO ANYTHIN - U ALSO DOAN NED 2 DEFINE IT IN CUSTOM `NLog.config`. 4 REFERENCE THOUGH, EQUIVALENT OV HARDCODD ASF DEFAULT LOGGIN WUD BE:
...
EXAMPLEZ BELOW SHOW U HOW U CAN CUSTOMIZE LOGGIN 2 UR LIKIN.

AS STARTR WE WILL USE **[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)** TARGET ONLY. R INITIAL `NLog.config` WILL LOOK LIEK DIS:
...
TEH EXPLANASHUN OV ABOOV CONFIG IZ RATHR SIMPLE - WE DEFINE WAN **LOGGIN TARGET**, WHICH IZ `ColoredConsole`, DEN WE REDIRECT **ALL LOGGERS** (`*`) OV LEVEL `Debug` AN HIGHR 2 `ColoredConsole`TARGET WE DEFIND EARLIR.
...
SINCE WE DIDNT DEFINE LOT OV PROPERTIEZ, SUCH AS `layout`, IT WUZ INITIALIZD 2 DEFAULT BUILT-IN VALUE, IN DIS CASE `${longdate}|${level:uppercase=true}|${logger}|${message}`. WE CAN CUSTOMIZE IT, 4 EXAMPLE BY LOGGIN MESAGE ONLY:
...
DIS WAN USEZ `SteamTarget` IN ORDR 2 SEND MESAGE 2 TEH MAIN BOTS STEAMID (`76561198006963719`) WHEN BOT NAMD `archi` RECEIVEZ DONASHUN TRADE. REQUIREZ ANOTHR BOT IN DA PROCES (SINCE U CANT SEND MESAGEZ 2 YOURSELF).
