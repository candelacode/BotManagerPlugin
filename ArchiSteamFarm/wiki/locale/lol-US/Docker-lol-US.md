...
ASF IZ AVAILABLE THRU SEVERAL TYPEZ OV **[TAGS](https://hub.docker.com/r/justarchi/archisteamfarm/tags)**:


### `main`

GENERIC BUILD OV ASF DAT IZ BUILT FRUM TEH VRY LATEST COMMIT IN `main` BRANCH, WHICH WERKZ TEH SAME AS GRABBIN LATEST ARTIFACT DIRECTLY FRUM R **[CI](https://github.com/JustArchiNET/ArchiSteamFarm/actions/workflows/publish.yml?query=branch%3Amain)** PIPELINE. IZ TEH HIGHEST LEVEL OV BUGGD SOFTWARE DEDICATD 2 DEVELOPERS AN ADVANCD USERS 4 DEVELOPMENT PURPOSEZ. TEH IMAGE IZ BEAN UPDATD WIF EACH COMMIT IN DA `main` GITHUB BRANCH, THEREFORE U CAN EXPECT VRY OFTEN CHANGEZ (AN STUFF BEAN BROKD). IZ HER 2 MARK CURRENT STATE OV TEH ASF PROJECT, WHICH IZ NOT NECESARILY GUARANTED 2 BE STABLE OR TESTD, JUS LIEK POINTD OUT IN R RELEASE CYCLE. **DIS TAG SHUD NOT BE USD IN ANY PRODUCSHUN ENVIRONMENT**. USEFUL 4 VERIFICASHUN WHETHR PARTICULAR COMMIT FIXD TEH ISSUE URE ENCOUNTERIN, WITHOUT WAITIN EVEN 4 PRE-RELEASE WID DAT COMMIT.


### `released`

GENERIC BUILD OV ASF DAT ALWAYS POINTS 2 TEH LATEST **[RELEASD](https://github.com/JustArchiNET/ArchiSteamFarm/releases)** ASF VERSHUN, INCLUDIN PRE-RELEASEZ. COMPARD 2 `main` TAG, IMAGE HER IZ BEAN UPDATD EACH TIEM NEW GITHUB TAG IZ PUSHD. DEDICATD 2 ADVANCD/POWR USERS DAT LUV 2 LIV ON TEH EDGE OV WUT CAN BE CONSIDERD STABLE AN FRESH AT TEH SAME TIEM. IN PRACTICE, IT WERKZ TEH SAME AS ROLLIN TAG POINTIN 2 TEH MOST RESENT `A.B.C.D` RELEASE AT TEH TIEM OV PULLIN. PLZ NOWT DAT USIN DIS TAG IZ EQUAL 2 USIN R **[PRE-RELEASEZ](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle)**.

### `stable`

GENERIC BUILD OV ASF DAT ALWAYS POINTS 2 TEH LATEST **[STABLE](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** ASF VERSHUN. COMPARD 2 `released` TAG, IMAGE HER IZ BEAN UPDATD ONCE NEW STABLE VERSHUN OV ASF IZ MADE AVAILABLE. RECOMMENDD 4 PEEPS DAT R LOOKIN 4 STABLE VARIANT OV  `released` TAG MENSHUND ABOOV.

### `latest`

OS-SPECIFIC BUILD OV ASF DAT ALWAYS POINTS 2 TEH LATEST **[STABLE](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** ASF VERSHUN. IN COMPARISON WIF OTHERS, DIS AR TEH TEH **ONLY TAG** DAT INCLUDEZ ASF AUTO-UPDATEZ. TEH OBJECTIV OV DIS TAG IZ 2 PROVIDE SANE DEFAULT DOCKR CONTAINR DAT IZ CAPABLE OV RUNNIN SELF-UPDATIN, OS-SPECIFIC BUILD OV ASF. CUZ OV DAT, TEH IMAGE DOESNT HAS 2 BE UPDATD AS OFTEN AS POSIBLE, AS INCLUDD ASF VERSHUN WILL ALWAYS BE CAPABLE OV UPDATIN ITSELF IF NEEDD.

OV COURSE, `UpdatePeriod` CAN BE SAFELY TURND OFF (SET 2 `0`), BUT IN DIS CASE U SHUD PROBABLY USE `stable` RELEASE INSTEAD. LIKEWIZE, U CAN MODIFY DEFAULT `UpdateChannel` IN ORDR 2 TRACK PRE-RELEASEZ INSTEAD IF UD LIEK 2.
...
GENERIC BUILD OV ASF DAT POINTS 2 TEH FIXD ASF VERSHUN MATCHIN TEH TAG. IN COMPARISON WIF ABOOV TAGS, DIS TAG IZ COMPLETELY FROZEN, WHICH MEANZ DAT TEH IMAGE HER WONT BE UPDATD ONCE PUBLISHD. DIS WERKZ SIMILAR 2 R GITHUB RELEASEZ DAT R NEVR TOUCHD AFTR TEH INITIAL RELEASE, WHICH GUARANTEEZ U STABLE AN FROZEN ENVIRONMENT. TYPICALLY U SHUD USE DIS TAG WHEN U WANTS 2 USE SUM SPECIFIC ASF RELEASE AN EXPECT DETERMINISTIC OUTCOME OV TEH BUILD (E.G. MANAGIN ASF VERSHUNS YOURSELF INSTEAD).

---

## WHICH TAG IZ TEH BEST 4 ME?

DAT DEPENDZ ON WUT URE LOOKIN 4. 4 MAJORITY OV USERS, `latest` TAG SHUD BE TEH BEST WAN AS IT OFFERS EGSAKTLY WUT DESKTOP ASF DOEZ, JUS IN SPESHUL DOCKR CONTAINR AS SERVICE. HOWEVR, DIS AR TEH NOT NECESARILY HOW DOCKR SHUD BE USD - NORMALLY URE EXPECTD 2 REBUILD UR CONTAINERS AN NOT RUN THEM FOREVR, AN IN DIS CASE U SHUD CONSIDR `stable` OR `released` TAG, WHICH FOLLOW DOCKR GUIDELINEZ. LASTLY, IF U WANTS 2 RUN SUM FIXD ASF VERSHUN INSTEAD, DEN NATURALLY `A.B.C.D` RELEASEZ R WUT URE LOOKIN 4.
...
THAR R 2 WAYS U CAN CHANGE TEH USR ASF IZ RUNNIN UNDR. TEH FURST WAN, RECOMMENDD, IZ 2 DECLARE `ASF_UID` ENVIRONMENT VARIABLE WIF TARGET UID U WANTS 2 RUN UNDR. SECOND, ALTERNATIV WAN, IZ 2 PAS `--user` **[FLAG](https://docs.docker.com/engine/reference/run/#user)**, WHICH IZ DIRECTLY SUPPORTD BY DOCKR.
...
TEH DIFFERENCE TWEEN `ASF_UID` AN `--user` FLAG IZ SUBTLE, BUT IMPORTANT. `ASF_UID` IZ CUSTOM MECHANISM SUPPORTD BY ASF, IN DIS SCENARIO DOCKR CONTAINR STILL STARTS AS `root`, AN DEN ASF STARTUP SCRIPT STARTS MAIN BINARY UNDR `ASF_UID`. WHEN USIN `--user` FLAG, URE STARTIN WHOLE PROCES, INCLUDIN ASF STARTUP SCRIPT AS GIVEN USR. FURST OPSHUN ALLOWS ASF STARTUP SCRIPT 2 HANDLE PERMISHUNS AN OTHR STUFF AUTOMATICALLY 4 U, RESOLVIN SUM COMMON ISSUEZ DAT U MITEVE CAUSD, 4 EXAMPLE IT ENSUREZ DAT UR `/app` AN `/asf` DIRECTORIEZ R AKSHULLY OWND BY `ASF_UID`. IN SECOND SCENARIO, SINCE WERE NOT RUNNIN AS `root`, WE CANT DO DAT, AN URE EXPECTD 2 HANDLE ALL OV DAT YOURSELF IN ADVANCE.

IF UVE DECIDD 2 USE `--user` FLAG, U NED 2 CHANGE OWNERSHIP OV ALL ASF FILEZ FRUM DEFAULT `1000` 2 UR NEW CUSTOM USR. U CAN DO SO BY EXECUTIN COMMAND BELOW:
...
UNLES U WANTS 2 PROVIDE CUSTOM ENCRYPSHUN KEY OR OTHR ADVANCD OPSHUNS, USUALLY U DOAN NED 2 INCLUDE ANY SPESHUL ENVIRONMENT VARIABLEZ, AS R DOCKR CONTAINERS R ALREADY CONFIGURD 2 RUN WIF SANE EXPECTD DEFAULT OPSHUN OV `--no-restart`, SO DAT FLAG DOEZ NOT NED 2 BE SPECIFID EXPLICITLY IN `ASF_ARGS`.
...
U CAN ACHIEVE TEH SAME EFFECT OV `docker run` COMMAND ABOOV BY USIN TEH FOLLOWIN `docker compose` CONFIG:
...
FRUM THINGS OTHR THAN WE ALREADY DISCUSD ABOOV, WEVE ADDD `--restart unless-stopped` 2 BOTH EXAMPLEZ ABOOV IN ORDR 2 START DIS CONTAINR AUTOMATICALLY AFTR RESTART OV UR MACHINE. FEELZ FREE 2 REMOOV/CHANGE IT 2 SUIT UR NEEDZ.
