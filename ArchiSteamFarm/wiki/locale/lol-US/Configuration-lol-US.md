...
UNLES U WANTS 2 ENABLE EXTRA ASF FUNCSHUNALITIEZ, THAR IZ NO NED 2 OBTAIN/PROVIDE TEH LICENSE.
...
| 14    | MarketableAscending       | TRY 2 FARM GAMEZ WIF UNMARKETABLE CARD DROPS FURST (WARNIN: EXPENSIV 2 CALCULATE) |
| 15    | MarketableDescending      | TRY 2 FARM GAMEZ WIF MARKETABLE CARD DROPS FURST (WARNIN: EXPENSIV 2 CALCULATE)   |
...
`ushort` TYPE WIF DEFAULT VALUE OV `1`. DIS PROPERTY CAN ENABLE SUM ADDISHUNAL ONLINE FEATUREZ ON TEH STEAM PLATFORM, AN IZ DEFIND AS BELOW:

| VALUE | NAYM       | DESCRIPSHUN                  |
| ----- | ---------- | ---------------------------- |
| 1     | StandardPC | NO SPESHUL MODE, DEFAULT     |
| 544   | STEAMDECK  | PRESENT ITSELF AS STEAM DECK |

TEH UNDERLYIN `EGamingDeviceType` TYPE DAT DIS PROPERTY IZ BASD ON INCLUDEZ MOAR AVAILABLE VALUEZ, HOWEVR, 2 TEH BEST OV R KNOWLEDGE THEY HAS ABSOLUTELY NO EFFECT AS OV TODAI, THEREFORE THEY WUZ CUT 4 VISIBILITY.

IF URE NOT SURE HOW 2 SET DIS PROPERTY, LEEF IT WIF DEFAULT VALUE OV `1`.
...
`string` TYPE WIF DEFAULT VALUE OV `null`. ASF WILL USE DIS PROPERTY WHEN LOGGIN IN 2 STEAM NETWORK, WHICH CAN BE USD 4 CUSTOMIZASHUN IN REGARDZ 2 HOW EGSAKTLY STEAM WILL DISPLAY ASF MACHINE AN SESHUN, E.G. WHEN DISPLAYIN DEVICEZ DAT R CURRENTLY LOGGD IN  **[HER](https://store.steampowered.com/account/authorizeddevices)**.

ASF PROVIDEZ FEW SPESHUL VARIABLEZ DAT U CAN OPSHUNALLY USE IN UR TEXT. `{0}` WILL BE REPLACD BY MACHINE NAYM AS PROVIDD BY UR OS, `{1}` WILL BE REPLACD BY ASFS PUBLIC IDENTIFIR, WHILE `{2}` WILL BE REPLACD BY ASFS VERSHUN.

UNLES U KNOE WUT URE DOIN, U SHUD KEEP IT WIF DEFAULT VALUE OV `null`. IN DIS CASE, ASF WILL DECIDE INTERNALLY BOUT TEH PROPR VALUE, WHICH IZ `{0} ({1}/{2})` AS OV TODAI. KEEP IN MIND DAT DIS AR TEH ONLY SUGGESHUN DAT STEAM NETWORK CUD, OR CUD NOT, RESPECT IN FULL.
...
| 2     | InJoinableGame     | CLIENT IZ IN JOINABLE GAME                 |
| 8     | RemotePlayTogether | CLIENT IZ USIN REMOTE PULAY TOGETHR SESHUN |
...
TEH UNDERLYIN `EUIMode` TYPE DAT DIS PROPERTY IZ BASD ON INCLUDEZ MOAR AVAILABLE VALUEZ, HOWEVR, 2 TEH BEST OV R KNOWLEDGE THEY HAS ABSOLUTELY NO EFFECT AS OV TODAI, THEREFORE THEY WUZ CUT 4 VISIBILITY. ALSO, U MITE BE INTERESTD IN CHECKIN `GamingDeviceType`, SINCE SUM ADDISHUNAL FEATUREZ R ENABLD THAR.
...
4 EXAMPLE, GIVEN FOLLOWIN DEFINISHUN:
...
THAR R EGSAKTLY 3 MEANINGFUL, AVAILABLE FLAGS 2 SWITCH ON/OFF (`A`, `B`, `C`), AN THEREFORE `8` POSIBLE VALID COMBINASHUNS OVERALL:

| FINAL VALUE | ENABLD FLAGS    |
| ----------- | --------------- |
| 0           | `NONE`          |
| 1           | `A`             |
| 2           | `B`             |
...
BY DEFINISHUN, IN ORDR 2 MAK TEH ABOOV POSIBLE, EACH STANDALONE FLAG MUST THEREFORE BE TEH POWR OV 2. DIS AR TEH Y ADDISHUNAL FLAG IN ABOOV EXAMPLE, `D`, WUD NED 2 CARRY VALUE OV `8` OR GREATR.

USUALLY, GRAFICAL TOOLS WILL DO TEH CALCULASHUN 4 U. IF URE EDITIN CONFIGS MANUALLY, U CAN ALWAYS USE CALCULATOR AN SIMPLY ADD TOGETHR UNDERLYIN VALUEZ OV ALL TEH FLAGS U WANTS 2 ENABLE - LIEK IN EXAMPLE ABOOV.

EXAMPLE: `"SteamProtocols": 7` (WHICH ENABLEZ FLAGS WIF VALUE OV `1`, `2` AN `4`, AS EXPLAIND ABOOV)
...
U CAN DISABLE DIS BEHAVIOUR WIF `--no-config-watch` **[COMMAND-LINE ARGUMENT](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments#arguments-lol-US)** IF U HAS SPECIFIC REASON, 4 EXAMPLE U DOAN WANTS FRUM ASF 2 REACT 2 FILE CHANGEZ IN `config` FOLDR.