...
> Ми маємо {0} ігор для фермерства.

Але у вашій мові більше сенсу буде мати таке речення:

> Кількість ігор на фермі дорівнює {0}.
...
**Будь ласка, уникайте використання відмітки "issue" якщо вам не потрібне техничне пояснення або дії адміністрації**. Ви вільні використовувати коментарі для обговорення, пов'язані з перекладом даного рядка, але проблема повинна використовуватися лише тоді, коли вам потрібно подальше технічне пояснення або корекція адміністратора, і це зазвичай стосується тих, хто навіть не знає мов, до яких ви перекладаєте, так що, будь ласка, дотримуйтесь англійської під час написання коментарів до задачі (щоб ми могли зрозуміти, що таке проблема).
...
- Поточний переклад невірний - цей тип треба обирати **лише** якщо переклад вже погоджений коректором, але ви вважаєте що він невірний, наприклад має помилку, або у вас є краща пропозиція, як це треба перекласти. Цей тип ніколи не треба використовувати для перекладів, які обрані спільнотою (голосування), бо у цьому випадку ви маєте зв'язатися с користувачем, який запропонував цей переклад, та попрохати його внести виправлення, або просто проголосувати за кращій переклад, як це зазначено у розділі "рецензування". Ми видалимо затвердження перекладу і повідомимо відповідного коректора за мову, щоб прийняти до себе свої коментарі та підтвердити ще раз.
...
- Mistake in the source string (помилка у вихідному рядку) - цей тип потрібно обрати лише якщо ви впевнені що оригінальній (англійський) рядок невірний. Досить рідкісно, але не всі ми говоримо англійською мовою, так що не соромтеся використовувати це, якщо у вас є загальна ідея як його можна вдосконалити. Крім того, це суворо пов'язане з розвитком, ви можете використовувати нашу **[GitHub звернення](https://github.com/JustArchiNET/ArchiSteamFarm/issues/new/choose)** для цієї мети, якщо бажаєте цього.
...
* Виділений рядок для перекладу, посилання включений в `приховані тексти` -> `Link addresses` (рідкісне, де весь рядок це посилання, найбільш поширене в бічній панелі - вони вимагають доступу до коректорів для перекладу, на жаль, )
...
Як бачите, слово `RateLimited` взяте у блок коду і відображує внутрішній статус ASF, який не треба перекладати. Так само, не слід перекладати й інші блоки коду, такі як назви параметрів конфігурації (наприклад: `TradingPreferences`), члени переліку (e. . `Стабільний` та `Пре-реліз` параметрів `Оновлення каналу`і так далі.
...
| Співробітник                                               | Мови                    |
| ---------------------------------------------------------- | ----------------------- |
| **[Astaroth](https://crowdin.com/profile/astaroth2012)**   | ЛОЛКАТА                 |
| **[Dead_Sam](https://crowdin.com/profile/Dead_Sam)**       | Португальська (BR)      |
| **[deluxghost](https://crowdin.com/profile/deluxghost)**   | Китайська (CN)          |
| **[Драгонакі](https://crowdin.com/profile/dragontaki)**    | Китайська (TW)          |
| **[Літтлефрайк](https://crowdin.com/profile/littlefreak)** | Німецька                |
| **[Ryzhehvost](https://crowdin.com/profile/Ryzhehvost)**   | Українська та російська |
| **[MrBurrBurr](https://crowdin.com/profile/MrBurrBurr)**   | ЛОЛКАТ, німецька        |
