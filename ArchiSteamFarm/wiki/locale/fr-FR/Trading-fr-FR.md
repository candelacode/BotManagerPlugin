...
ASF inclut la prise en charge des  échanges non interactifs (hors ligne) de Steam. La réception (acceptation / refus) ainsi que l’envoi de transactions sont disponibles immédiatement et ne nécessitent pas de configuration particulière, mais nécessitent évidemment un compte Steam sans restriction (Dépensé 5 $ dans le magasin). Seules les fonctionnalités de trading limitées sont disponibles pour les comptes restreints.

---

## Logique

Tout d'abord, il est possible de désactiver **toutes les offres** entrantes, en utilisant le drapeau `DisableIncomingTradesParsing` dans `BotBehaviour`. Utiliser ceci, comme le nom l'indique, désactivera toutes les fonctionnalités liées à l'analyse des trades entrants, qui inclut la logique par défaut, ainsi que toutes les fonctionnalités supplémentaires disponibles qui dépendent de la réaction à l'offre de négociation entrante. Puisque les paramètres par défaut sont déjà non-intrusifs, vous ne devriez envisager d’utiliser cette option que si vous n’avez absolument aucune intention d’ASF de faire quoi que ce soit en rapport avec les transactions entrantes.

Ce qui suit explique la logique lorsque les transactions entrantes sont activées, ce qui est l'option par défaut.
