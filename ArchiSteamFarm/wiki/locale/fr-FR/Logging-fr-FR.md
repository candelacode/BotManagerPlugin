...
Par défaut, ASF se connecte à `ColoredConsole` (sortie standard) et à `Fichier`. `La journalisation du fichier` inclut le fichier `log.txt` dans le répertoire du programme, et le répertoire `logs` à des fins d'archivage.

L'utilisation de la configuration personnalisée NLog désactive automatiquement la configuration ASF par défaut, votre configuration remplace **complètement** la journalisation ASF par défaut, ce qui signifie que si vous voulez garder. . Notre cible `ColoredConsole` , alors vous devez la définir **vous-même**. Cela vous permet non seulement d'ajouter des ** extra** cibles d'authentification, mais également de désactiver ou de modifier **les cibles par défaut**.

Si vous souhaitez utiliser l'authentification ASF par défaut sans aucune modification, vous n'avez rien à faire: vous n'avez pas non plus besoin de la définir dans la personnalisation `NLog.config`. Pour référence cependant, l’équivalent de l'authentification ASF codée en dur serait:
...
ASF tente de marquer les messages de manière appropriée en fonction des niveaux de journalisation fournis par NLog, qui vous permet de ne capturer que des messages spécifiques à partir de niveaux de log spécifiques au lieu de tous ceux-ci. Bien sûr, le niveau d'authentification pour un message spécifique ne peut pas être personnalisé, car ASF décide clairement à quel point un message est sérieux, mais vous pouvez certainement rendre ASF moins / plus silencieux, comme bon vous semble.
...
En ce qui concerne la gestion des versions - ASF essaie de toujours utiliser la version la plus récente de NLog disponible sur **[ NuGet](https://www.nuget.org/packages/NLog)** au moment de la publication d'ASF. Il ne devrait pas être problématique d'utiliser une fonctionnalité que vous pouvez trouver sur le wiki NLog dans ASF - assurez-vous simplement que vous utilisez également ASF à jour.
...
Des exemples ci-dessous vous montrent comment vous pouvez personnaliser la journalisation à votre goût.

Pour commencer, nous utiliserons la cible **[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)** uniquement. Notre `NLog.config` initial ressemblera à ceci:
...
L’explication de la configuration ci-dessus est assez simple: nous définissons une **cible de journalisation**, qui est  `ColoredConsole`, puis nous redirigeons **tous les enregistreurs** (`*`) de niveau `Debug`  et supérieur à la `ColoredConsole` définie précédemment.
...
Puisque nous n'avons pas défini beaucoup de propriétés, comme le layout ``, il a été initialisé à une valeur intégrée par défaut, dans ce cas `${longdate}|${level:uppercase=true}|${logger}|${message}`. Nous pouvons le personnaliser, par exemple en enregistrant uniquement les messages :
...
Nous pouvons également modifier la configuration pour vous connecter à plusieurs cibles. Nous allons nous connecter à `ColoredConsole` et **[`Fichier`](https://github.com/nlog/nlog/wiki/File-target)** en même temps.
...
Enfin, ASF utilise différents niveaux de journalisation pour vous aider à comprendre ce qui se passe. Nous pouvons utiliser ces informations pour modifier la sévérité de la journalisation. Disons que nous voulons tout enregistrer (`Trace`) à `Fichier`, mais seulement `Avertissement` et plus **[niveau](https://github.com/NLog/NLog/wiki/Configuration-file#log-levels)** à la `ColoredConsole`. Nous pouvons y parvenir en modifiant nos `règles`:
...
L'exemple ci-dessus générera le fichier `logs/chat/0-0-76561198069026042.txt` lorsque vous parlez avec **[ArchiBot](https://steamcommunity.com/profiles/76561198069026042)**:

```text
2018-07-26 01:38:38 2018-07-26 01:38:38 comment allez-vous? -> 76561198069026042
2018-07-26 01:38:38 Je fais bien, et vous? <- 76561198069026042
...
Celui-ci utilise `SteamTarget` pour envoyer un message au steamID du bot principal (`76561198006963719`) lorsque le bot nommé `archi` reçoit un échange de dons. Nécessite un autre bot dans le processus (puisque vous ne pouvez pas vous envoyer de messages).
...
<p spaces-before="0"><em x-id="4">steamID</em> - SteamID déclaré comme non signé en 64 bits par l'utilisateur Steam cible (comme <code>SteamOwnerID`) ou la cible `chatID</ > (lorsque <code>chatGroupID` est défini). Obligatoire. La valeur par défaut est `0` qui désactive complètement la cible de journalisation.

_botName_ - Nom du bot (comme il est reconnu par ASF, sensible à la casse) qui enverra des messages à `steamID` déclaré ci-dessus. Non requis. La valeur par défaut est `null`, ce qui sélectionne automatiquement **n'importe quel** bot actuellement connecté. Il est recommandé de définir cette valeur correctement, car `SteamTarget` ne prend pas en compte de nombreuses limitations de Steam, telles que le fait que vous devez avoir `steamID` de la cible dans votre liste d'amis. Cette variable est définie comme le type [layout](https://github.com/NLog/NLog/wiki/Layouts) , vous pouvez donc utiliser une syntaxe spéciale, comme `${logger}` afin d'utiliser le bot qui a généré le message.
