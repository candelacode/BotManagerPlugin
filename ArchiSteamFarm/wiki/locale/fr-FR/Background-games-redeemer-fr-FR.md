...
Si notre compte atteint le statut `RateLimited` durant le processus, la file d'attente est suspendue temporairement pendant une heure entière, jusqu'à la fin du délai d'attente. Ensuite, le processus se poursuit là où il est laissé, jusqu'à ce que la file d'attente soit vide ou qu'un autre `RateLimited` se produit.
