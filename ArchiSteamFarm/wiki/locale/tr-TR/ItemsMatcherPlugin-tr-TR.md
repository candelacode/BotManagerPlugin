...
Since the moment you stop using (announcing on) our listing, your data becomes unavailable to general public within a maximum of 15 minutes, and is otherwise kept on our server for a maximum of two weeks - everything is automatically deleted after that period. No action is required from you for that to happen.
...
`MatchActively`, tamamen yeniden tasarlanmış bir konsepte dayanarak, ASF'yi aktif kart eşleştirme işlevselliğiyle daha da geliştiren resmi `ItemsMatcher` eklentisinin bir parçası olarak yeniden hayata geçirildi. Resurrecting `MatchActively` feature required from us **extraordinary amount of work** to create ASF backend, entirely new service hosted on a server, with more than a hundred of paid proxies attached for resolving inventories, all exclusively to allow ASF clients to make use of `MatchActively` like before. Due to the amount of work involved, as well as resources that are not free and require to be paid on monthly basis by us (domain, server, proxies), we've decided to offer this functionality to our sponsors, that is, people that already support ASF project on monthly basis, thanks to whom we can make those paid resources available.
...
Her durumda, `MatchActively`'nin herkes için uygun olmadığını anlıyoruz ve neden ücretsiz olarak sunamadığımızı da anlayacağınızı umuyoruz. Eğer kimse bu özelliğin maliyetlerini karşılamaya yardımcı olmakla ilgilenmeseydi, zaten en başından beri var olmazdı, çünkü kimsenin sürdürmek istemediği aylık giderlerden kısmak zorunda kalırdık. Thankfully, we're in better position than that, and you can decide yourself if you're willing to use `MatchActively` on those terms, or not.
