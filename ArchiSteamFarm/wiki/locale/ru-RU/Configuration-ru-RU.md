...
Включение этого режима позволит вам обеспечить требуемый вход консоли другими способами, т.е. ASF-ui (ASF API) или через команду **[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands#input-command)**. Если вы не уверены как настроить этот параметр - оставьте ему значение по-умолчанию `false`.

---

### `IdleFarmingPeriod`

Параметр типа `byte` со значением по-умолчанию `8`. Когда ASF нечего фармить, он будет проверять каждые `IdleFarmingPeriod` часов, не появилось ли на аккаунте новых игр для фарма. Эта функция не нужна когда мы говорим о добавлении новых игр на аккаунт, поскольку алгоритм ASF достаточно умен чтобы автоматически проверять в этом случае страницу со значками. `IdleFarmingPeriod` в основном для тех случаев, когда в старую игру, которае есть у нас на аккаунте, добавили коллекционные карточки. В этом случае нет никакого события, поэтому ASF приходится периодически проверять страницу со значками чтобы предусмотреть такой вариант. Значение `0` выключает эту функцию. Также проверьте: `ShutdownOnFarmingFinished` предпочтение в `FarmingPreferences`.

---

### `InventoryLimiterDelay`

Параметр типа `byte` со значением по-умолчанию `4`. ASF обеспечит наличие по крайней мере `инвентаризации LimiterDelay` секунд в промежутках между двумя запросами последовательного веб-инвентаризации во избежание ограничения скорости - те, к примеру, используются в процессе маркировки уведомлений о инвентарных запасах как прочитанные, также могут быть использованы сторонними плагинами для получения инвентаря других пользователей. Это свойство не используется для получения нашего собственного инвентаря, так как ASF использует гораздо более эффективный внутренний сетевой вызов для этого, так что это не повлияет на такие команды, как `loot` или `transfer`. Значение по умолчанию `4` было установлено на основе маркировки более 100 экземпляров последовательных ботов, и должны удовлетворять большинство (если не все) пользователей. Однако вы можете уменьшить это значение или даже изменить его на `0`, если у вас очень мало ботов, тогда ASF будет игнорировать задержку и отмечать инвентарь Steam гораздо быстрее. Однако будьте внимательны: слишком низкое значение **приведет** к тому, что Steam временно заблокирует ваш IP, и вы не сможете совершать звонки вообще. Вам может также понадобиться увеличить это значение если у вас много ботов который производят много запросов к инвентарю, хотя возможно в этом случае вам стоит вместо этого уменьшить количество запросов. Если у вас нет **веских** причин редактировать этот параметр, рекомендуется оставить ему значение по умолчанию.
...
Если вы не хотите включить дополнительные функциональные возможности ASF, то не требуется получения/предоставления лицензии.
...
Параметр типа `ImmutableHashSet<string>` с пустым значением по-умолчанию. Это свойство определяет список названий сборок плагинов, которые в черном списке или в белом списке считаются для автоматических обновлений, согласно `PluginsUpdateMode` определен ниже.
...
Параметр типа `byte` со значением по-умолчанию `0`. Это свойство определяет режим обновления плагинов, который дает значение `PluginsUpdateList` описанному выше. Указав это свойство, вы можете легко включить/отключить автоматические обновления для всех плагинов, кроме объявленных.

- Значение `0`, называемое `белый список`, **отключает** автоматическое обновление всех плагинов, кроме тех, которые определены в `PluginsUpdateList`.
- Значение `1`, называется `Blacklist`, **включает** автоматическое обновление всех плагинов, кроме тех, которые определены в `PluginsUpdateList`.

**команда ASF хотела бы напомнить вам, что для вашей собственной безопасности вы должны включить автоматические обновления только от доверенных сторонников**. Имейте в виду, что вредоносные плагины могут решить обновить себя или выполнить удаленные команды **независимо от** этой настройки, именно поэтому эта настройка применима к функциональности обновления плагинов, предоставленных ASF, исключительно, и вы должны убедиться, что вы правильно проверили каждый плагин, который вы решили использовать.

Обновления плагинов выполняются по умолчанию вместе с процедурой обновления ASF - **[`UpdateChannel`](#updatechannel)** и **[`UpdatePeriod`](#updateperiod)**. Стандартные механизмы обновления ASF, такие как команда `обновления` также вызовут обновление дополнительных плагинов. Если вместо этого вы хотите обновить плагины вручную без обновления версии ASF одновременно, `updateplugins` команда предоставляет такую возможность.

Если у вас нет причин редактировать этот параметр, рекомендуется оставить ему значение по-умолчанию.

---

### `Возможное завершение работы`

Параметр типа `bool` со значением по-умолчанию `false`. Если включено, ASF попытается завершить процесс по возможности - когда все ваши зарегистрированные боты будут остановлены. Это может быть особенно полезно в сочетании с `ShutdownOnFarmingFinished` для всех ваших экземпляров ботов, поскольку, таким образом ASF будет автоматически отключаться, когда последний из ваших ботов закончит заниматься сельским хозяйством.

Поскольку ожидание большинства пользователей заключается в том, что процесс будет запущен в любое время. . для использования `IPC` эта опция по умолчанию отключена.
...
По-умолчанию ASF будет использовать все доступные протоколы в качестве меры борьбы с простоями и другими проблемами Steam. Обычно это свойство необходимо изменять, если вы хотите ограничить использование ASF только одним или двумя конкретными протоколами. Такие меры могут потребоваться в различных ситуациях, Например, если вы заблокировали UDP трафик на вашем брандмауэре и вы хотите убедиться, что только TCP трафик прошел, или если вы используете `WebProxy` и хотите использовать его также для соединения Steam, как только `WebSocket` протокол поддерживает это.
...
Параметр типа `byte` со значением по-умолчанию `1`. Этот параметр задаёт используемый канал обновлений, который будет использоваться либо для авто-обновлений (если `UpdatePeriod` больше `0`), либо для оповещения о доступных обновлениях (в противном случае). В настоящее время ASF поддерживает три канала обновлений - `0` - `None`, `1`, , который называется `Стабильный`, и `2`, который называется `PreRelease`. Канал `Stable` это канал обновлений по-умолчанию, который должен использоваться большинством пользователей. Канал `PreRelease`, помимо `Стабильных` релизов, также включает **Ранние сборки**, предназначенные для опытных пользователей и других разработчиков, чтобы протестировать новые возможности, подтвердить исправления или оставить отзывы о планируемых улучшениях. **Версия Ранних сборок часто содержит неисправленные ошибки, функции рабочего процесса или переписанные реализации**. Если вы не считаете себя продвинутым пользователем, то оставайтесь на канале обновления по умолчанию `1` (`Stable`). Канал `PreRelease` посвящен пользователям, которые знают, как сообщать об ошибках, решение проблем и обратная связь - техническая поддержка не предоставляется. Ознакомьтесь с **[циклом выпуска](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-ru-RU)** ASF если хотите узнать больше. Вы также можете установить параметру `UpdateChannel` значение `0` (`None`) если вы хотите полностью отключить проверку наличия новых версий. Установка параметра `UpdateChannel` в `0` полностью отключит все функции связанные с обновлениями, в том числе командой `update`. Использование канала `None` **крайне не рекомендуется**, поскольку в этом случае вы подвергнете себя разнообразным проблемам (упомянутым в описании параметра `UpdatePeriod` ниже).
...
Параметр типа `string` со значением по-умолчанию `null`. Это свойство определяет адрес прокси-сервера, который будет использоваться для внутренней связи с https, особенно для таких сервисов, как `github. om`, `api.steampowered.com`, `steamcommunity.com` и `store.steampowered.com`. Он применяется к общей (спецификации неботов) коммуникации, а также коммуникации, если не задано свойство конфигурации `WebProxy`. Проксирование ASF запросов может оказаться исключительно полезным для обхода различных брандмауэров, особенно большого брандмауэра в Китае.
...
Если вам требуется проксирование и внутренних сетевых коммуникаций Steam (CMs), то вам следует настроить свойство **[`SteamProtocols`](#steamprotocols)** бота на значение, разрешающее только websocket-транспорт, то есть на значение `4`, поскольку только websockets поддерживаются для проксирования.
...
```json
{
    ...
    "Игровой Идл": [],
    "Игровой Тип": 1,
    "Часы-UntilCardDrops": 3,
    "LootableTypes": [1, 3, 5],
    "Имя машины": null,
    "Соответствующие типы": [5],
    "Флаги онлайн": 0,
    "Онлайн-Статус": 1,
    "Формат пароля": 0,
    "Настройки обмена": 0,
...
| Имя | Имя                              | Описание                                                                                                                        |
| --- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 0   | None                             | Нет особого поведения бота, разумные настройки по умолчанию                                                                     |
...
| 64  | Отключение IncomingTradesParsing | ASF никогда не будет обрабатывать входящие предложения                                                                          |
...
В общем, вы хотите изменить это свойство, если вы планируете изменить различные параметры автоматизации, связанные с деятельностью бота. Настройки по умолчанию включают в себя работу ASF в неинвазивном режиме, что позволяет только выгодную автоматизацию, которая не совпадает с желанием пользователя.
...
`DisableIncomingTradesParsing` не позволит ASF анализировать входящие торговые предложения, это означает, что вся торговая функциональность, связанная с этим будет неоперативной. Поскольку ASF по умолчанию работает в наименее инвазивном режиме, принятие торговых предложений только от пользователей `Master` и выше, никогда не трогать другие торговые предложения - при парсинге входящих сделок по умолчанию включен парсинг. Эта настройка имеет больше смысла для людей, которые хотели бы обеспечить отсутствие дополнительных запросов/накладных расходов, связанных с разбором сделок, отключением всей логики в процессе, например, потому что они знают, что их боты никогда не получат мастер-торговые запросы, и поэтому вообще не требуют участия ASF в их торговой деятельности. Имейте в виду, что включение этой опции отключит все остальные опции, которые также зависят от входящих сделок, Например, `Принять пожертвования` или `SteamTradeMatcher` - пользовательские плагины также не смогут обрабатывать входящие предложения обычным образом.
...
Из-за дополнительных накладных расходов на использование этой опции, рекомендуется использовать его только для аккаунтов ботов, у которых есть реальная возможность отделить сеты от себя - например, активировать, если вы уже используете `SendOnFarmingFinished` предпочтение в `FarmingPreferences`, `команды SendTradePeriod` или `loot` обычным образом.
...
| 14       | MarketableAscending       | Попробуйте сначала фермерские игры с немаркетологической картой (предупреждение: рассчитать дорогую) |
| 15       | MarketableDescending      | Попробуйте сначала фермерские игры с маркетологической картой (предупреждение: вычислить дорогу)     |
...
Параметр типа `byte flags` со значением по-умолчанию `0`. Это свойство определяет поведение ASF, связанное с сельским хозяйством, и оно определяется следующим образом:

| Значение | Имя                                     |
| -------- | --------------------------------------- |
| 0        | None                                    |
| 1        | Сельскохозяйственная пауза по умолчанию |
| 2        | ShutdownOnFarmingFinished               |
| 4        | SendOnFarmingFinished                   |
| 8        | FarmPriorityQueueOnly                   |
| 16       | SkipRefundableGames                     |
| 32       | Пропущенные Невоспроизводимые Игры      |
| 64       | EnableRiskyCardsDiscovery               |
| 256      | AutoUnpackBoosterPacks                  |

Обратите внимание что это параметр типа `flags`, и следовательно ему может быть присвоена любая комбинация приведенных выше значений. Ознакомьтесь с **[json mapping](#json-mapping)**, если хотите узнать больше. Если вы не включите никаких флагов, будет использоваться вариант `None`.

Все варианты описаны ниже.

`FarmingPausedByDefault` определяет начальное состояние модуля `CardsFarmer`. Как правило, бот автоматически начнет фермерство, когда он будет запущен, либо из-за команды `Включено` или `запустить`. Используя `FarmingPausedByDefault` можно использовать, если вы хотите вручную `возобновить` процесс автоматического земледелия, Например, потому что вы хотите использовать `играть` все время и не использовать автоматический модуль `CardsFarmer` - это работает точно так же, как `пауза` **[команда](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**.

`ShutdownOnFarmingFinished` позволяет выключить бота после того, как он закончит фермерство. ASF обычно является "занятием" счетом на все время активного процесса. Когда на аккаунте закончен фарм карточек, ASF периодически проверяет (каждые `IdleFarmingPeriod` часов), не появились ли новые игры с карточками, чтобы продолжить фарм без необходимости перезапуска процесса. Это полезно для большинства пользователей, поскольку ASF продолжает фармить по мере необходимости. Однако вы можете захотеть остановить этот процесс в том случае, когда данный аккаунт полностью фермирован, вы можете достичь этого с помощью этого флага. Если этот параметр включен, ASF будет выходить из аккаунта когда фарм на нём закончен, а значит не сможет периодически проверять или занимать его. Вам нужно решить, хотите ли вы чтобы ASF работал с данным аккаунтом всё время, или ASF должен останавливать его, когда закончин фарм.

Этот параметр имеет больше смысла сочетаться с `ShutdownIfpossible`, так что когда все учетные записи будут остановлены, ASF также будет выключен. Разместите машину на отдых, и вы сможете запланировать другие действия, такие как сн или выключение в тот же момент сброса последней карты.

`SendOnFarmingFinished` позволяет автоматически отправлять торговлю паром, содержащую все, что было вызвано этой точкой пользователю, имеющему разрешение `Master` , Это очень удобно, если вы не хотите беспокоиться о сделках самостоятельно. Эта функция аналогична команде `loot`, поэтому помните что для неё нужно чтобы был установлен пользователь с правами `Master`, а также может понадобиться правильный `SteamTradeToken`, ну и конечно аккаунту должно быть разрешено совершать обмены. В дополнение к запуску `loot` после окончания фарма, если эта функция активна, ASF будет также вызывать `loot` каждый раз при получении уведомления о получении новых предметов (только когда фарм не запущен), и после завершения каждого обмена при котором получены новые предметы(всегда). Это особенно удобно для передачи "по цепочке" предметов полученных от других людей на основной аккаунт. Обычно вам нужно использовать **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-ru-RU)** вместе с этой функцией, хотя это не обязательно, если вы планируете своевременно обрабатывать подтверждения 2FA вручную.

`FarmPriorityQueueOnly` определяет, следует ли ASF рассматривать для автоматического земледелия только те приложения, которые вы добавили в приоритетную очередь фермерского хозяйства, доступную с командами `fq` **[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**. Если этот параметр включен, ASF будет пропускать все `appID` отсутствующие в списке, позволяя вам выбрать игры для автоматического фарма. Помните, что если вы не добавили ни одной игры в очередь, ASF будет нечего фармить на вашем аккаунте.

`SkipRefundableGames` определяет, следует ли ASF пропустить игры, которые все еще возвращаются от автоматического земледелия. Возврат средств разрешён за игры, которые были куплены не более чем 2 недели назад через магазин Steam, и в которые наиграно менее 2 часов, это описано на странице **["Возврат средств в Steam"](https://store.steampowered.com/steam_refunds)**. По умолчанию ASF игнорирует политику Steam полностью и полностью возмещает все убытки, как ожидало бы большинство людей. Тем не менее, вы можете использовать этот флаг, если хотите убедиться, что ASF не будет обрабатывать ваши возвратные игры слишком скоро, позволяет вам самостоятельно оценить эти игры и возвращать деньги в случае необходимости, не беспокоясь о том, что ASF отрицательно влияет на игровое время. Пожалуйста, обратите внимание, что если вы включите этот параметр, игры купленные в магазине Steam не будут фармиться ASF в течение 14 дней с даты покупки, и это будет отображаться "На этом аккаунте нечего фармить!" если других игр для фарма на аккаунте нет.

`SkipUnplayedGames` определяет, следует ли ASF пропускать игры, которые вы еще не запустили. Несыгранная игра в этом контексте означает, что у вас есть точно не зарегистрировано время воспроизведения в Steam. Если вы используете этот флаг, то такие игры будут пропущены до тех пор, пока Steam не зарегистрирует любое время для них. Это позволяет вам лучше контролировать какие игры ASF имеют право на ферму, пропускать те, что у вас еще не было шансов опробовать, сохранение дополнительных возможностей в выбранном Steam - например, предложение несыгранных игр для игр.

`EnableRiskyCardsDiscovery` включает дополнительный откат, который срабатывает, когда ASF не может загрузить одну или несколько страниц значков и, следовательно, не может найти игры, доступные для фарминга. В частности, на некоторых аккаунтах с большим количеством сбросов карт может возникнуть ситуация, когда загрузка страниц со значками становится невозможной (из-за накладных расходов), и на этих аккаунтах становится невозможным фарм просто потому, что мы не можем загрузить информацию, на основе которой мы можем начать процесс. Для этих немногих случаев эта опция позволяет использовать альтернативный алгоритм, которая использует комбинацию возможных ускорителей для создания и увеличения наборов счета, чтобы найти потенциально доступные игры для простоя, затем тратить чрезмерное количество ресурсов на проверку и получение необходимой информации, и попытки начать процесс земледелия на ограниченном объеме данных и информации, чтобы в конечном итоге достичь ситуации, когда страница значков загружается, и мы сможем использовать обычный подход. Обратите внимание, что при использовании этого резервного варианта ASF работает только с ограниченными данными, поэтому для ASF совершенно нормально находить гораздо меньше дропа, чем на самом деле — другой дроп будет найден на более позднем этапе процесса фарма.
...
`AutoUnpackBoosterPacks` автоматически распаковывает все бустеры при получении уведомления о новых предметах. Это позволит вам сразу же получить дополнительные новые карты, что может быть востребовано сценарием, особенно в сочетании с другими опциями (например, `SteamTradeMatcher` или `CompleteTypesToSend`).
...
### `Тип игрового устройства`

Тип `шорта` по умолчанию `1`. Это свойство может включать некоторые дополнительные онлайн функции на платформе Steam, и определяется следующим образом:

| Значение | Имя            | Описание                              |
| -------- | -------------- | ------------------------------------- |
| 1        | Стандартный ПК | Нет специального режима, по умолчанию |
| 544      | Палуба         | Представьте себя как Паровая Палуба   |

Базовый тип `EGamingDeviceType` , который основан на этом свойстве, включает в себя больше доступных значений, Однако, как нам известно, они не имеют никаких последствий на сегодняшний день, поэтому они были сокращены для видимости.

Если вы не уверены как настроить этот параметр - оставьте ему значение по-умолчанию `​1​`.
...
Параметр типа `string` со значением по-умолчанию `null`. ASF будет использовать это свойство при входе в сеть Steam, , который может быть использован для настройки относительно того, как именно Steam будет отображать ASF машину и сессию, а также для настройки. . при отображении устройств, которые в настоящее время вошли в систему **[здесь](https://store.steampowered.com/account/authorizeddevices)**.

ASF предоставляет несколько особых переменных, которые вы можете использовать в своём тексте. `{0}` будет заменено именем машины, предоставленным вашей ОС, `{1}` будет заменено публичным идентификатором ASF, а `{2}` будет заменено версией ASF.

Если вы не знаете, что вы делаете, то сохраните его со значением по умолчанию `null`. В этом случае ASF определит внутреннее значение `{0} ({1}/{2})` на сегодня. Имейте в виду, что это только предложение, которое сеть Steam может или не может соблюдаться в полном объеме.
...
| 2        | InJoinableGame      | Клиент в совместной игре                                      |
| 8        | Удалите игру вместе | Клиент использует сеанс дистанционного воспроизведения вместе |
...
Если вы решили сменить `парольФормат` бота, который уже вошел в сеть Steam, по крайней мере один раз, Возможно вы получите одноразовую ошибку расшифровки при следующем запуске бота - это вызвано тем, что `PasswordFormat` также используется для автоматического шифрования/дешифрования чувствительных свойств в `Bot. b` файл базы данных. Вы можете спокойно игнорировать эту ошибку, так как ASF сможет восстановить эту ситуацию самостоятельно. Если это происходит постоянно, например, каждый перезапуск, его следует изучить.
...
Параметр типа `byte` со значением по-умолчанию `0`. Это свойство работает очень похоже на `SendOnFarmingFinished` предпочтение в `FarmingPreferences`, с одним отличием - вместо отправки сделки в процессе фермерского хозяйства, мы также можем отправлять его каждые `SendTradePeriod` часов, независимо от того, сколько осталось от фермы. Это полезно, если вы хотите получать предметы с дополнительных аккаунтов на регулярной основе, не дожидаясь окончания фарма. Значение по-умолчанию `​0​` отключает эту функцию, если вы хотите чтобы ваш бот отсылал вам предложения обмена, например, каждый день - поставьте сюда значение `24`.
...
Мы рекомендуем указать только одного пользователя как `Master`, и сколько захотите - как `Operator` и ниже. Хотя технически возможно установить несколько `Мастеров` и ASF будет корректно работать с ними, например, принимая все их сделки, отправленные боту, ASF будет использовать только одного из них (с наименьшим паровым идентификатором) для каждого действия, требующего одной цели, например, запроса `loot`, так же как и свойства типа `SendOnFarmingFinished` в `FarmingPreferences` или `SendTradePeriod`. Если вы хорошо понимаете эти ограничения, особенно тот факт что запрос `loot` всегда будет отсылать предметы аккаунту `Master` с самым маленьким steam ID, независимо от того, кто из `Master`ов отдал эту команду - вы можете задать здесь несколько пользователей с правами `Master`, но мы всё же рекомендуем схему с одним `Master`.
...
Параметр типа `byte` со значением по-умолчанию `0`. Это свойство определяет режим пользовательского интерфейса, который будет объявлен ботом после входа в сеть Steam. Это может повлиять на видимость учетной записи, например в чате Steam, если ваше присутствие позволяет через `OnlineStatus`. В настоящее время вы можете выбрать один из режимов ниже:

| Имя | Имя        | Описание                         |
| --- | ---------- | -------------------------------- |
| `0` | VGUI       | Режим Steam клиента по умолчанию |
| `1` | Tenfoot    | Большой режим изображения        |
| `2` | Мобильный  | Мобильное приложение Steam       |
| `3` | Веб        | Сессия веб-браузера              |
| `5` | MobileChat | Мобильный чат Steam              |

Базовый тип `EUIMode` , который базируется на этом объекте, включает в себя больше доступных значений. к лучшему из наших знаний, они не имеют никакого эффекта на сегодняшний день, поэтому они были сокращены для видимости. Кроме того, вы можете быть заинтересованы в проверке `GamingDeviceType`, так как здесь включены дополнительные функции.
...
Параметр типа `string` со значением по-умолчанию `null`. Это свойство определяет адрес веб-прокси который будет использоваться для специфичных для бота внутренних http-соединений, особенно для таких служб, как `api. teampowered.com`, `steamcommunity.com` и `store.steampowered.com`. Если не установлено, ASF вместо этого будет использовать глобальные `WebProxy`. Проксирование ASF запросов может оказаться исключительно полезным для обхода различных брандмауэров, особенно большого брандмауэра в Китае.
...
`BotName.keys` - это специальный файл, который можно использовать для импорта ключей в **[background games redeemer](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Background-games-redeemer)**. Это не обязательный файл, и не создаётся, а только распознаётся ASF. Этот файл автоматически удаляется после того как ключи успешно импортированы.

`BotName.maFile` является специальным файлом, который может быть использован для импорта **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication)**. Это не обязательный файл, и не создаётся, а только распознаётся ASF если ваш бот `BotName` ещё не имеет подключенного 2ФА ASF. Этот файл автоматически удаляется после того как 2ФА ASF успешно импортирован.
...
`ImmutableList<valueType>` - Неизменяемая коллекция значений в данном `valueType`. В JSON задаётся как массив элементов заданного типа `valueType`. ASF использует тип `List` чтобы показать что этот параметр поддерживает указание нескольких значений, и что их порядок может иметь значение.

Пример для `ImmutableList<byte>`: `"FarmingOrders": [15, 11, 7]`

---

`ImmutableHashSet<valueType>` - Неизменяемая коллекция (установлен) уникальных значений в данном `valueType`. В JSON задаётся как массив элементов заданного типа `valueType`. ASF использует `HashSet` чтобы показать что данный параметр имеет смысл только для уникальных значений, поэтому он будет намеренно игнорировать любые возможные дубликаты во время разбора файла (если вы их туда случайно добавите).

Пример для `ImmutableHashSet<uint>`: `"Blacklist": [267420, 303700, 335590]`

---

`ImmutableDictionary<keyType, valueType>` - Неизменяемый словарь (карта), который сопоставляет уникальный ключ, указанный в его `keyType`, со значением, указанным в его `valueType`. В JSON задаётся как объект с парами ключ-значение. Помните, что в этом случае `keyType` всегда берётся в кавычки, даже если это значение таких типов как `ulong`. Также здесь имеется строгое требование чтобы ключ был уникальным в ассоциативном массиве, на этот раз обусловленное также форматом JSON.
...
Например, указанное следующее определение:
...
Есть ровно 3 значимые, доступные флаги для включения/выключения (`A`, `B`, `C`), и поэтому `8` возможные допустимые комбинации в целом:

| Окончательное значение | Включены флаги  |
| ---------------------- | --------------- |
| 0                      | `None`          |
| 1                      | `A`             |
| 2                      | `B`             |
...
По определению, для того чтобы сделать вышеуказанное возможным, каждый отдельный флаг должен иметь силу двух. Вот почему дополнительный флаг выше примера `D`должен иметь значение `8` или выше.

Обычно графические инструменты будут подсчитаны для вас. Если вы редактируете конфигурацию вручную, вы всегда можете использовать калькулятор и просто добавить лежащие в его основе значения всех флагов, которые вы хотите включить, например выше.

Пример: `"SteamProtocols": 7` (который разрешает флаги со значением `1`, `2` и `4`, как разъясняется выше)
...
В связи с этим ASF автоматически мигрирует/оптимизирует ваши конфигурации, переформатируя их и удаляя поля, содержащие значения по умолчанию. Вы можете отключить это поведение с помощью командной строки `--no-config-migrate` **[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments#arguments)** если у вас есть конкретная причина, Например, вы предоставляете конфигурационные файлы только для чтения, и вы не хотите, чтобы ASF модифицировал их.
...
Вы можете отключить это поведение с помощью командной строки `--no-config-watch` **[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments#arguments)** если у вас есть конкретная причина, Например, вы не хотите, чтобы ASF реагировал на изменения файлов в папке `конфигурации`.