...
`--no-restart` - по умолчанию ASF следует **[`Автоперезапуск`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#autorestart)** настройки, , который вы можете использовать для уточнения того, разрешена ли перезагрузка при необходимости. Некоторые решения, которые мы предоставляем для управления процессами и явно несовместимы с функцией автоперезапуска ASF, например запуск ASF в `docker` или `system`, потому что они требуют только выхода из системы, так как они обязаны возобновить ее после этого, если они сочтут это необходимым. Поскольку редактирование произвольной конфигурации нежелательно из пользовательского опыта, этот переключатель просто переопределяет свойство конфигурации `AutoRestart` путем явной инициализации его в `false`, даже если вы указали иначе в файле config. Благодаря этому ASF может быть заблаговременно проинформирован о работе в такой среде. без требования предоставить совместимый файл конфигурации `AutoRestart: false`

Помимо вышесказанного, `--no-restart`, вопреки `AutoRestart: false`, также запретит вам использовать команду `перезапуска` или иным образом перезапустить ASF процесс, чтобы перезапустить, так как переключатель явно указывает на то, что он не совместим с такой установкой, в то время как свойство `Автоперезапуск` только определяет поведение по умолчанию.

Обычно вы можете (и должны) контролировать поведение, описанное в конфигурационном файле, хотя если вы запускаете ASF внутри пользовательского скрипта или другого аналогичного окружения, вы также можете использовать этот переключатель, который запретит ASF перезапускать сам себя.
...
`--system-required` - указание этого аргумента заставит ASF попытаться сообщить ОС что этому процессу требуется чтобы система была запущена в течение всей его работы. Полезность этого может быть доказано фармом на компьютере или ноутбуке ночью. ASF сможет не давать системе перейти в режим сна во время фарма. В настоящее время эта функция поддерживается на Linux и Windows.

В Linux вам понадобится корректная работа **[dbus](https://en.wikipedia.org/wiki/D-Bus)** с демоном входа, поддерживающим **[`Inhibit()`](https://systemd.io/INHIBITOR_LOCKS/)** функции. Два самых популярных демона - `systemd` и `elogind`- подтвердили свою поддержку. Большинство окружений рабочего стола (таких как Gnome или KDE) должны работать вне коробки, так как они уже зависят от этой функциональности для своих собственных нужд.

Никаких особых требований к Windows, он должен быть выполнен из коробки.