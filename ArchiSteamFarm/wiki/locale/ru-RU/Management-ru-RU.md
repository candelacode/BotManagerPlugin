...
```sh
### Редактирование /etc/systemd/ArchiSteamFarm@asf.service.d/override. onf
### Все между этим и комментарием ниже станет новым содержимым файла
...
# Перезапустить =on-success
# Перезапустить Sec=1s
...
Конечно, альтернативой является `cp` файл и управление им самостоятельно, но это позволит вам гибко изменять его, даже если вы решили сохранить оригинальный ASF-блок, например, с помощью **[symlink](https://wikipedia.org/wiki/Symbolic_link)**.

---

## Никогда не запускайте ASF от имени администратора!

ASF включает в себя собственную проверку того, запускается ли процесс с правами администратора (`root`) или нет. Запуск от имени `root` **не** требуется для любого вида операций, выполняемых процессом ASF, при условии правильно настроенного окружения, в котором он работает, и поэтому должен рассматриваться как **плохая практика**. Это означает, что в Windows ASF **никогда** не должна выполняться с настройками "запускать от имени администратора", а в Unix ASF должна иметь **выделенную учетную запись пользователя** для себя, или использовать свою собственную в случае настольной системы.

Для дальнейшей проработки *почему* не поощряем ASF как `root`, ссылается на **[суперпользователь](https://superuser.com/questions/218379/why-is-it-bad-to-run-as-root)** и другие ресурсы. Если вы все еще не убедились, спросите себя, что будет с вашей машиной, если ASF процесс выполнит команду `rm -rf /*` сразу после запуска (комментарий переводчика: трындец вашей операционке!).
...
ASF не останавливает вас делать это, только показывает предупреждение с коротким уведомлением. Просто не удивляйтесь, если в один день из-за ошибки в программе он взорвет всю вашу ОС с полной потерей данных - вы были предупреждены. Развлекайтесь XD
