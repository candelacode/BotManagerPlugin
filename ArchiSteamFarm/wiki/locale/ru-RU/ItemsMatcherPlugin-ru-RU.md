...
Для того чтобы попасть в список, вы должны удовлетворять набору требований. Как минимум, у вас должно быть разрешено `PublicListing` в `RemoteCommunication` (настройка по умолчанию), `SteamTradeMatcher` включен в `TradingPreferences`, **[public inventory](https://steamcommunity.com/my/edit/settings)** настройки конфиденциальности, **[неограниченный](https://help.steampowered.com/faqs/view/71D3-35C2-AD96-AA3A)** аккаунт и **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication#asf-2fa)** активен. Дополнительными требованиями являются наличие 2ФА, активного не менее 15 дней назад, последняя смена пароля более 5 дней назад, отсутствие каких-либо ограничений на аккаунте, таких как блокировки, экономического запрета и запрета на торговлю. Естественно, у вас должен быть по крайней мере один (торговый) предмет в вашем инвентаре из указанного `подходящих типов`, таких как торговые карты. Кроме того, боты с количеством предметов более `500000` не принимаются из-за чрезмерных накладных расходов. В этом случае мы рекомендуем разделить ваш инвентарь между несколькими аккаунтами.
...
Мы показываем все учетные записи ASF, которые были активны за **последние 15 минут**. Пользователи сортируются по их относительной полезности - `MatchEverything` ботов, которые показаны с `любым` значком, принимающим все 1:1 сделки, затем по количеству уникальных игр, и, наконец, по общему количеству предметов.
...
С момента прекращения использования (анонса) нашего списка, Ваши данные становятся недоступными для широкой общественности в течение не более 15 минут, и в противном случае сохраняется на нашем сервере максимум две недели - все автоматически удаляется после этого периода. Для этого вам не требуется никаких действий.
...
Чтобы использовать данный режим, вам необходимо соответствовать ряду требований. Как минимум, у вас должен быть аккаунт **[без ограничений](https://help.steampowered.com/faqs/view/71D3-35C2-AD96-AA3A)** **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication#asf-2fa)** активный и по крайней мере один допустимый тип в `MatchableTypes`, например, торговые карты. Дополнительными требованиями являются наличие 2ФА, активного не менее 15 дней назад, последняя смена пароля более 5 дней назад, отсутствие каких-либо ограничений на аккаунте, таких как блокировки, экономического запрета и запрета на торговлю.
...
В любом случае, мы понимаем, что `MatchActively` не для всех, и мы надеемся, что вы также понимаете, почему мы не можем предложить это бесплатно. Если никто не был заинтересован в том, чтобы покрыть расходы на эту функцию, то просто не существует, чтобы начать, так как мы будем вынуждены сократить ежемесячные расходы, которые никто не готов поддерживать. К счастью, мы находимся в лучшем положении, и вы можете решить, если вы готовы использовать `MatchActively` на этих терминах.
