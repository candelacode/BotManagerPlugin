...
Http сервер ASF теперь ожидает входящих запросов на нескольких конечных точках. Если вы не предоставили пользовательский файл конфигурации для IPC, то он будет `localhost`, оба использующих IPv4 **[127. .0.1](http://127.0.0.1:1242)** и **[[::1]](http://[::1]:1242)** на стандартном `1242` порте. Вы можете получить доступ к нашему IPC интерфейсу через вышеперечисленные ссылки, но только с той же машины, что и в процессе ASF.
...
Если вам не требуется доступ из всех источников, а только, например, для вашей локальной сети, тогда лучше проверить локальный IP адрес сервера ASF, например `192.168.0.10` и используйте вместо `*` в примере конфигурации выше. К сожалению, это возможно только в том случае, если ваш LAN адрес всегда одинаков, в противном случае вы, вероятно, будете иметь более удовлетворительные результаты с помощью `*` и вашего собственного брандмауэра поверх того, что позволяет только локальным подсетям получить доступ к порту ASF.
...
server {
    ...
    location ~* /Api/NLog {
        ...
        # Только если вам нужно переопределить хост по умолчанию
# proxy_set_header Host 127. .0. ;

        # При прокси запросах к ASF
        # они имеют решающее значение для правильной идентификации первоначального IP, позволяет ASF напр. забанить фактических правонарушителей вместо вашего сервера nginx
        # Указанное ими позволяет ASF правильно устранять IP адреса пользователей, делающих запросы. Запуск работы nginx в качестве обратного прокси
        # Не задавая их, ASF будет относиться к вашему nginx как к клиенту - в данном случае nginx будет действовать как традиционный прокси
        # Если вы не можете размещать сервис nginx на той же машине, что и ASF, скорее всего вы хотите установить KnownNetworks соответствующим образом помимо
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Host $host:$server_port;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-Server $host;

        # Мы добавляем эти 3 дополнительных опции для websockets proxying, см. https://nginx. rg/ru/docs/http/websocket.html
...
# proxy_set_header Host 127. .0. ;
        ...
        # Х-заголовки всегда должны указываться при проксировании запросов к ASF
        # Они имеют решающее значение для правильной идентификации первоначального IP, позволяет ASF напр. забанить фактических правонарушителей вместо вашего сервера nginx
        # Указанная им возможность позволяет ASF правильно устранять IP адреса пользователей, делающих запросы. Запуск работы nginx в качестве обратного прокси
        # Не указывая их, заставляет ASF относиться к вашему nginx, как к клиенту, nginx будет выступать как к традиционному прокси в данном случае
        # Если вы не можете размещать сервис nginx на той же машине, что и ASF, скорее всего вы хотите установить KnownNetworks соответствующим образом помимо
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Host $host:$server_port;
        proxy_set_header X-Forwarded-Proto $scheme;
