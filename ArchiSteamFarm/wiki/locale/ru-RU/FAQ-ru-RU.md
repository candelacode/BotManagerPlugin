...
- **Вам нужно иметь игру в на вашем аккаунте Steam чтобы получить из неё карточки. Совместное использование семьи не является собственностью и не учитывается.**
- **Ваша игра не может быть помечена как [частная](https://help.steampowered.com/faqs/view/1150-C06F-4D62-4966), ASF автоматически пропускает такие игры во время фермерства.**
...
- **Вы не можете разбрасывать карты на [ограниченных аккаунтах](https://help.steampowered.com/faqs/view/71D3-35C2-AD96-AA3A)вне зависимости от их собственных игр и способа активации.**
- **Из платных игр, которые вы получили бесплатно во время акции, невозможно получить карточки, независимо от того, что указано на странице магазина.**
...
**Да**. Для работы ASF требуются ваши учётные данные, точно так же как для официального клиента Steam, поскольку используются те же методы взаимодействия с сетью Steam. Однако это не означает, что вы должны поместить учетные данные в конфигурации ASF, вы можете использовать ASF с пустым `SteamLogin` и/или `SteamPassword`, и вводите, что данные о каждом запуске ASF, если это требуется (а также несколько других учетных данных для входа, относятся к конфигурации). При таком подходе ваши данные нигде не сохраняются, но конечно же ASF не сможет стартовать без вашей помощи. ASF также предоставляет ещё несколько способов повысить свою **[безопасность](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Security-ru-RU)**, вы можете прочитать о них в соответствующем разделе wiki, если считаете себя продвинутым пользователем. Если вы не считаете себя таковым, и вы не хотите вносить ваши учётные данные в конфигурацию ASF, тогда просто не делайте этого, и вместо этого вводите их по мере необходимости, когда ASF попросит об этом.
...
Лучший способ это сделать - использовать настройку **[`GamesPlayedWhileIdle`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#gamesplayedwhileidle)**, которая будет играть в выбранные вами игры, когда у ASF нет карт для фермы. Если вы хотите накручивать время в этих играх постоянно, даже когда у вас есть карточки для фарма в других играх, вы можете совместить её с **[`FarmPriorityQueueOnly`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#farmingpreferences)**, чтобы ASF пытался получить карточки только для тех игр, которые вы явно указали, или же с **[` FarmingPausedByDefault`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#farmingpreferences)**, который поставит модуль фарма карточек на паузу, пока вы сами его не включите.

Вы также можете воспользоваться командой **[`play`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands#commands-1)**, которая заставит ASF играть в выбранные вами игры. Однако имейте в виду, что ` play ` следует использовать только для игр, в которые вы хотите играть временно, так как это не постоянное состояние, в результате чего ASF возвращается в состояние по умолчанию, например при отключении от сети Steam. Поэтому мы рекомендуем использовать `GamesPlayedWhileIdle`, за исключением случая когда вы хотите запустить избранные игры на короткое время, а затем вернуться к обычной работе.
...
ASF также корректно обрабатывает **многосерийные** видео. В Steam есть много игр с карточками, для которых указан неправильный `appID` на странице значков, например **[Double Fine Adventure](https://store.steampowered.com/app/402590)** - IM будет использовать неправильный `appID`, что приведет к отсутствию карт и зависанию процесса. Опять же, вам придётся либо добавлять его в черный список либо пропускать вручную, и оба варианта требуют вашего внимания. ASF автоматически определяет правильный ` appID ` для фарма, что приводит к выпадению карт.
...
- **Вы можете пропускать игры с возвратом денег** (`SkipRefundableGames` в функции `FarmingPreferences` бота). ASF имеет правильную встроенную логику для игр с возвратом денег, и вы можете настроить ASF таким образом, чтобы он автоматически не фармил игры с возвратом денег. Это позволяет вам оценить, стоит ли свежекупленная в Steam игра своих денег, не давая ASF попытаться получить карточки как можно скорее. Если вы играли в игру более 2 часов, или с момента покупки прошло 2 недели, ASF продолжит фарм игры, поскольку запросить возврат средств за неё уже нельзя. До тех пор, вы полностью контролируете запуск игры, и при необходимости можете запросить возврат средств за неё, и для этого вам не нужно добавлять игру в черный список или выключать ASF.

- **Вы можете пропустить несыгранные игры** (`SkipUnplayedGames` в боте `FarmingPreferences`). ASF имеет надлежащую встроенную логику за часы в играх, и вы можете настроить ASF не фермировать неиспользованные игры автоматически. Это позволяет вам управлять играми и фермой, без необходимости вручную черный список всех их или полностью пропускать с помощью ASF.
...
**Нет**. ASF, в отличие от некоторых других инструментов, которые интегрируются с вашим клиентом Steam, имеет свою независимую реализацию этого клиента, а сеть Steam позволяет играть одновременно **только одному клиенту**. Однако вы в любой момент можете отключить ASF просто запустив игру (и нажав "ОК" когда сеть Steam спросит, нужно ли отключить сеанс на другом компьютере) - ASF будет терпеливо ждать пока вы закончите играть, и после этого продолжит работу. Как альтернатива - вы можете играть в офлайн-режиме в любое время, если вас это устраивает.

Имейте в виду, что в любом случае скорость падения карт при воспроизведении нескольких игр близка к нулю, Таким образом, не существует никаких прямых выгод от того, что они могут делать это с помощью некоторых других инструментов, В то время как существует сильная выгода от отсутствия вмешательства в другие игры, запущенные с ASF, что имеет решающее значение e. . VAC-мудрость.
...
Нет, это не возможно, потому что ASF (в отличие от других инструментов, таких как Idle Master, SGI или SAM) никоим образом не вмешивается ни с паром, ни с его процессами. Физически невозможно получить VAC за использование ASF, даже во время использования защищённых серверов со включенным ASF - просто потому что **ASF даже не нужно чтобы клиент Steam был установлен** для нормальной работы. ASF является одной из немногих фермерских программ, которая в настоящее время может гарантировать VAC-доступ.
...
Поэтому, как и любое ПО для Steam, ASF не одобрен Valve, и я не могу гарантировать что вы не будете забанены если Valve вдруг решит банить аккаунты, с которых пользовались ASF. Это крайне маловероятно, учитывая тот факт, что ASF используется более чем на несколько миллионов аккаунтов Steam, с момента ее первого освобождения, которое произошло более 10 лет назад - но до сих пор возможность, независимо от фактической вероятности.
...
Вы используете это программное обеспечение на свой страх и риск. Это очень маловероятно, что вы можете забанить за это, но если вы делаете, вы можете винить только себя за это.

---

### Забанен ли кто-нибудь за него?
...
Этот случай проанализировать проще, из-за подробностей которые сообщил пользователь. Очевидно пользователь использовал **очень устаревшую версию ASF**, в которой присутствовала ошибка, из-за которой ASF отсылал чрезмерное количество запросов к серверам Steam. Эта ошибка не существовала исходно, она появилась из-за критических изменений в Steam, и была исправлена в будущих версиях ASF. **ASF поддерживается только в [последней стабильной версии](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest), выпущенной на GitHub**.

Нельзя предполагать, что некоторые устаревшие версии ASF будут работать так же, как они используются для работы навсегда, особенно потому, что Steam постоянно меняется независимо от того, нравится ли вам это или нет. Если такая вещь произойдет во всем мире, то она быстро обновляется и выпускается всем пользователям как bugfix. Valve не запретит вдруг более миллиона пользователей ASF из-за нашей или их ошибки, по очевидным причинам. Однако, если вы намеренно отказываетесь от использования последней версии ASF, вы по определению являетесь незначительным меньшинством пользователей, **подверженных подобным инцедентам**, из-за **отсутствия поддержки**, поскольку никто не следит за вашей устаревшей версией ASF, никто не исправляет в ней ошибки и никто не проверяет, не забанят ли вас просто за то, что вы её запустили. **Пожалуйста, пользуйтесь последними версиями программного обеспечения**, не только ASF, но и других приложений тоже.
...
Все случаи выше имеют между собой одно общее - ASF это просто инструмент, и это **вам** решать, как вы будете его использовать. Вас не забанят за само использование ASF, но могут за то **как** вы его используете. Это может быть вспомогательная утилита чтобы фармить только с одного основного аккаунта, или огромная ферма с тысячами ботов. В любом случае, я не даю юридических консультаций, и в первую очередь вы должны сами принять решение об использовании ASF. Я не скрываю никакой информации, которая могла бы вам помочь. тот факт, что ASF забанил некоторых людей (и в каком контексте), так как у меня нет оснований - именно ты выбираешь, что ты хочешь делать с этой информацией.

Если вы спросите меня - руководствуйтесь здравым смыслом, не создавайте больше ботов, чем мы рекомендуем, не отправляйте одновременно сотни обменов, всегда используйте последнюю версию ASF, и всё у вас _должно_ быть в порядке. Каждый инцидент такого рода по **определённым причинам** всегда происходил с людьми, проигнорировавшими наши рекомендации, проверенные методы и советы, полагая, что они знают лучше нас, например, сколько ботов они могут запустить. Было ли это совпадением, или реальным фактором - решать вам. Мы не предлагаем никаких правовых советов, только давая вам наши мысли о том, что вы можете найти полезным, или игнорировать их полностью и действовать только на фактах, о которых говорилось выше.

---

### Какую персональную информацию раскрывает ASF?

Вы можете посмотреть подробности в разделе **[Удалённая связь](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Remote-communication)**. Вам следует ознакомится с этим разделом если вы беспокоитесь о своей конфиденциальности, например если вам интересно почему аккаунты, используемые в ASF, автоматически присоединяются к нашей группе Steam. ASF не собирает никакой критической с точки зрения конфиденциальности информации, и не передаёт подобные данные третьим лицам.
...
Да, и эта версия ни в коем случае не является неподдерживаемой, мы просто не делаем для неё официальную сборку. Ознакомьтесь с разделом **[совместимости](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Compatibility)** для общего варианта. У ASF нет строгой зависимости от ОС, она будет работать где угодно при условии, что вы сможете раздобыть работоспособную .NET runtime, например под 32-разрядной Windows, хотя вы и не найдёте у нас пакета `win-x86`.

---

### ASF - супер! Могу ли я сделать пожертвование?

Да, и мы будем очень рады, что вам нравится наш проект! Вы можете найти различные возможности пожертвования в рамках каждой версии **[](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** а также **[на главной странице](https://github.com/JustArchiNET/ArchiSteamFarm)**. Помимо обычных денежных пожертвований мы также принимаем предметы Steam, так что вы всегда можете пожертвовать скины, ключи или небольшую часть карт, которые вы получили с помощью ASF, если захотите. Заранее спасибо за вашу щедрость!
...
Да, если вы просто хотите запустить ASF с паузами карт, сельскохозяйственный модуль, вы можете установить `FarmingPausedByDefault` в свойстве `FarmingPreferences` для этого установить. Это позволит вам запустить его командой `resume` в процессе работы.

Если вы хотите полностью отключить модуль фермерства на картах и гарантировать, что он никогда не будет запущен без вашего явного указания, то мы рекомендуем установить `FarmPriorityQueueOnly` в `FarmingPreferences` бота, что вместо того, чтобы просто приостановить его, полностью отключит фермерство, пока вы сами не добавите игры в холостую очередь приоритетов.
...
ASF - консольное приложение, у которого нет окна, чтобы сворачиваться, поскольку окно создаётся для вас вашей ОС. Однако вы можете использовать любой сторонний инструмент, способный выполнять эту задачу, например **[RBTray](https://github.com/benbuck/rbtray)** для Windows, или **[screen](https://linux.die.net/man/1/screen)** для Linux/macOS. Это просто примеры, есть много других приложений с похожим функционалом.
...
Да, это можно сделать несколькими способами. Для получения дополнительной информации ознакомьтесь с разделом **[команд](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**.

---

### Я хочу помочь с переводом ASF. Что мне нужно для этого сделать?

Спасибо за ваш интерес! Все детали вы можете найти в разделе **[локализация](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Localization)**.

---

### Я подключил к ASF только один (основной) аккаунт, cмогу ли я как-то посылать команды через чат Steam?

**Да**, это объяснено в разделе **[Команды](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands#notes)**. Вы можете сделать это через групповой чат Steam, хотя использование **[ASF-ui](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC#asf-ui)** может быть легче для вас.

---

### ASF работает, но карточки не выпадают!

Коэффициент фермерства карт отличается от игры в игре, так как вы можете прочитать в разделе **[производительности](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Performance)**. На выпадение требуется некоторое время, обычно **несколько часов на одну игру**, и вам не стоит ожидать что карточки начнут выпадать уже через несколько минут после запуска программы. Если вы видите что ASF активно проверяет наличие карточек, и переключается на следующую игру после того как закончит фарм текущей, то все работает нормально. Также возможно что вы включили такую опцию как `DismissInventoryNotifications` в `BotBehaviour`, которая автоматически отмечает все уведомления о новых предметах в инвентаре как просмотренные. Проверьте конфигурацию **[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration)** для деталей.

---

### Как полностью остановить процесс ASF?

Просто закройте приложение ASF, например кликнув [X] если вы под Windows. Если вместо этого вы хотите остановить определенного бота по вашему выбору, но держать другие будут запущены, затем взгляните на свойства конфигурации `Включено` **[бота](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#bot-config)**, or `stop` **[command](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**. Если вы хотите остановить автоматический процесс земледелия, то оставьте ASF запущенным для вашей учетной записи, затем вот какой вариант `FarmingPausedByDefault` для `FarmingPreferences` в свойстве **[бота](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#bot-config)** и `пауза` **[команда](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)** предназначена.
...
Команда ASF предлагает **владеть** до **10 аккаунтов Steam в сумме**, а значит, и запускать до **10 ботов в сумме**. Все вышеперечисленное не поддерживается и делается на свой страх и риск, вопреки нашему предложению здесь. Эта рекомендация основана на внутренней документации Valve, а также на наших собственных предложениях. Будете ли вы следовать этому правилу или нет - решать вам, ASF это инструмент, который не пойдёт против вашей воли, даже если в результате ваши аккаунты Steam могут быть забанены. Поэтому ASF будет показывать вам предупреждение, если вы превысили рекомендованный предел, но позволит вам запустить что угодно на ваш страх и риск и без нашей поддержки.
...
**No**. ASF не связан ни с какими сервисами и любые такие утверждения - ложь. Ваша учетная запись Steam - это ваше имущество, и вы можете использовать свой аккаунт любым удобным вам способом, но Клапан четко заявил в официальном сообщении **[ToS](https://store.steampowered.com/subscriber_agreement)** следующее:
...
Это ваш аккаунт и ваш выбор. Не говорите что никто не предупреждал вас. ASF это программа, которая соответствует всем требованиям, упомянутым выше, поскольку вы никому не передаёте свои учётные данные, вы используете программу лично, но любые "службы выбивания карточек" потребуют от вас учётные данные, что противоречит требованию выше (даже нескольким). Как с оценкой **[Steam ToS](https://store.steampowered.com/subscriber_agreement)** мы не предлагаем никаких юридических советов, и вы должны решить, хотите ли вы использовать эти сервисы или не - согласно нам **это прямо нарушает [Steam ToS](https://store.steampowered.com/subscriber_agreement)** и может привести к приостановлению, если Клапан обнаружит. Как уже сказано выше, **мы настоятельно рекомендуем НЕ пользовать подобными сервисами**.
...
Есть два способа решить эту проблему. Первый - внести эту игру в чёрный список с помощью **[команды](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)** `fbadd` или **[параметром конфигурации](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration)** `Blacklist`. Таким образом ASF не будет пытаться фармить карточки из этой игры, но это не решит исходную проблему, не позволяющую вам получить карточки из этой игры. Во-вторых, вы можете использовать инструмент самообслуживания поддержки Steam чтобы удалить бесплатную лицензию со своего аккаунта, оставив только полную лицензию с карточками. Чтобы сделать это, сначала зайдите на страницу **[Лицензии и активации ключей](https://store.steampowered.com/account/licenses)** и найдите обе лицензии для этой игры - платную и бесплатную. Обычно это легко - у обоих похожее имя, но у бесплатной к имени добавлено "limited free promotional package" или "promo" в названии товара, и кроме того - в прое "Способ покупки" стоит "Бесплатно". Иногда это более сложно, например если бесплатная лицензия была в составе какого-то набора, и поэтому у неё указано другое имя. Если вы нашли две такие лицензии - значит это действительно описанная выше проблема, и вы можете спокойно удалить бесплатную лицензию, не боясь потерять игру.

Чтобы удалить бесплатную лицензию из вашей учетной записи, посетить страницу поддержки **[Steam](https://help.steampowered.com/wizard/HelpWithGame)** и поместить название игры в поле поиска, игра должна быть доступна в разделе "продукты", нажмите на нее. В качестве альтернативы вы можете просто воспользоваться ссылкой `https://help.steampowered.com/wizard/HelpWithGame?appid=<appID>`, заменив в ней `<appID>` на appID игры, вызывающей проблемы. После этого нажмите кнопку "Я хочу навсегда удалить эту игру со своего аккаунта" и выберите проблемную бесплатную лицензию, которую вы нашли ранее, обычно с припиской "limited free promotional package" в имени (или подобное). После удаления бесплатной лицензии, ASF должен без проблем получить карточки из этой игры, вам стоит перезапустить процесс фарма после удаления, просто чтобы убедиться что Steam отображает на этот раз правильную лицензию.

---

### ASF не находит игру `X` как доступную для фарма, но я знаю что в ней есть коллекционные карточки Steam!

К этому могут привести две причины. Первая, и самая очевидная, причина - это то, что вы ссылаетесь на **магазин Steam**, где указано что в игре имеются коллекционные карточки. Это **неверное** заключение, поскольку эта отметка в магазине только указывает на то, что в игре **есть** коллекционные карточки, но не обязательно значит что эта функция уже **включена**. Вы можете прочитать больше об этом в **[официальном анонсе](https://steamcommunity.com/games/593110/announcements/detail/1954971077935370845)**.

Короче говоря, карта выбрасывает значок в Steam магазине ничего не значит, проверьте свои страницы значков **[](https://steamcommunity.com/my/badges)** для подтверждения, включена ли игра или нет - это также то, что делает ASF. Если игра, о которой вы говорите не отображается в списке, как игра с доступным выпадением карточек, то эту игру **невозможно** фармить, независимо от причин.
...
Разумеется, все это предполагает, что вы запускаете ASF с нетронутыми настройками по умолчанию, так как вы также можете добавить эту игру в черный список фермерства, используйте выбранные `фермерские предпочтения` , такие как `FarmPriorityQueueOnly` или `SkipRefundableGames`, и так далее.
...
### `Невозможно войти в Steam: TryAnotherCM/Invalid`, `ServiceUnavailable/Invalid`

Как и в предыдущем случае, сервер с которым вы подключились явно недоступен. Обычно происходит во время окна обслуживания Steam, вы ничего не можете сделать об этом ASF будет автоматически повторять попытку на другом сервере, пока один не примет его запрос. Он не должен длиться больше часа максимума.

---

### `Не удаётся получить информацию о значках, попробуем позже!`

Обычно это означает, что вы используете родительский PIN-код Steam для доступа к своей учетной записи, но забыли указать его в конфигурации ASF. Вы должны указать действующий PIN-код в свойстве конфигурации бота `SteamParentalCode`, иначе ASF не сможет получить доступ к большей части веб-контента и, следовательно, не сможет работать должным образом. Направляйтесь к настройке **[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration)** для того, чтобы узнать больше об `SteamParentalCode`.
...
Как проверить что Steam упал? **[Статус Steam](https://steamstat.us)** является отличным источником проверки, должен ли Steam **быть** вверх, если вы заметите ошибки, особенно связанные с Сообществом или Web API, то Steam испытывает трудности. Вы можете оставить ASF в покое и позволить ему выполнять свою работу после короткого времени простоя или выйти из него и подождать сами.

Однако это не всегда так, как в некоторых ситуациях проблемы Steam могут не обнаружить статус Steam, Например, такой случай произошл, когда Клапан нарушил поддержку HTTPS сообщества Steam 7 июня 2016 - доступ к **[SteamCommunity](https://steamcommunity.com)** через HTTPS вызвал ошибку. Поэтому не стоит слепо доверять тому, что выдаёт Steam Status, лучше самостоятельно проверить что всё работает как положено.
...
Здесь нет "золотого правила" которое работает для всех, потому что блокировки осуществляются под сильным влиянием сторонних факторов, именно поэтому вам нужно экспериментировать самому и найти значение, которое работает для вас. Вы также можете игнорировать то, что я говорю и использовать что-то вроде `10000`, что гарантирует правильную работу, но потом не жалуйтесь, что ваш ASF реагирует на все по 10 секунд и что проверка значков занимает 5 минут. Кроме того, весьма вероятно, что ни один ограничитель не поможет, поскольку у вас так много ботов, что вы достигли **[жёсткого лимита](#how-many-bots-can-i-run-with-asf)**, упомянутого выше. Да, вполне возможно, что вы сможете без проблем войти в сеть Steam (клиент), но сеть Steam (веб-сайт) откажется вас слушать, если у вас будет установлено 100 сеансов одновременно. ASF для работы нужны как сеть Steam, так и web-подсистема Steam, и отказ любой из них приведёт к проблемам, которые вы не решите.
...
Вероятнее всего вы пользуетесь Windows и у вашей консоли включено выделение мышью. Обратитесь к **[этого](https://stackoverflow.com/questions/30418886/how-and-why-does-quickedit-mode-in-command-prompt-freeze-applications)** вопроса о StackOverflow для технических объяснений. Вам нужно отключить выделение мышью, для чего кликните правой кнопкой мыши на заголовке окна ASF, выберите "Свойства", перейдите на закладку "Настройки" и снимите галочку в соответствующем пункте.
...
Далее, если вы не используете **[2FA ASF](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication)**, Вполне возможно, что ASF на самом деле принял/отправил торговлю, но вы должны подтвердить ее по электронной почте. Аналогично, если вы пользуетесь классической 2ФА, вам нужно подтвердить обмен в вашем аутентификаторе. Подтверждения сейчас **обязательные**, поэтому если вы не хотите подтверждать обмены самостоятельно - подумайте о импорте аутентифицатора в 2ФА ASF.
...
Вы можете удалить `BotName.db` и `BotName.bin` (если доступно) затрагиваемой учетной записи и попробовать снова привязать ASF к вашему аккаунту, но это скорее всего ничего не сделает. Некоторые пользователи сообщили, что **[деавторизовал все устройства](https://store.steampowered.com/twofactor/manage)** на Steam должны помочь, изменение пароля сделает то же самое. Тем не менее, это лишь негарантированные условия работы, которые даже не гарантированы для работы, реальное решение на основе ASF - импортировать ваш аутентификатор как **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication)** - таким образом ASF может генерировать токены автоматически при необходимости, и вам не нужно вводить их вручную. Обычно эта проблема волшебным образом решается сама собой через некоторое время, поэтому вы можете просто подождать, пока это произойдёт. Конечно вы можете также попросить о помощи Valve, потому что я не могу заставить сеть Steam принимать наши ключи входа.
...
### Я не могу ввести данные для входа, которые запрашивает ASF
### `System.InvalidOperationException: Невозможно прочитать ключи, если приложение не имеет консоли или когда консольный вход был перенаправлен`
### `System.IO.IOException: Input/output error`
### `Неверный ввод RequestInput()!`

Если эта ошибка возникла во время ввода ASF (напр. вы можете увидеть `GetUserInput()` в stacktrace), а это вызвано тем, что ваше окружение запрещает ASF читать стандартные входные данные вашей консоли. Это может произойти по многим причинам, но наиболее распространенным является запуск ASF в неправильной среде (напр. в фоне `systemd`, `nohup` или `&` вместо e. . `экран` на Linux). Если ASF не может получить доступ к стандартному потоку ввода, вы уведите в журнале эту ошибку и ASF не сможет использовать ваши данные в процессе работы.

Обычно необходимо исправить вышеуказанную проблему, то есть разрешить ASF доступ к стандартным входным данным, чтобы вы могли предоставить детали. Однако если вы **ожидаете**, что это произойдет, и **намерены** запустить ASF в среде без ввода, то вам следует явно указать ASF, что это так, установив режим **[`Headless`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#headless)** соответствующим образом. В этом режиме ASF не запрашивает пользовательский ввод ни при каких условиях, позволяя вам безопасно использовать ASF в окружении без средств ввода. Вы можете ответить на выбранные сообщения с помощью других средств в этом режиме, например, в ASF-ui.
...
В большинстве случаев эта ошибка вызвана **неверным временем/датой на вашей машине**. У каждого сертификата SSL есть дата выдачи и дата окончания. Если ваша дата является недействительной и за пределами этих двух границ, сертификату нельзя доверять из-за потенциальной **[MITM](https://en.wikipedia.org/wiki/Man-in-the-middle_attack)** и ASF отказывается от соединения.
...
**Убедитесь, что вы скачали ASF из надёжного источника**. Единственным официальным и доверенным источником является **[ASF релизы](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** на GitHub (и это также источник автоматических обновлений ASF) - **любой другой источник по определению не может содержать вредоносное ПО, добавленное другими пользователями** - вы не должны доверять другому местоположению загрузки, Убедитесь, что ваш ASF всегда приходит от нас.

Если вы убедились, что скачали ASF из надёжного источника, то вероятнее всего это просто ложно-положительное срабатывание антивируса. Это **случалось раньше**, **случилось сейчас**, и **будет случаться в будущем**. Если вы беспокоитесь о реальной безопасности использования ASF, то я рекомендую просканировать ASF большим количеством антивирусов для получения реального соотношения срабатываний, например с помощью сайта **[VirusTotal](https://www.virustotal.com)** (или другого аналогичного сервиса на ваш выбор).
