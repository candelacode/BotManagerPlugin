...
Прежде всего, можно отключить все предложения по сделке **** с помощью флага `DisableIncomingTradesParsing` в разделе `BotBehaviour`. Используя это, как видно из названия, отключит все функциональные возможности, связанные с парсингом входящих сделок, , которая включает ниже логику по умолчанию, а также все доступные функции, зависящие от реакции на входящее предложение сделки. Поскольку настройки по умолчанию уже являются ненавязчивыми, вы должны использовать эту опцию только в том случае, если вы не имеете абсолютно никакого намерения со стороны ASF делать все, что связано с входящими сделками.

Ниже описывается логика при разборе входящих торговых предложений, что является параметром по умолчанию.
