...
**`Простой`** алгоритм лучше всего работает для учетных записей, у которых есть неограниченные карты. Это основной используемый ASF алгоритм. Бот находит игры для фарма, и фармит их одну за другой, пока не выпадут все карточки. Это связано с тем, что скорость выпадения карточек при фарме более одной игры одновременно стремится к нулю и это совершенно неэффективно.

**`Complex`** - это новый алгоритм, который был реализован с целью помочь ограниченным аккаунтам увеличить свою прибыль. ASF сначала использует стандартный алгоритм **`Simple`** для всех игр, прошедших `HoursUntilCardDrops` часов воспроизведения, затем, если нет игр с >= `HoursUntilCardDrops` часов, все игры (до `32` максимума) будут совпадать с < `Часы UntilCardDrops` часа одновременно, до тех пор, пока любой из них не попадёт на `Часы UntilCardDrops` часов, затем ASF продолжит цикл с начала (используйте **`Simple`** в этой игре, вернуться к одновременно с < `HoursUntilCardDrops` и так далее). В этом случае мы можем используем фарм нескольких игр для накрутки часов в этих играх до нужного значения. Имейте в виду, что в часы фермерского хозяйства ASF **не** карт фермы, , так что не будет проверять на наличие капли карты в течение этого периода (по вышеуказанным причинам).

На данный момент ASF выбирает алгоритм фарма карточек исключительно основываясь на параметре конфигурации `HoursUntilCardDrops` (который устанавливаете **вы**). Если `HoursUntilCardDrops` имеет значение `0`, Алгоритм **`Простой`** будет использоваться, в противном случае **`Алгоритм Complex`** вместо него будет настроен на сброс времени воспроизведения во всех играх за несколько часов до того, как будет выведено на карту.

---

### **Нет очевидного ответа, какой алгоритм будет лучше для вас**.

Это одна из причин, почему вы не выбираете алгоритм фарма карточек, а вместо этого сообщаете ASF, есть ли на вашем аккаунте ограничения на выпадение карточек, или нет. Если на аккаунте есть неограниченный дроп, алгоритм **`Simple`** будет **работать лучше** на этом аккаунте, так как мы не будем тратить время на доведение всех игр до `X` часов - соотношение дропа карт близко к 0% при фарминге нескольких игр. С другой стороны, если на вашем аккаунте снижается лимит карты, **`Алгоритм Complex`** будет лучше для вас, так как в фермерском доме нет очков, если игра не дошла до `HoursUntilCardDrops` часов - так что мы сначала заработаем **время игры** **затем** карт в режиме соло.
