...
- `linux-arm` работает на 32 битных ОС на базе ARM (ARMv7+) GNU/Linux с glibc 2.35/musl 1.2.3 и новее. Этот вариант охватывает такие платформы, как Raspberry Pi 2 (и более новые), он **не** будет работать с более старыми архитектурами ARM, такими как ARMv6 в Raspberry Pi 0 & 1, он также не будет работать с ОС, не реализующими необходимое окружение GNU/Linux (например, Android).
- `linux-arm64` работает на 64-битных ОС на основе ARMv8+) GNU/Linux с glibc 2.27/musl 1.2.3 и новее. Этот вариант охватывает такие платформы, как Raspberry Pi 3 (и новее), он **не** будет работать с 32-битными ОС, в которых отсутствуют необходимые 64-битные библиотеки (например, 32-битная Raspberry Pi OS), он также не будет работать с ОС, в которых не реализовано необходимое окружение GNU/Linux (например, Android).
- `linux-x64` работает на 64-разрядных ОС GNU/Linux с glibc 2.27/musl 1.2.3 и новее.
- `osx-arm64` работает на 64-разрядных операционных системах macOS на базе ARM (кремний Apple) версии 13 и новее.
- `osx-x64` работает на 64-битных ОС macOS в версии 15 и новее.
- `win-arm64` работает на **последних версиях** 64-битных ОС на базе ARMv8+) Windows в версии 10, 11 и новее.
- `win-x64` работает с **новейшими** 64-битными ОС Windows в версии 10, 11, Server 2016+ и новее.
...
Полный список всех поддерживаемых платформ и ОС от .NET 10.0 см. в примечаниях к выпуску **[](https://github.com/dotnet/core/blob/main/release-notes/10.0/supported-os.md)**.
...
В настоящее время программа ASF ориентирована на **.NET 10.0** (`net10.0`), но в будущем она может быть ориентирована на более новые платформы. `net10.0` поддерживается с 10.0.100 SDK (10.0. время запуска), хотя ASF может предпочитать **последнее время на момент компиляции**, поэтому вы должны убедиться, что у вас есть **[последняя версия SDK](https://dotnet.microsoft.com/download)** (или по крайней мере время запуска) для вашей машины. Generic ASF-вариант может отказаться запускаться, если при компиляции время выполнения старше указанного минимально поддерживаемого.
