...
- If you're using official app and you don't mind resetting your 2FA credentials, the best way is to disable 2FA, then **[create](#creation)** new 2FA credentials by using **[joint authenticator](#joint-authenticator)**, which will allow you to use official app and ASF 2FA. Este método **não requer root**, jailbreak ou qualquer conhecimento avançado, mal seguindo instruções escritas aqui, e é provavelmente superior para este cenário.
...
Em geral, o processo de importação envolve o abandono `maFile` no formato apropriado ao diretório `config` do ASF, sobre o qual o ASF pegará esse arquivo e o removerá automaticamente, por razões de segurança.
...
- Você pode gerar tokens e esses tokens são aceitos pela Steam (você pode entrar com eles)
