...
Em primeiro lugar, é possível desativar **todas as ofertas de troca** usando a bandeira `DisableIncomingTradesParsing` em `BotBehaviour`. Usando isso, como o nome implica, desativará todas as funcionalidades relacionadas ao processamento de negociações de entrada, o que inclui abaixo da lógica padrão, bem como todos os recursos extras disponíveis que dependem de reagir à oferta de troca recebida. Como as configurações padrão já são não-invasivas, você deve considerar usar essa opção somente se você não tiver absolutamente nenhuma intenção do ASF de fazer qualquer coisa relacionada às trocas recebidas.

O abaixo explica lógica quando a análise de ofertas de troca de entrada está habilitada, qual é a opção padrão.
