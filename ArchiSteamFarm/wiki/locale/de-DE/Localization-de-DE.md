...
Wie Sie sehen können, befindet sich das Wort `RateLimited` hier in einem Codeblock und zeigt den internen ASF-Code-Status an, der nicht übersetzt werden sollte. Genauso sollten Sie keine anderen Codeblöcke übersetzen, wie z.B. Namen der Konfigurationseigenschaften (z.B. `TradingPreferences`), Enum-Mitglieder (z. . `Stable` und `PreRelease` Optionen `UpdateChannel`) und ähnliches.
