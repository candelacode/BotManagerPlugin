# ItemsMatcherPlugin (Gegenstandsabgleich‑Plugin)
...
We display all eligible ASF accounts that were active in the **last 15 minutes**. Benutzer werden nach ihrem relativen Nutzen sortiert – `MatchEverything`-Bots mit dem Banner `Any` (alle 1:1 Trades werden akzeptiert), danach zählen die einzigartigen Spiele und schließlich zählt die Anzahl Gegenstände.
...
Ihre Daten werden in maximal 15 Minuten nicht mehr zur Verfügung gestellt, sobald Sie unsere Liste nicht mehr verwenden (bekannt geben) und wird ansonsten maximal zwei Wochen auf unserem Server aufbewahrt - alles wird nach diesem Zeitraum automatisch gelöscht. Dafür ist keine Aktion von Ihnen erforderlich.
...
Es ist uns bewusst, dass die `MatchActively` nicht für alle gilt, dennoch sind wir zuversichtlich, dass Sie nachvollziehen können, weshalb wir es nicht kostenlos anbieten können. Wenn niemand daran interessiert war, die Kosten für dieses Feature zu decken, dann würde es zunächst einfach nicht existieren. da wir gezwungen sein würden, monatliche Ausgaben zu senken, die niemand zu unterhalten bereit ist. Glücklicherweise sind wir in der besseren Position, und Sie können selbst entscheiden, ob Sie `MatchActively` zu diesen Bedingungen verwenden wollen oder nicht.
