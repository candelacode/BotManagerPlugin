...
Wenn Sie die standardmäßige ASF-Protokollierung ohne irgendwelche Veränderung verwenden möchten, müssen Sie nichts tun – auch brauchen Sie dies nicht in der `NLog.config` zu definieren. Zum Vergleich: Das Äquivalent zur fest definierten ASF-Protokollierung (Standard) wäre:
...
Untenstehende Beispiele zeigen Ihnen, wie Sie das Logging nach Ihren Wünschen anpassen können.

Als Starter verwenden wir nur **[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)** Ziel. Unsere initiale `NLog.config` wird wie folgt aussehen:
...
Die Erklärung der obigen Konfiguration ist ziemlich einfach – wir definieren ein **Protokollziel**, das `ColoredConsole` ist, dann leiten wir **alle „Schreiber“** (`*`) der Ebene `Debug` und höher zum Ziel `ColoredConsole` um, das wir zuvor definiert haben.
...
Da wir nicht viele Eigenschaften definiert haben, wie zum Beispiel das `Layout`, wurde dieser auf einen eingebauten Standardwert initialisiert in diesem Fall `${longdate}|${level:uppercase=true}|${logger}|${message}`. Wir können es anpassen, zum Beispiel nur durch Protokollieren von Nachrichten:
...
Das obige Beispiel generiert `logs/chat/0-0-76561198069026042.txt` Datei beim Gespräch mit **[ArchiBot](https://steamcommunity.com/profiles/76561198069026042)**:
...
Dieser verwendet `SteamTarget` um eine Nachricht an die SteamID des Hauptbots zu senden (`76561198006963719`), wenn Bot mit dem Namen `archi` einen Spendenhandel erhält. Benötigt einen anderen Bot in diesem Prozess (da Sie keine Nachrichten an sich selbst senden können).
