...
ASF ist über verschiedene **[Tags](https://hub.docker.com/r/justarchi/archisteamfarm/tags)** verfügbar:


### `main`

Generisches ASF-Build, das vom letzten Commit im `Hauptbereich` erstellt wurde, das gleiche funktioniert wie das Erfassen neuester Artefakte direkt aus unserer **[CI](https://github.com/JustArchiNET/ArchiSteamFarm/actions/workflows/publish.yml?query=branch%3Amain)** Pipeline. Es ist die höchste Stufe von Bug-Software, die Entwicklern und fortgeschrittenen Benutzern für Entwicklungszwecke gewidmet ist. Das Bild wird mit jedem Commit im `Hauptzweig` GitHub aktualisiert, daher können Sie sehr oft Änderungen erwarten (und Dinge gebrochen werden). Es ist hier, um den aktuellen Status des ASF-Projekts zu markieren , die nicht unbedingt garantiert ist, stabil oder getestet zu sein, genau wie in unserem Release-Zyklus. **Dieses Tag sollte in keiner Produktionsumgebung** verwendet werden. Nützlich zur Überprüfung, ob bestimmte Commit das aufgetretene Problem behoben hat, ohne auf eine Vorabversion mit diesem Commit zu warten.


### `released`

Generische Version von ASF, die immer auf die neueste **[verweist, die](https://github.com/JustArchiNET/ArchiSteamFarm/releases)** ASF Version inklusive Pre-Releases. Verglichen mit dem `Haupt-` Tag wird das Bild hier jedes Mal aktualisiert, wenn ein neues GitHub Tag gedrückt wird. Geeignet für Fortgeschrittene und Power-User, die es lieben, am Rande dessen zu leben, was als stabil und zugleich frisch angesehen werden kann. In der Praxis funktioniert es genauso wie ein fortlaufender *Tag* mit dem Hinweis auf die neueste `A.B.C.D` Version zum Zeitpunkt des Downloads (`pull`). Bitte beachten Sie, dass die Verwendung dieses Tags gleich der Verwendung unserer **[Pre-Releases](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle)** ist.

### `stabil`

Generic build of ASF that always points to the latest **[stable](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** ASF version. Verglichen mit dem `freigegebenen` Tag wird das Bild hier aktualisiert, sobald eine neue stabile ASF Version verfügbar ist. Empfohlen für Leute, die nach einer stabilen Variante von `suchen, das` Tag oben erwähnt wurde.

### `latest`

OS-spezifische Version von ASF, die immer auf die neueste **[stabile](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** ASF Version verweist. Im Vergleich zu anderen ist dies der **Only Tag** , der ASF automatische Updates enthält. Der Zweck dieses **Tags** ist es, einen funktionierenden (Standard)-Docker-Container zur Verfügung zu stellen, der selbstständig in der Lage ist, eine aktualisierte, OS-spezifische ASF-Instanz auszuführen. Dadurch muss das Image (Abbild) nicht so oft auf den neuesten Stand gebracht werden, da die enthaltene ASF-Version immer in der Lage ist, sich bei Bedarf selbst zu aktualisieren.

Natürlich kann `UpdatePeriod` sicher ausgeschaltet werden ( `0`), aber in diesem Fall sollten Sie wahrscheinlich `stable` Release verwenden. Ebenso können Sie die Standardeinstellung `UpdateChannel` ändern, um die Vorabversionen zu verfolgen, wenn Sie es wünschen.
...
Generisches ASF-Build, das auf die feste ASF-Version verweist, die dem Tag entspricht. Im Vergleich zu obigen Tags ist dieser Tag vollständig eingefroren, was bedeutet, dass das Bild hier nicht aktualisiert wird, sobald es veröffentlicht wurde. Dies funktioniert ähnlich wie bei unseren GitHub-Versionen, die nach der ersten Version nie wieder berührt werden, was Ihnen eine stabile und gefrorene Umgebung garantiert. Normalerweise sollten Sie diesen Tag verwenden, wenn Sie ein bestimmtes ASF-Release verwenden wollen und deterministisches Ergebnis des Builds erwarten (e. . Sie können ASF-Versionen selbst verwalten).

---

## Welcher *Tag* ist für mich der Beste?

Das hängt davon ab, wonach Sie suchen. Für die Mehrheit der Benutzer sollte das `latest` *Tag* das Beste sein, da es genau das bietet, was Desktop-ASF bietet, nur eben als Dienst in einem speziellen Docker-Container. Jedoch ist dies nicht unbedingt die Art und Weise, wie Docker verwendet werden sollte - normalerweise solltest du deine Container neu bauen und sie nicht ewig laufen lassen, und in diesem Fall sollten Sie `stabil` oder `freigegeben` Tag in Betracht ziehen, die den Docker-Richtlinien folgen. Wenn Sie stattdessen eine reparierte ASF-Version verwenden wollen, dann suchen Sie natürlich nach `A.B.C.D` Releases.
...
Es gibt zwei Wege, um den Benutzer ASF zu ändern. Die erste empfohlene ist `ASF_UID` Umgebungsvariable mit Ziel-UID zu deklarieren, unter der Sie laufen möchten. Zweitens – Alternative – die Übergabe des `--user`-**[Flag](https://docs.docker.com/engine/reference/run/#user)**, was direkt von Docker unterstützt wird.
...
# Alternativ wenn Sie die Einschränkungen unten
Docker ausführen -it -u 1001 -v /home/archi/ASF/config:/app/config --name asf --pull always justarchi/archisteamfarm
```

Der Unterschied zwischen `ASF_UID` und `--user` ist subtil, aber wichtig. `ASF_UID` ist ein benutzerdefinierter Mechanismus, der von ASF unterstützt wird. In diesem Szenario beginnt der Docker Container immer noch als `root`, und dann startet das Start-Skript ASF unter `ASF_UID`. Wenn Sie `--user` verwenden, starten Sie den gesamten Prozess, einschließlich des ASF-Startskripts als gegebener Benutzer. Die erste Option erlaubt es dem ASF-Start-Skript, Berechtigungen und andere Dinge automatisch für Sie zu handhaben und löst einige häufige Probleme, die Sie möglicherweise verursacht haben, zum Beispiel stellt es sicher, dass Ihre `/app` und `/asf` Verzeichnisse tatsächlich `ASF_UID` gehören. Im zweiten Szenario, da wir nicht als `root` ausführen, können wir das nicht gewährleisten, und es wird von Ihnen erwartet, dass Sie das alles selbst im Voraus erledigen.

Wenn Sie sich entschieden haben, `--user` Flag zu verwenden, Sie müssen den Besitz aller ASF-Dateien von Standard `1000` auf Ihren neuen benutzerdefinierten Benutzer ändern. Sie können dies erreichen, indem Sie den folgenden Befehl ausführen:

```shell
# Nur ausführen, wenn Sie den ASF_UID
Docker exec -u root asf_container_name chown -hR 1001 /app /asf verwenden
```
...
Sofern Sie keinen benutzerdefinierten Verschlüsselungsschlüssel oder andere erweiterte Optionen bereitstellen möchten, müssen Sie in der Regel keine speziellen Umgebungsvariablen einbinden da unsere Docker-Container bereits so konfiguriert sind, dass sie mit einer vernünftig erwarteten Standardoption von `--no-restart`laufen, so dass das Flag nicht explizit in `ASF_ARGS` angegeben werden muss.
...
```shell
docker laufen -p 127.0.0.1:1242:1242 -p [::1]:1242:1242 -v /home/archi/ASF/config:/app/config -v /home/archi/ASF/plugins:/app/plugins --name asf --pull always --restart unless-stop justarchi/archisteamfarm
```
...
Sie können den gleichen Effekt von `Docker führen Sie den` Befehl oben aus, indem Sie den folgenden `Docker aus` konfigurieren:

```yaml
Version: "3. "
Dienste:
  asf:
    Bild: justarchi/archisteamfarm
    container_name: asf
    Neustart: nicht weniger gestoppte
    Ports:
      - "127. .0.1:1242:1242"
      - "[::1]:1242:1242"
    Volumen:
...
Von anderen Dingen als bereits oben diskutiert, wir haben `--restart unless-stop` zu beiden obigen Beispielen hinzugefügt, um diesen Container nach einem Neustart des Rechners automatisch zu starten. Fühlen Sie sich frei, es nach Ihren Bedürfnissen zu entfernen/zu ändern.
