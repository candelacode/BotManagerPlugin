...
ASF beinhaltet eine eigene einzigartige IPC-Schnittstelle, die für die weitere Interaktion mit dem Prozess verwendet werden kann. IPC steht für **[Interprozesskommunikation](https://en.wikipedia.org/wiki/Inter-process_communication)** und in der einfachsten Definition ist dies "ASF Web-Schnittstelle", basierend auf **[Kestrel HTTP-Server](https://learn.microsoft.com/aspnet/core/fundamentals/servers/kestrel)** der für weitere Integration in den Prozess verwendet werden kann. sowohl als Frontend für den Endbenutzer (ASF-ui) als auch als Backend für externe Integrationen (ASF API).
...
als bereits erwähnt, ist ASF-UI ein Community-Projekt, das nicht von den Kern-ASF-Entwicklern betreut wird. Es folgt seinem eigenen Fluss in der **[ASF-UI Repository](https://github.com/JustArchiNET/ASF-UI)**, welches für alle damit verbundenen Fragen, Probleme, Fehlerberichte und Vorschläge verwendet werden sollte.
...
Ja, wir empfehlen dafür einen Reverse-Proxy zu verwenden. Auf diese Weise können Sie wie gewohnt auf Ihren Webserver zugreifen, der dann auf dem gleichen Gerät auf die IPC-Schnittstelle von ASF zugreift. Alternativ können Sie **[benutzerdefinierte Konfiguration](#enabling-access-from-all-ips)** mit entsprechender URL verwenden, wenn Sie keinen Reverse-Proxy verwenden möchten. Wenn sich Ihr Gerät beispielsweise in einem VPN mit der Adresse `10.8.0.1` befindet, dann können Sie als Abhör-URL `http://10.8.0.1:1242` in der IPC-Konfiguration einstellen, was den IPC-Zugriff von Ihrem privaten VPN aus ermöglichen würde, aber nicht von irgendwo anders.

### Kann ich die IPC-Schnittstelle von ASF hinter einem Reverse-Proxy als Apache oder Nginx verwenden?

**Ja**, unser IPC ist vollständig kompatibel mit einem solchen Setup, sodass Sie ihn auch für zusätzliche Sicherheit und Kompatibilität vor Ihren eigenen Programmen hosten können, wenn Sie möchten. Im Allgemeinen ist der Kestrel http-Server von ASF sehr sicher und birgt kein Risiko, wenn er direkt mit dem Internet verbunden ist, aber wenn man ihn hinter einen Reverse-Proxy als Apache oder Nginx stellt, kann er zusätzliche Funktionen bieten die sonst nicht möglich wären (z. B. die Sicherung der ASF-Schnittstelle mit einer **[Basic Authentication](https://https://de.wikipedia.org/wiki/HTTP-Authentifizierung#Basic_Authentication)**).
...
server {
    ...
    location ~* /Api/NLog {
        ...
        # Nur wenn Sie den Standardhost
# proxy_set_header Host 127 überschreiben müssen. .0. ;

        # X-Header sollten immer angegeben werden, wenn Anfragen an ASF
        # Sie sind entscheidend für die korrekte Identifizierung der ursprünglichen IP, erlaubt ASF z. die eigentlichen Straftäter an Stelle des nginx Servers
        # angeben erlaubt ASF die korrekte Auflösung der IP-Adressen von Benutzern die Anfragen stellen - nginx als Reverse-Proxy
        # Nicht anzugeben führt dazu, dass ASF Ihre nginx als Client behandelt - nginx wird in diesem Fall als traditioneller Proxy fungieren,
        # Wenn Sie nicht in der Lage sind, nginx-Dienst auf derselben Maschine als ASF zu Hosten Sie möchten KnownNetworks wahrscheinlich zusätzlich zu diesen
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_forentsprechend einstellen ;
        proxy_set_header X-Forwarded-Host $host:$server_port;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-Server $host;

        # Wir fügen diese 3 zusätzlichen Optionen für Websockets Proxying, siehe https://nginx. rg/de/docs/http/websocket.html
...
# proxy_set_header Host 127 überschreiben müssen. .0. ;
        ...
        # X-Header sollten immer angegeben werden, wenn Anfragen an ASF
        # Sie sind entscheidend für die korrekte Identifizierung der ursprünglichen IP, erlaubt ASF z. die eigentlichen Straftäter an Stelle Ihres nginx-Servers
        # angeben erlaubt ASF die korrekte Auflösung der IP-Adressen von Benutzern die Anfragen stellen - nginx als Reverse-Proxy
        # Nicht spezifizieren führt dazu, dass ASF Ihre nginx als Client behandelt - nginx wird in diesem Fall als traditioneller Proxy fungieren,
        # Wenn Sie nicht in der Lage sind, nginx-Dienst auf derselben Maschine als ASF zu Hosten Sie möchten KnownNetworks höchstwahrscheinlich zusätzlich zu diesen
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_forangemessen setzen;
        proxy_set_header X-Forwarded-Host $host:$server_port;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-Server $host;
    }
