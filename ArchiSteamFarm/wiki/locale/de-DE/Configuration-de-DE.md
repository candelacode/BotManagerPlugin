...
Wenn Sie diesen Modus aktivieren, können Sie die benötigte Konsoleneingabe über andere Mittel, z. B. ASF-ui (ASF API), oder durch **[`Eingabe`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-de-DE#input-befehl)** Befehl. Wenn Sie sich nicht sicher sind, wie Sie diese Variable einstellen sollen, belassen Sie diese bei dem Standardwert `false`.
...
Typ `byte` mit einem Standardwert `4`. ASF stellt sicher, dass es mindestens `InventoryLimiterDelay` Sekunden zwischen zwei aufeinanderfolgenden Webinventaranfragen liegen, um ein Auslösen des Anfragelimits zu vermeiden; diese werden zum Beispiel bei der Markierung von Inventarbenachrichtigungen als gelesen markiert, und kann auch von Drittanbieter-Erweiterungen verwendet werden, die das Inventar anderer Benutzer abrufen. Dieses Objekt wird nicht für das Abrufen unseres eigenen Inventars verwendet, da ASF dafür wesentlich effizientere interne Netzwerkanforderungen verwendet, sodass es keine Befehle wie `loot` oder `transfer` in einer Weise beeinflusst. Standardwert von `4` wurde basierend auf der Markierung von Beständen von über 100 aufeinanderfolgenden Bot-Instanzen gesetzt und sollte die meisten (wenn nicht alle) der Benutzer zufrieden stellen. Sie können es jedoch reduzieren oder sogar auf `0` ändern, wenn Sie sehr geringe Bots haben, sodass ASF die Verzögerung ignorieren und Steam-Inventare deutlich schneller markieren wird. Seien Sie jedoch gewarnt, dass ein zu niedriger Wert **definitiv** dazu führt, dass Steam Ihre IP vorübergehend sperrt und keine Anfrage mehr beantwortet wird. Sie müssen diesen Wert möglicherweise auch erhöhen, wenn Sie viele Bots mit vielen Inventar-Anfragen ausführen, obwohl Sie in diesem Fall wahrscheinlich versuchen sollten, die Anzahl dieser Anfragen zu begrenzen. Wenn Sie keinen **triftigen** Grund haben diese Variable zu bearbeiten, sollte der Standardwert belassen werden.
...
Sofern Sie keine zusätzlichen ASF-Funktionen aktivieren möchten, ist es nicht notwendig, die Lizenz zu erwerben/zur Verfügung zu stellen.
...
`ImmutableHashSet<string>` Typ mit einem leeren Standardwert. Diese Eigenschaft definiert die Liste der Plug-in-Assemblynamen, die entweder auf der Sperrliste oder auf der Whitelist stehen, um für automatische Aktualisierungen in Betracht gezogen zu werden, gemäß den unten definierten `PluginsUpdateMode`.
...
Standardmäßig verwendet ASF alle verfügbaren Steam-Protokolle als Maßnahme um Ausfälle und ähnliche Steam-Probleme möglichst zu vermeiden. Normalerweise möchten Sie diese Variable ändern, wenn Sie ASF darauf beschränken möchten, nur ein oder zwei bestimmte Protokolle zu verwenden. Solche Maßnahmen könnten in verschiedenen Situationen erforderlich sein. Etwa wenn Sie den UDP-Verkehr auf Ihrer Firewall blockiert haben und nur den TCP-Verkehr durchlaufen möchten, oder wenn Sie `WebProxy` verwenden und ihn auch für Steam Client-Verbindungen verwenden möchten, da nur das `WebSocket`-Protokoll dies unterstützt.
...
Typ `byte` mit einem Standardwert `1`. Diese Variable definiert den Aktualisierungskanal, der entweder für automatische Aktualisierungen (wenn `UpdatePeriod` größer als `0` ist) oder (Alternativ) Aktualisierungsbenachrichtigungen verwendet wird. Derzeit unterstützt ASF drei Aktualisierungskanäle: `0`, welcher `None` genannt wird, `1`, namens `Stable` (stabil), und den experimentellen Kanal `2` (`Experimental`). Der Kanal `Stable` ist der standardmäßige Veröffentlichungskanal, der von der Mehrheit der Benutzer verwendet werden sollte. `Pre-Release`-Kanal zusätzlich zu `Stable`-Releases, enthält auch **Pre-Releases** für fortgeschrittene Benutzer und andere Entwickler, um neue Funktionen zu testen, bestätigen Sie Bugfixes oder geben Sie Feedback zu geplanten Verbesserungen. **Vor-Release-Versionen enthalten häufig nicht gepatchte Fehler, Work-in-Progress-Funktionen oder umgeschriebene Implementierungen**. Wenn Sie sich nicht als fortgeschrittener Benutzer betrachten, bleiben Sie bitte beim Standard-Aktualisierungskanal `1` (`Stable`). Der `PreRelease`-Kanal ist Benutzern gewidmet, die wissen, wie man Fehler meldet, mit Problemen umgehen und Feedback geben — es wird keine technische Unterstützung gegeben. Sehen Sie sich den **[Veröffentlichungszyklus](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-de-DE#veröffentlichungszyklus)** von ASF an, um mehr darüber zu erfahren. Sie können auch den`UpdateChannel` auf `0` (`None`) setzen, wenn alle Versionsüberprüfungen vollständig deaktivieren werden sollen. Falls `UpdateChannel` auf `0` gesetzt ist, wird die gesamte Funktionalität im Zusammenhang mit Aktualisierungen vollständig deaktiviert, einschließlich des Befehls `update`. Es wird **ausdrücklich** von der Verwendung des `None`-Kanals **abgeraten**, weil Sie sich dadurch allen möglichen Probleme aussetzten (erwähnt in [`UpdatePeriod`](#UpdatePeriod) Beschreibung unten).
...
Typ `string` mit einem Standardwert `null`. Diese Variable definiert eine Web-Proxy-Adresse, die für interne http-bezogene Kommunikation verwendet wird, insbesondere für Dienste wie `github.com`, `api.steampowered.com`, `steamcommunity.com` und `store.steampowered.com`. Es gilt für allgemeine (nicht-bot-spezifische) Kommunikation sowie für bot-spezifische Kommunikation, wenn ihre entsprechende `WebProxy` Konfigurationseigenschaft nicht festgelegt ist. Die Bearbeitung von ASF-Anfragen könnte außerordentlich nützlich sein, um verschiedene Arten von Firewalls zu umgehen, insbesondere die große Firewall in China.
...
Wenn Sie auch interne Steam-Netzwerk-Kommunikation (CMs) benötigen, dann sollten Sie sicherstellen, **[`SteamProtocols`](#steamprotocols)** Bot zu einem Wert zu konfigurieren, der nur Websocket-Transport zulässt, etwa ein Wert von `4`, da nur Websockets für Proxying unterstützt werden.
...
Im Allgemeinen möchten Sie diese Variable bearbeiten, wenn Sie verschiedene Automatisierungseinstellungen im Zusammenhang mit der Aktivität des Bots ändern möchten. Die Standardeinstellungen beinhalten ASF im nicht-invasiven Modus, was nur eine sinnvolle Automatisierung ermöglicht, die nicht gegen den Willen des Benutzers verstößt.
...
`DisableIncomingTradesParsing` verhindert, dass ASF eingehende Handelsangebote analysiert, was bedeutet, dass die gesamte Handelsfunktionalität, die damit zusammenhängt, nicht operativ ist. Da ASF standardmäßig im am wenigsten invasiven Modus arbeitet, werden nur Handelsangebote von `Master`-Benutzern akzeptiert und darüber andere Handelsangebote nie bearbeiten - eingehendes Trades Parsing ist standardmäßig aktiviert. Diese Einstellung ist für Personen am sinnvollsten, die keine zusätzlichen Anforderungen/Overhead im Zusammenhang mit dem Parsen von Trades sicherstellen möchten, wodurch die gesamte Logik im Prozess deaktiviert wird, etwa weil sie wissen, dass ihre Bots niemals Master-Handelsanfragen erhalten werden, und erfordern daher überhaupt keine Teilnahme von ASF an ihrer Handelstätigkeit. Beachten Sie, dass diese Option alle anderen Optionen, die von eingehenden Trades abhängig sind, deaktivieren wird (wie z. B. `AcceptDonations` oder `SteamTradeMatcher`) - benutzerdefinierte Plug-ins können eingehende Handelsangebote auch nicht auf übliche Weise verarbeiten.
...
| 14   | MarketableAscending       | Versuche zuerst Spiele mit nicht vermarktbaren Karten zu bewirtschaften (Warnung: teuer zu berechnen)                   |
| 15   | MarketableDescending      | Versuche zuerst Spiele mit markierbaren Kartenabfällen zu bewirtschaften (Warnung: teuer zu berechnen)                  |
...
Diese Option ist am sinnvollsten, um mit `ShutdownIfPossible` kombiniert zu werden, also wenn alle Konten gestoppt werden, wird ASF ebenfalls herunterfahren Ihre Maschine auszuruhen und erlaubt es Ihnen, andere Aktionen, wie Schlaf oder Herunterfahren zum gleichen Zeitpunkt der letzten Karte fallen zu planen.
...
`AutoUnpackBoosterPacks` entpackt automatisch alle Booster-Packs nach Erhalt neuer Gegenstandsbenachrichtigungen. Dadurch erwerben Sie sofort zusätzliche Kartenfunde, die insbesondere in Kombination mit anderen Optionen gewünscht werden könnten (z. B. `SteamTradeMatcher` oder `CompleteTypesToSend`).
...
`ushort` mit dem Standardwert `1`. Diese Eigenschaft kann einige zusätzliche Online-Funktionen auf der Steam-Plattform aktivieren und ist wie folgt definiert:

| Wert | Name        | Beschreibung                    |
| ---- | ----------- | ------------------------------- |
| 1    | Standard PC | Kein spezieller Modus, Standard |
| 544  | SteamDeck   | Stellt sich als Steam Deck vor  |

Der zugrunde liegende `EGamingDeviceType` Typ, auf dem diese Variable basiert, enthält mehr verfügbare Werte; doch nach bestem Wissen haben sie derzeit keine Wirkung, deshalb wurden sie wegen ihrer Sichtbarkeit abgeschnitten.

Wenn Sie sich nicht sicher sind, wie Sie diese Option konfigurieren, belassen Sie den Standardwert `1`.
...
Typ `string` mit dem Standardwert `null`. ASF verwendet diese Eigenschaft, wenn du dich im Steam Netzwerk einloggst die für die Anpassung verwendet werden kann, um zu sehen, wie genau Steam die ASF Maschine und Session anzeigt, z. . wenn Geräte angezeigt werden, die derzeit in **[eingeloggt sind, hier](https://store.steampowered.com/account/authorizeddevices)**.

ASF bietet einige spezielle Variablen, die Sie optional in ihrem Text verwenden können. `{0}` wird durch den von Ihrem Betriebssystem angegebenen Maschinennamen ersetzt, `{1}` wird durch ASF's public identifier, ersetzt während `{2}` durch die ASF-Version ersetzt wird.

Wenn Sie nicht wissen, was Sie tun, sollten Sie es mit dem Standardwert `Null` behalten. In diesem Fall entscheidet ASF intern über den richtigen Wert, der `{0} ({1}/{2})` von heute. Denken Sie daran, dass dies nur ein Vorschlag ist, dass das Steam-Netzwerk in vollem Umfang respektieren kann oder nicht.
...
| 2    | InJoinableGame     | Der Client ist im beitretbaren Spiel            |
| 8    | RemotePlayTogether | Client verwendet gemeinsame Remote-Play-Session |
...
Weitere Erläuterungen zu diesem Thema finden Sie im Abschnitt **[Telekommunikation](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Remote-communication-de-DE)**. Wenn Sie keinen Grund haben diese Eigenschaft zu bearbeiten, sollte der Standardwert belassen werden.
...
Typ `string` mit dem Standardwert `null`. Diese Eigenschaft definiert den Steam-Familienansicht (PIN). ASF erfordert Zugriff auf Familienansicht-Ressourcen. Wenn Sie diese Funktion verwenden, sollten Sie ASF für eine problemlose Ausführung die PIN zum entsperren verraten. Der Standardwert `null` bedeutet, dass für die Freischaltung dieses Kontos kein Steam-Familienansicht (PIN) erforderlich ist, und das ist wahrscheinlich das, was Sie wollen, wenn Sie nicht die Steam-Elternfunktionalität verwenden.
...
| 16   | MatchActively       | Nimmt aktiv an **[STM](https://www.steamtradematcher.com)**-artigen Handelsangeboten teil. Besuchen Sie **[ItemsMatcherPlugin](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/ItemsMatcherPlugin-de-De#matchactively)** (Gegenstandsabgleich‑Plugin) für weitere Informationen. |
...
Typ `byte` mit einem Standardwert `0`. Diese Eigenschaft legt den Benutzeroberflächenmodus fest, mit dem der Bot nach dem Einloggen im Steam Netzwerk angekündigt wird. Es kann beeinflussen, wie das Konto angezeigt wird, etwa im Steam Chat, sofern Ihre Anwesenheit dies durch `OnlineStatus` erlaubt. Derzeit können Sie einen der folgenden Optionen wählen:

| Wert | Name       | Beschreibung                |
| ---- | ---------- | --------------------------- |
| `0`  | VGUI       | Standard-Steam-Client-Modus |
| `1`  | Tenfoot    | Big picture mode            |
| `2`  | Mobile     | Mobile Steam App            |
| `3`  | Web        | Web-Browsersitzung          |
| `5`  | MobileChat | Mobile Steam Chat-App       |

Der zugrunde liegende `EUIMode` Typ, auf dem diese Eigenschaft basiert, enthält jedoch mehr verfügbare Werte; nach bestem Wissen haben diese derzeit absolut keine Wirkung, deshalb wurden sie von der Sichtbarkeit ausgegrenzt. Sie könnten auch daran interessiert sein, `GamingDeviceType`zu überprüfen, da einige zusätzliche Funktionen dort aktiviert sind.

Wenn Sie sich nicht sicher sind, wie Sie diese Option konfigurieren, belassen Sie den Standardwert `0`.

---

### `WebProxy`

Typ `string` mit dem Standardwert `null`. Diese Eigenschaft definiert eine Web-Proxy-Adresse, die für bot-spezifische interne http-bezogene Kommunikation verwendet wird, insbesondere für Dienste wie `api. teampowered.com`, `steamcommunity.com` und `store.steampowered.com`. Falls nicht gesetzt, wird ASF stattdessen die globale `WebProxy` Einstellung verwenden. Die Bearbeitung von ASF-Anfragen könnte außerordentlich nützlich sein, um verschiedene Arten von Firewalls zu umgehen, insbesondere die große Firewall in China.
...
Wenn du auch interne Steam-Netzwerk-Kommunikation (CMs) benötigst dann sollten Sie sicherstellen, **[`SteamProtocols`](#steamprotocols)** Bot zu einem Wert zu konfigurieren, der nur Websocket Transport zulässt, i. . Ein Wert von `4`, da nur Websockets für Proxying unterstützt werden.
...
Die `log.txt` Datei enthält das Protokoll, das durch deinen letzten ASF-Lauf erzeugt wurde. Diese Datei enthält keine sensiblen Informationen und ist äußerst nützlich, wenn es um Probleme, Abstürze oder einfach nur als Information an dich geht, was im letzten ASF-Lauf passiert ist. Wir werden sehr oft nach dieser Datei fragen, wenn Sie auf Probleme oder Fehler stoßen. ASF verwaltet diese Datei automatisch für Sie, aber Sie können ASF **[Logging](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Logging)** Modul weiter optimieren, wenn Sie fortgeschrittene Benutzer sind.
...
`BotName.keys` ist eine spezielle Datei, die zum Importieren von Schlüsseln in **[Hintergrundspiele einlöst](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Background-games-redeemer)**. Es ist nicht zwingend erforderlich und wird nicht generiert, aber von ASF anerkannt. Diese Datei wird nach dem erfolgreichen Import der Produkt-Schlüssel automatisch gelöscht.

`BotName.maFile` ist eine spezielle Datei, die zum Importieren von **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication)** verwendet werden kann. Es ist nicht zwingend erforderlich und wird nicht generiert, aber von ASF erkannt, wenn dein `BotName` ASF 2FA noch nicht verwendet. Diese Datei wird nach erfolgreichem Import von ASF 2FA automatisch gelöscht.
...
`Immutable List<valueType>` - Unveränderliche Sammlung (Liste) von Werten in der angegebenen `valueType`. In JSON ist es definiert als Array von Elementen in gegebenem `valueType`. ASF verwendet `list` um anzugeben, dass die angegebene Variable mehrere Werte unterstützt und deren Reihenfolge relevant sein kann.

Beispiel für `ImmutableList<byte>`: `"Bauernbestellungen": [15, 11, 7]`

---

`ImmutableHashSet<valueType>` - Unveränderliche Sammlung (Set) von eindeutigen Werten im angegebenen `valueType`. In JSON ist es definiert als Array von Elementen in gegebenem `valueType`. ASF verwendet `HashSet`, um anzugeben, dass eine angegebene Variable nur für eindeutige Werte Sinn ergibt (und deren Reihenfolge unwichtig ist). Daher ignoriert es bewusst alle potenziellen Duplikate während des Parsen (falls sie trotzdem angegeben wurden).

Beispiel für `ImmutableHashSet<uint>`: `"Blacklist": [267420, 303700, 335590]`

---

`ImmutableDictionary<keyType, valueType>` - Unveränderbares Wörterbuch (Karte), das einen eindeutigen Schlüssel abbildet, der in seinem `keyType`angegeben ist, um den Wert in seinem `valueType` anzugeben. In JSON wird es als Objekt mit Schlüssel-Wert-Paaren definiert. Bedenke, dass `keyType` in diesem Fall immer angegeben wird, auch wenn es sich um einen Wert-Typ wie `ulong` handelt. Es gibt auch eine strenge Anforderung, dass der Schlüssel auf der gesamten map (Zuordnung) eindeutig ist, diesmal ebenfalls von JSON erzwungen.
...
Es gibt genau 3 aussagekräftige, verfügbare Flaggen zum Ein-/Ausschalten (`A`, `B`, `C`), und daher `8` mögliche gültige Kombinationen insgesamt:

| Endwert | Aktivierte Flaggen |
| ------- | ------------------ |
| 0       | `None`             |
| 1       | `A`                |
| 2       | `B`                |
...
Um das oben genannte möglich zu machen, muss daher per definitionem jede eigenständige Flagge die Macht von zwei sein. Dies ist der Grund, warum zusätzliche Flagge in oben, `D`, den Wert `8` oder höher tragen müsste.

Üblicherweise werden grafische Werkzeuge die Berechnung für Sie durchführen. Wenn Sie die Konfigurationen manuell bearbeiten, Sie können immer den Rechner verwenden und einfach die zugrunde liegenden Werte aller Markierungen zusammenfügen, die Sie aktivieren möchten - wie im Beispiel oben.

Beispiel: `"SteamProtocols": 7` ((aktiviert Flags mit dem Wert `1`, `2` und `4`, wie oben erläutert)
...
Sie können dieses Verhalten mit `--no-config-watch` **[Kommandozeilenargument](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Command-line-arguments#arguments)** deaktivieren, wenn Sie einen bestimmten Grund haben zum Beispiel wollen Sie von ASF nicht auf Dateiänderungen im Ordner `config` reagieren.