...
Zunächst ist es möglich, **alle** eingehenden Handelsangebote zu deaktivieren, indem Sie `DeableIncomingTradesParsing` in `BotBehaviour` verwenden. Wie der Name schon sagt, wird die Funktionalität im Zusammenhang mit eingehenden Trades deaktiviert, die unten stehende Standardlogik sowie alle verfügbaren zusätzlichen Funktionen beinhaltet, die von der Reaktion auf das eingehende Handelsangebot abhängen. Da die Standardeinstellungen bereits nicht aufdringlich sind, Sie sollten diese Option nur in Erwägung ziehen, wenn Sie absolut keine Absicht von ASF haben, irgendetwas im Zusammenhang mit den eingehenden Transaktionen zu tun.

Die folgende erklärt Logik, wenn eingehende Handelsangebote Parsing aktiviert ist, was die Standardoption ist.
