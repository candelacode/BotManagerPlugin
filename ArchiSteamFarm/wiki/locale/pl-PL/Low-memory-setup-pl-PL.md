...
.NET runtime allows you to **[tweak garbage collector](https://docs.microsoft.com/dotnet/core/run-time-config/garbage-collector)** in a lot of ways, effectively fine-tuning the GC process according to your needs. Udokumentowaliśmy poniżej właściwości, które w naszej opinii są szczególnie ważne.
...
Możesz włączyć wybrane właściwości poprzez ustawienie odpowiednich zmiennych środowiskowych. For example, on Linux (shell):
