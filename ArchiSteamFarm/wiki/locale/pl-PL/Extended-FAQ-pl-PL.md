...
Same goes for security of your PC. Yes, having malware on your PC ruins every single security aspect of ASF, as it can read sensitive details from ASF config files or process memory and even influence the program to do stuff that it wouldn't do otherwise. Nie, ostatnia pęknięta, którą uzyskałeś z wątpliwego źródła, nie była "fałszywie pozytywna", jak ktoś ci powiedział, jest jednym z najskuteczniejszych sposobów przejęcia kontroli nad czyimś PCC, ludzie będą się zarażać, a nawet będą postępować zgodnie z instrukcjami jak fascynować.
...
This way we ensured that whatever was written to `SHA512SUMS` matches the resulting files and they weren't tampered with. However, it doesn't prove yet that `SHA512SUMS` file you checked against really comes from us. Są dwa sposoby, aby to zweryfikować.

The first way, and also the one that ASF uses for auto-update process, is making a call to our backend server by visiting `https://asf.JustArchi.net/Api/Checksum/{Version}/{Variant}` URL, replacing `{Version}` with ASF version number, such as `6.1.4.3`, and `{Variant}` with your selected ASF variant, such as `generic` or `linux-x64`. Suma kontrolna znajduje się w odpowiedzi JSON, którą powinieneś porównać z `SHA512SUMS` i/lub AF zip pliku artefakt. Our server provides checksums only for current **[stable](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** and **[pre-release](https://github.com/JustArchiNET/ArchiSteamFarm/releases)** version of ASF, as only those existing ASFs will ever consider updating to.
...
Drugi sposób obejmuje wykorzystanie wbudowanego pliku `SHA512SUMS.sign` , który zawiera cyfrowy podpis PGP potwierdzający autentyczność `SHA512SUMS`. Ponieważ buduj artefakty i podpis są generowane jako część procesu budowy, nie gwarantuje integralności w przypadku naruszenia GitHub (dlatego używamy naszego własnego niezależnego serwera do celów weryfikacji), ale wystarczy, jeśli pobrałeś ASF z nieznanego źródła i chcesz upewnić się, że jest to prawidłowy artefakt wytworzony przez nasz proces wydawania GitHub, Zamiast zagwarantować, że GitHub nie został całkowicie zagrożony.
