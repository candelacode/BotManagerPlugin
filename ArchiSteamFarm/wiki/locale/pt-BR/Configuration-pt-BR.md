...
Ativar este modo permitirá que você forneça a entrada necessária no console por outros meios, ou seja, pelo ASF-ui (API do ASF) ou por meio do comando **[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-pt-BR#comando-input)**. Se você não tem certeza de como definir essa propriedade, deixe-a com o valor padrão `false`.
...
Tipo `byte flags` com o valor padrão `7`. Esta propriedade define os protocolos do Steam que o ASF usará ao se conectar aos servidores do Steam, conforme definidos abaixo:
...
Por favor, note que esta propriedade é um campo do tipo `flags`, portanto é possível escolher qualquer combinação dos valores disponíveis. Confira o **[mapeamento json](#mapeamento-json)** se você quiser aprender mais. Caso nenhum dos flags seja habilitado, isso resultará na opção `None`, e essa opção é inválida por si só.

Por padrão, o ASF usará todos os protocolos disponíveis do Steam como uma forma de lidar com indisponibilidades e outros problemas semelhantes do Steam. Normalmente, você vai querer alterar esta propriedade se quiser limitar o ASF a usar apenas um ou dois protocolos específicos. Tais medidas podem ser necessárias em várias situações, por exemplo, caso você tenha bloqueado o tráfego UDP no seu firewall e queira garantir que apenas o tráfego TCP seja utilizado, ou caso você esteja usando `WebProxy` e queira usá-lo também para a conexão do cliente do Steam, já que apenas o protocolo `WebSocket` oferece suporte a isso.
...
Tipo `string` com o valor padrão `null`. Esta propriedade define o endereço de proxy web que será usado para comunicação interna relacionada a http, especialmente para serviços como `github.com`, `api.steampowered.com`, `steamcommunity.com` e `store.steampowered.com`. Isso se aplica à comunicação geral (não específica de bot), assim como à comunicação específica de bot caso a propriedade de configuração `WebProxy` equivalente não esteja definida. O uso de proxy para as requisições do ASF pode ser extremamente útil para contornar diversos tipos de firewalls, especialmente o grande firewall da China.
...
Caso você também precise fazer proxy da comunicação interna com a rede do Steam (CMs), então deve garantir a configuração da propriedade **[`SteamProtocols`](#steamprotocols)** do bot para um valor que permita apenas o transporte via websocket, ou seja, um valor `4`, já que apenas websockets são compatíveis com proxy.

Caso não tenha um motivo para editar esta propriedade, você deve mantê-la no padrão.
...
| Valor | Nome                          | Descrição                                                                                                               |
| ----- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 0     | None                          | Nenhum comportamento especial do bot, configurações padrão adequadas                                                    |
...
| 32    | MarkBotMessagesAsRead         | Fará com que o ASF marque automaticamente como lidas as mensagens de outros bots do ASF (executados na mesma instância) |
| 64    | DisableIncomingTradesParsing  | Fará com que o ASF nunca processe ofertas de troca recebidas                                                            |

Por favor, note que esta propriedade é um campo do tipo `flags`, portanto é possível escolher qualquer combinação dos valores disponíveis. Confira o **[mapeamento json](#mapeamento-json)** se você quiser aprender mais. Caso nenhum dos flags seja habilitado, isso resultará na opção `None`.

Em geral, você vai querer modificar esta propriedade caso espere alterar várias configurações de automação relacionadas à atividade do bot. As configurações padrão envolvem o ASF rodando em modo não invasivo, o que habilita apenas automações benéficas que não vão contra a vontade do usuário.
...
`DisableIncomingTradesParsing` fará com que o ASF pare de analisar as ofertas de troca recebidas, o que significa que toda a funcionalidade de troca relacionada a isso ficará inoperante. Como o ASF funciona no modo menos invasivo por padrão, aceitando apenas ofertas de troca de usuários `Master` e superiores, sem nunca mexer em outras ofertas de troca, a análise de trocas recebidas vem habilitada por padrão. Essa configuração faz mais sentido para pessoas que desejam garantir que não haja requisições ou sobrecarga adicionais relacionadas à análise de trocas, desativando toda essa lógica no processo, por exemplo, porque sabem que seus bots nunca receberão solicitações de troca de master e, portanto, não precisam que o ASF participe de sua atividade de trocas. Tenha em mente que, ao especificar esta opção, todas as outras opções que dependem de trocas recebidas também serão desativadas, como `AcceptDonations` ou `SteamTradeMatcher` – plugins personalizados também não poderão processar ofertas de troca recebidas da maneira usual.
...
| 14    | MarketableAscending       | Tenta coletar dos jogos com cartas não comercializáveis primeiro (aviso: caro para calcular)             |
| 15    | MarketableDescending      | Tenta coletar dos jogos com cartas comercializáveis primeiro (aviso: caro para calcular)                 |
...
Tipo `ushort` com valor padrão de `1`. Esta propriedade pode habilitar alguns recursos online adicionais na plataforma Steam e é definida como abaixo:

| Valor | Nome       | Descrição                    |
| ----- | ---------- | ---------------------------- |
| 1     | Padrao     | Nenhum modo especial, padrão |
| 544   | Deck Steam | Apresente-se como Steam Deck |
...
Caso você não tem certeza de como definir essa propriedade, deixe-a com o valor padrão `1`.
...
Tipo `string` com o valor padrão `null`. O ASF usará essa propriedade ao fazer login na rede Steam, que pode ser usado para personalização em relação a como exatamente o Steam exibirá a sessão e o computador do ASF. . ao exibir dispositivos que estão logados em **[aqui](https://store.steampowered.com/account/authorizeddevices)**.

O ASF fornece algumas variáveis especiais que você pode usar em seu texto. `{0}` será substituído pelo nome da máquina como fornecido pelo seu SO, `{1}` será substituído pelo identificador público do ASF, enquanto `{2}` será substituído pela versão do ASF.
...
Caso você não tem certeza de como definir essa propriedade, deixe-a com o valor padrão `0`.

---

### `WebProxy`

Tipo `string` com o valor padrão `null`. Esta propriedade define o endereço de proxy web que será usado para a comunicação interna relacionada a http específica do bot, especialmente para serviços como `api.steampowered.com`, `steamcommunity.com` e `store.steampowered.com`. Caso não esteja definida, o ASF usará a configuração global `WebProxy` mencionada acima. O uso de proxy para as requisições do ASF pode ser extremamente útil para contornar diversos tipos de firewalls, especialmente o grande firewall da China.
...
O arquivo `log.txt` contém o registro gerado pela última execução do ASF. Esse arquivo não contem nenhuma informação confidencial, e é extremamente útil quando acontecem problemas, travamentos ou simplesmente como informação sobre o que aconteceu na última vez que o ASF foi executado. Muitas vezes nós pediremos por esse arquivo se você se deparar com problemas. O ASF gerencia automaticamente esse arquivo para você, mas você pode ajustar ainda mais o módulo do ASF **registro[](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Logging)** se você for um usuário avançado.
...
`NomeDoBot.maFile` é um arquivo especial que pode ser usado para importar **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication)**. Ele não é obrigatório e nem gerado pelo ASF, mas reconhecido por ele se o seu `BotName` ainda não usa o ASF 2FA. Esse arquivo é apagado automaticamente quando o ASF 2FA for importado com sucesso.
