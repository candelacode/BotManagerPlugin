...
O ASF inclui sua própria interface IPC que pode ser usada para maior interação com o processo. IPC significa **[comunicação entre processos](https://pt.wikipedia.org/wiki/Comunica%C3%A7%C3%A3o_entre_processos)** e, na definição mais simples, é a "interface web do ASF" baseada no **[servidor HTTP Kestrel](https://learn.microsoft.com/aspnet/core/fundamentals/servers/kestrel)** que pode ser usada para uma integração adicional com o processo, tanto como um frontend para o usuário final (ASF-ui), quanto como um backend para integrações de terceiros (ASF API).
...
O servidor http do ASF agora vai escutar nos endpoints selecionados. Se você não fornecer um arquivo de configuração personalizado para IPC, ele será `localhost`, ambos baseados em IPv4 **[127.0.0.1](http://127.0.0.1:1242)** e baseados em IPv6 **[[::1]](http://[::1]:1242)** na porta padrão `1242`. Você pode acessar nossa interface IPC através dos links acima, mas somente na mesma máquina que executa o processo ASF.
...
Se você não precisa de acesso vindo de todos os locais, mas apenas de sua LAN, por exemplo, é uma ideia muito melhor verificar o endereço IP local da máquina que hospeda o ASF, por exemplo `192.168.0.10` e usá-lo em vez de `*` no exemplo de configuração acima. Infelizmente, isso só é possível se o endereço da sua LAN for sempre o mesmo, caso contrário, você provavelmente terá resultados mais satisfatórios com `*` e seu próprio firewall, além de permitir que apenas sub-redes locais acessem a porta da ASF.
...
Sim, recomendamos usar um proxy reverso para isso. Dessa forma você pode acessar seu servidor web como de costume, o qual então acessará o IPC do ASF no mesmo computador. Alternativamente, se você não quiser executar com um proxy reverso, pode usar uma **[configuração personalizada](#habilitando-acesso-de-todos-os-ips)** com a URL apropriada para isso. Por exemplo, se seu computador estiver em uma VPN com o endereço `10.8.0.1`, então você pode configurar `http://10.8.0.1:1242` como URL de escuta na configuração do IPC, o que habilitaria o acesso IPC de dentro da sua VPN privada, mas não de outro lugar.
...
server {
    ...
    location ~* /Api/NLog {
        ...
        # Somente se você precisar sobrescrever o host padrão
#       proxy_set_header Host 127.0.0.1;

        # Os cabeçalhos X devem sempre ser especificados ao fazer proxy de requisições para o ASF
        # Eles são cruciais para a identificação correta do IP original, permitindo que o ASF, por exemplo, bloqueie os verdadeiros infratores em vez do seu servidor nginx
        # Especificá-los permite que o ASF resolva corretamente os endereços IP dos usuários que fazem as requisições – fazendo o nginx funcionar como um proxy reverso
        # Não especificá-los fará com que o ASF trate seu nginx como o cliente – o nginx atuará como um proxy tradicional neste caso
        # Se você não puder hospedar o serviço nginx na mesma máquina que o ASF, provavelmente vai querer configurar a opção KnownNetworks adequadamente, além disso
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Host $host:$server_port;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-Server $host;

        # Adicionamos essas 3 opções extras para proxy de websockets, veja https://nginx.org/en/docs/http/websocket.html
