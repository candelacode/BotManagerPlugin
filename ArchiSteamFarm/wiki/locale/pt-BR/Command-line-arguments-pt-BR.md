...
`--no-restart` - por padrão, o ASF segue a propriedade de configuração **[`AutoRestart`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#autorestart)**, que você pode usar para especificar se o reinício é permitido quando necessário. Algumas soluções que fornecemos assumem o gerenciamento do processo e são explicitamente incompatíveis com a função de reinício automático do ASF, como executar o ASF em `docker` ou `systemd`. Caso considerem apropriado, elas exigem apenas que o processo seja finalizado, pois é responsabilidade delas reiniciá-lo posteriormente. Como a edição arbitrária da configuração não é desejada para a experiência do usuário, este parâmetro simplesmente substitui a propriedade de configuração `AutoRestart`, inicializando-a explicitamente como `false`, mesmo que você tenha especificado outro valor na configuração. Graças a isso, o ASF pode ser informado antecipadamente sobre a execução em um ambiente desse tipo, sem a necessidade de fornecer um arquivo de configuração compatível com `AutoRestart: false`.

Além do mencionado acima, `--no-restart`, ao contrário de `AutoRestart: false`, também impedirá você de usar o comando `restart` ou qualquer outra forma de solicitar o reinício do processo do ASF, pois este parâmetro indica explicitamente que não é compatível com esse tipo de configuração, enquanto a propriedade `AutoRestart` apenas especifica o comportamento padrão.

Normalmente, você pode (e deve) controlar o comportamento explicado aqui no arquivo de configuração. Caso esteja executando o ASF dentro de um script personalizado ou outro ambiente similar, talvez também queira utilizar este parâmetro, que impedirá o ASF de reiniciar automaticamente.
