...
Antes de tudo, é possível desativar **todas** as ofertas de troca recebidas, utilizando a opção `DisableIncomingTradesParsing` em `BotBehaviour`. Usando isso, como o nome implica, desativará todas as funcionalidades relacionadas ao processamento de negociações de entrada, o que inclui abaixo da lógica padrão, bem como todos os recursos extras disponíveis que dependem de reagir à oferta de troca recebida. Como as configurações padrão já são não intrusivas, você deve considerar o uso dessa opção apenas caso realmente não tenha qualquer intenção de que o ASF faça algo relacionado às trocas recebidas.
