...
As dicas abaixo **envolvem um aumento significativo no uso de memória** e, portanto, devem ser usados com cautela.
...
./ArchiSteamFarm # Para build específica do Sistema Operacional
./ArchiSteamFarm.sh # Para build genérica
```
...
.\ArchiSteamFarm.exe # Para build específica do Sistema Operacional
.\ArchiSteamFarm.cmd # Para build genérica
```
...
Usando as recomendações acima permite que você tenha um desempenho superior para o ASF que deverá ficar muito mais rápido mesmo com centenas ou milhares de bots habilitados. O CPU não deverá mais ser um gargalo, já que o ASF pode usar todo o desempenho do seu CPU caso necessário, reduzindo o tempo necessário ao mínimo possível. O próximo passo seria fazer upgrades de CPU e RAM ou dividir a carga de trabalho entre vários servidores e instâncias do ASF.