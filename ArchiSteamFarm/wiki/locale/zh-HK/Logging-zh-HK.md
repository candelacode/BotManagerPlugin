...
如果您要在不進行任何修改的情況下使用預設 ASF 日誌記錄，則無需執行任何操——您也不需要在自訂 `NLog.config`中定義它。 但是，作為參考，相當於硬編碼的ASF默認日誌記錄將是：
...
Examples below show you how you can customize logging to your liking.

As a starter we will use **[ColoredConsole](https://github.com/nlog/nlog/wiki/ColoredConsole-target)** target only. 我們的初始` NLog.config `將如下所示：
...
Since we didn't define a lot of properties, such as `layout`, it was initialized to a default built-in value, in this case `${longdate}|${level:uppercase=true}|${logger}|${message}`. We can customize it, for example by logging message only:
...
This one uses `SteamTarget` in order to send a message to the main bot's steamID (`76561198006963719`) when bot named `archi` receives a donation trade. Requires another bot in the process (since you can't send messages to yourself).
