...
| 14 | MarketableAscending       | Try to farm games with unmarketable card drops first (warning: expensive to calculate) |
| 15 | MarketableDescending      | Try to farm games with marketable card drops first (warning: expensive to calculate)   |
...
預設值為 `null` 的 `string` 類型。 ASF will use this property when logging in to Steam network, which can be used for customization in regards to how exactly Steam will display ASF machine and session, e.g. when displaying devices that are currently logged in **[here](https://store.steampowered.com/account/authorizeddevices)**.
...
`ImmutableHashSet<valueType>` - Immutable collection (set) of unique values in given `valueType`. 在 JSON 中, 它被定義為給定 `valueType` 中的元素陣列。 ASF uses `HashSet` to indicate that given property makes sense only for unique values and that their order doesn't matter, therefore it'll intentionally ignore any potential duplicates during parsing (if you happened to supply them anyway).
