...
ASF theo mặc định **không** kèm sẵn `MonitoringPlugin`, nhưng nó được kèm như một phần tùy chọn tại mọi bản ra mắt của ASF. Download the plugin from the official **[release](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** that matches your ASF version, then create a dedicated `plugins/ArchiSteamFarm.OfficialPlugins.Monitoring` directory for the plugin, and finally extract the archive there.
